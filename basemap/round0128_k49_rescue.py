...
GRAPH_PART_SCHEMA = "round0128-jina-diverse-25m-k49-graph-part-v1"
GRAPH_SHARD_SCHEMA = "round0128-jina-diverse-25m-k49-graph-shard-v1"
GRAPH_SCHEMA = "round0128-jina-diverse-25m-k49-fuzzy-graph-v1"
TRAIN_CONFIG_SCHEMA = "round0128-diverse-jina-k49-train-config-v1"
PRODUCTION_CONFIG_SCHEMA = "round0128-production-config-v1"
TRAIN_RECEIPT_SCHEMA = "round0128-diverse-jina-k49-train-receipt-v1"
CORE_SCHEMA = "round0128-diverse-jina-k49-core-geometry-v1"
OOD_SCHEMA = "round0128-diverse-jina-k49-ood-evaluation-v1"
MATCHED_DENSITY_SCHEMA = "round0128-k49-matched-fineweb-density-v1"
DECISION_SCHEMA = "round0128-k49-degree-rescue-decision-v1"

MAP_KEY = "r0128-diverse-jina-25m-k49-seed42"
MAP_LABEL = MAP_KEY
POSITIVE_DESTINATION_POLICY = (
    "R0128-global-retained-fuzzy-tconorm-k49-graph"
)
...
class Round0128Error(RuntimeError):
    """The R0128 single-treatment contract was violated."""
...
def verify_r0107_seed42_initial_state(model: Any) -> dict[str, Any]:
    ...
    if observed != R0107_SEED42_INITIAL_STATE_SHA256:
        raise Round0128Error("seed-42 initialized model state changed")
    return {
        "algorithm": (
...
def _signature_payload(value: Mapping[str, Any]) -> tuple[int, str]:
    try:
        ...
        raise Round0128Error("artifact signature is malformed") from exc
...
def assert_treatment_isolation(
    ...
    if (
        ...
        raise Round0128Error("R0107 and R0128 doses are not exact")
    if _signature_payload(left["input"]["compact_mapping"]) != (
        _signature_payload(right["input"]["compact_mapping"])
    ):
        raise Round0128Error("compact retained population changed")
    if int(left["graph"]["n_neighbors_including_self"]) != 16:
        raise Round0128Error("control is not the reviewed k15 graph")
    if int(right["graph"]["n_neighbors_including_self"]) != N_NEIGHBORS:
        raise Round0128Error("treatment is not the registered k49 graph")
    ...
    if right != left:
        raise Round0128Error(
            "R0128 config changes more than graph identity/degree metadata"
        )
    return {
...
def paired_density_materiality(
    *,
    ...
    if (
        ...
        raise Round0128Error("paired native density arrays changed")
...
def _metric(value: Mapping[str, Any], *keys: str) -> float:
    ...
    for key in keys:
        if not isinstance(current, Mapping) or key not in current:
            raise Round0128Error(f"missing comparison metric {'/'.join(keys)}")
        current = current[key]
    try:
    ...
    if not np.isfinite(result):
        raise Round0128Error(f"nonfinite comparison metric {'/'.join(keys)}")
    return result
...
def noninferiority_checks(
    *,
    ...
    if not isinstance(treatment_core_checks, Mapping):
        raise Round0128Error("treatment core checks are missing")
