...
def excluded_source_edge_ranges(sources, excluded_source_rows):
    """Return sorted-edge ranges whose sources are excluded, plus their count."""
    src = np.asarray(sources)
    excluded = np.asarray(excluded_source_rows, dtype=np.int64)
    if (excluded.ndim != 1 or not np.array_equal(excluded, np.unique(excluded))):
        raise ValueError("excluded source rows must be sorted and unique")
    previous = None
    for start in range(0, len(src), 5_000_000):
        block = src[start:start + 5_000_000]
        if (len(block) and previous is not None and int(block[0]) < previous) or (
                len(block) > 1 and np.any(block[1:] < block[:-1])):
            raise ValueError("retained-node sampling requires a source-sorted edge list")
        if len(block):
            previous = int(block[-1])
    left = np.searchsorted(src, excluded, side="left")
    right = np.searchsorted(src, excluded, side="right")
    ranges = tuple((int(lo), int(hi)) for lo, hi in zip(left, right) if hi > lo)
    return ranges, int(np.sum(right - left, dtype=np.int64))
...
class HostStreamEdgeSampler:
    ...
    def __init__(self, dataset, sources, targets, weights, n_nodes,
        ...
        if retained_node_rows is not None:
            ...
            ranges, self.excluded_positive_edges = excluded_source_edge_ranges(
                self._src_h, excluded)
            ...
            self._excluded_source_ranges = ranges
