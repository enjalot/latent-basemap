"""Coverage-aligned training contract for the deliberate balanced-120M rung."""
...
from .round0065_substrates import (
    SUBSTRATE_SCHEMA,
    subset_spec,
)
...
class Round0079Error(RuntimeError):
    """The balanced-120M training capability contract changed."""
...
def train_config_from_capabilities(
    *,
    ...
    if (
        ...
        or graph_manifest.get("round_id") != "0078"
        ...
        raise Round0079Error("balanced-120M graph/substrate geometry changed")
    ...
    config["schema"] = "round0079-production-config-v1"
    config["phrase"] = (
        "balanced 120M MiniLM seed42 native-k15 coverage-aligned rung"
    )
    config["row_universe"] = {
    ...
    config["execution"] = {
        ...
        "expected_pipeline_stamp": {
            ...
            "positive_destination_policy": (
                "native-balanced-120m-representative-only-k15;self-removed"
            ),
            "negative_sampling": (
                "uniform-balanced-120m-retained-rows-nonself"
            ),
        ...
        "scale_transition": {
            ...
            "selected_tier": TIER,
            "selection": (
                "next deliberate rung after reviewed 90M noninferiority "
            ...
            "treatment": "balanced 90M evidence to balanced 120M rung",
    ...
    config["transform"] = {
        ...
        "input": "balanced-120m-int8-plus-exact-fp16-row-scale",
