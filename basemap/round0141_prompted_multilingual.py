"""Prompted Jina embeddings for R0087's second multilingual tranche.

R0141 is deliberately a data-production capability.  It preserves the exact
R0087 row order for Japanese, Korean, and Dutch while replacing the historical
...
LANGUAGE_TRANCHES = (
    {
        "node_id": "embed_jpn_Jpan",
        "language": "jpn_Jpan",
        "dataset": "fineweb2-jpn_Jpan-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [0, ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [16_645_462, 17_480_916],
    },
    {
        "node_id": "embed_kor_Hang",
        "language": "kor_Hang",
        "dataset": "fineweb2-kor_Hang-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [ROWS_PER_LANGUAGE, 2 * ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [17_480_916, 18_316_370],
    },
    {
        "node_id": "embed_nld_Latn",
        "language": "nld_Latn",
        "dataset": "fineweb2-nld_Latn-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [2 * ROWS_PER_LANGUAGE, 3 * ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [18_316_370, 19_151_824],
    },
)
CORPUS_ROWS = 3 * ROWS_PER_LANGUAGE
R0087_GLOBAL_START = 16_645_462
R0087_GLOBAL_STOP = 19_151_824
...
NODE_SCHEMA = "round0141-canonical-document-multilingual-node-v1"
CORPUS_SCHEMA = "jina-document-multilingual-jpn-kor-nld-2p506m-v1"
CAPABILITY = CORPUS_SCHEMA


class Round0141Error(RuntimeError):
    """The exact R0141 data-production contract was violated."""
...
def validate_seal(value: Mapping[str, Any], *, label: str) -> None:
    ...
    if value.get("identity_sha256") != sha256_bytes(canonical_json(body)):
        raise Round0141Error(f"{label} identity seal is invalid")
...
def load_inventory_manifest(
    ...
    *,
    ...
    if signature["sha256"] != expected_sha256:
        raise Round0141Error("R0087 inventory manifest bytes changed")
    ...
    if (
        ...
        or positions != [12, 13, 14]
        or any(
            int(budgets.get(dataset, -1)) != ROWS_PER_LANGUAGE
            for dataset in datasets
        )
    ):
        raise Round0141Error("R0087 multilingual inventory capability is not exact")
    ...
    *,
    ...
    for tranche in LANGUAGE_TRANCHES:
        ...
        if (
            ...
            raise Round0141Error(f"R0087 range is malformed for {dataset}")
...
def tranche_for_node(node_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise Round0141Error(f"unknown R0141 work node {node_id!r}")
...
def source_for_node(
    ...
    if len(matches) != 1:
        raise Round0141Error(f"source layout does not close for {node_id}")
    return matches[0]


def validate_source_layout(layout: Sequence[Mapping[str, Any]]) -> None:
    if len(layout) != len(LANGUAGE_TRANCHES):
        raise Round0141Error("multilingual source layout has wrong tranche count")
    corpus_cursor = 0
    ...
    for expected, observed in zip(LANGUAGE_TRANCHES, layout, strict=True):
        ...
        if (
            ...
            raise Round0141Error("multilingual source layout identity changed")
    ...
    if corpus_cursor != CORPUS_ROWS or accepted_cursor != R0087_GLOBAL_STOP:
        raise Round0141Error("multilingual source layout did not close")
...
def validate_node_receipt(
    ...
    receipt = dict(value)
    validate_seal(receipt, label=f"R0141 node {node_id}")
    ...
    if (
        ...
        raise Round0141Error(f"R0141 node {node_id} contract changed")
    ...
    if (
        ...
        raise Round0141Error(f"R0141 node {node_id} byte closure changed")
    ...
    for chunk in chunks:
        ...
        if (
            ...
            raise Round0141Error(f"R0141 node {node_id} chunk coverage changed")
        cursor = stop
    ...
    if (
        ...
        raise Round0141Error(f"R0141 node {node_id} telemetry is invalid")
    return receipt
...
def validate_coverage(chunks: Sequence[Mapping[str, Any]]) -> None:
    ...
    for chunk in sorted(
        ...
        if local_start != prior:
            raise Round0141Error("multilingual per-language row order changed")
    ...
    if (
        ...
        raise Round0141Error("canonical multilingual corpus does not close")
