"""Prompted Jina embeddings for the Polish/Portuguese/Russian tranche.

R0143 is deliberately a data-production capability.  It preserves the exact
R0087 row order for Portuguese and Russian, and uses R0087's authenticated
held-out Polish probe as a prompted-corpus-only source.  It replaces the
historical raw convention with the literal ``Document: `` convention accepted
in R0114.
...
LANGUAGE_TRANCHES = (
    {
        "node_id": "embed_pol_Latn",
        "language": "pol_Latn",
        "dataset": "fineweb2-pol_Latn-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [0, ROWS_PER_LANGUAGE],
        "r0087_global_row_range": None,
        "inventory_role": "heldout-language-probe",
    },
    {
        "node_id": "embed_por_Latn",
        "language": "por_Latn",
        "dataset": "fineweb2-por_Latn-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [ROWS_PER_LANGUAGE, 2 * ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [19_151_824, 19_987_278],
        "inventory_role": "training-candidate",
    },
    {
        "node_id": "embed_rus_Cyrl",
        "language": "rus_Cyrl",
        "dataset": "fineweb2-rus_Cyrl-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [2 * ROWS_PER_LANGUAGE, 3 * ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [19_987_278, 20_822_732],
        "inventory_role": "training-candidate",
    },
)
CORPUS_ROWS = 3 * ROWS_PER_LANGUAGE
R0087_GLOBAL_START = 19_151_824
R0087_GLOBAL_STOP = 20_822_732
...
NODE_SCHEMA = "round0143-canonical-document-multilingual-node-v1"
CORPUS_SCHEMA = "jina-document-multilingual-pol-por-rus-2p506m-v1"
CAPABILITY = CORPUS_SCHEMA


class Round0143Error(RuntimeError):
    """The exact R0143 data-production contract was violated."""
...
def validate_seal(value: Mapping[str, Any], *, label: str) -> None:
    ...
    if value.get("identity_sha256") != sha256_bytes(canonical_json(body)):
        raise Round0143Error(f"{label} identity seal is invalid")
...
def load_inventory_manifest(
    ...
    *,
    ...
    if signature["sha256"] != expected_sha256:
        raise Round0143Error("R0087 inventory manifest bytes changed")
    ...
    selected = [
        item for item in LANGUAGE_TRANCHES
        if item["r0087_global_row_range"] is not None
    ]
    datasets = [str(item["dataset"]) for item in selected]
    source_order = list(selection.get("source_order") or [])
    try:
        positions = [source_order.index(dataset) for dataset in datasets]
    except ValueError as error:
        raise Round0143Error(
    ...
    if (
        ...
        or (selection.get("budgets") or {}).get(
            "fineweb2-pol_Latn-chunked-500-jina-v5-nano"
        ) is not None
        or (value.get("inventory") or {}).get(
            "fineweb2-pol_Latn-chunked-500-jina-v5-nano", {}
        ).get("role") != "heldout-language-probe"
    ):
        raise Round0143Error("R0087 multilingual inventory capability is not exact")
...
def source_layout_from_inventory(
    ...
    *,
    ...
    """Map two selected ranges plus the authenticated held-out Polish source."""
    all_ranges = list((inventory.get("selection") or {}).get("ranges") or [])
    catalog = dict(inventory.get("inventory") or {})
    ...
    for tranche in LANGUAGE_TRANCHES:
        ...
        expected_global = tranche["r0087_global_row_range"]
        inventory_role = str(tranche["inventory_role"])
        if expected_global is None:
            entry = dict(catalog.get(dataset) or {})
            shards = list(entry.get("shards") or [])
            if (
                matches
                or entry.get("role") != "heldout-language-probe"
                or entry.get("language") != tranche["language"]
                or int(entry.get("rows", -1)) != 2_000_000
                or len(shards) != 1
            ):
                raise Round0143Error(
                    "Polish must remain an authenticated, unselected R0087 probe"
                )
            shard = dict(shards[0])
            selected = None
        else:
        ...
        if (
            ...
            raise Round0143Error(f"R0087 range is malformed for {dataset}")
...
def tranche_for_node(node_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise Round0143Error(f"unknown R0143 work node {node_id!r}")
...
def source_for_node(
    ...
    if len(matches) != 1:
        raise Round0143Error(f"source layout does not close for {node_id}")
    return matches[0]


def validate_source_layout(layout: Sequence[Mapping[str, Any]]) -> None:
    if len(layout) != len(LANGUAGE_TRANCHES):
        raise Round0143Error("multilingual source layout has wrong tranche count")
    corpus_cursor = 0
    ...
    for expected, observed in zip(LANGUAGE_TRANCHES, layout, strict=True):
        ...
        accepted_rows = observed.get("r0087_global_row_range")
        ...
        if (
            ...
            or observed.get("inventory_role") != expected["inventory_role"]
            ...
            or (
                ...
                and int(accepted_rows[0]) != accepted_cursor
            )
            ...
            raise Round0143Error("multilingual source layout identity changed")
    ...
    if corpus_cursor != CORPUS_ROWS or accepted_cursor != R0087_GLOBAL_STOP:
        raise Round0143Error("multilingual source layout did not close")
...
def validate_node_receipt(
    ...
    receipt = dict(value)
    validate_seal(receipt, label=f"R0143 node {node_id}")
    ...
    if (
        ...
        or receipt.get("inventory_role") != expected["inventory_role"]
        ...
        raise Round0143Error(f"R0143 node {node_id} contract changed")
    ...
    if (
        ...
        raise Round0143Error(f"R0143 node {node_id} byte closure changed")

    cursor = 0
    corpus_offset = int(expected["corpus_global_row_range"][0])
    accepted_range = expected["r0087_global_row_range"]
    accepted_offset = int(accepted_range[0]) if accepted_range is not None else None
    for chunk in chunks:
        ...
        if (
            ...
            raise Round0143Error(f"R0143 node {node_id} chunk coverage changed")
        cursor = stop
    ...
    if (
        ...
        raise Round0143Error(f"R0143 node {node_id} telemetry is invalid")
    return receipt
...
def validate_coverage(chunks: Sequence[Mapping[str, Any]]) -> None:
    ...
    for chunk in sorted(
        ...
        accepted_range = chunk.get("r0087_global_row_range")
        if expected is not None and expected["r0087_global_row_range"] is None:
            accepted_valid = accepted_range is None
            accepted_stop = accepted_cursor
        else:
            accepted_values = list(accepted_range or [])
            accepted_valid = (
                len(accepted_values) == 2
                and int(accepted_values[0]) == accepted_cursor
                and int(accepted_values[1]) - int(accepted_values[0])
                == local_stop - local_start
            )
            accepted_stop = (
                int(accepted_values[1]) if len(accepted_values) == 2
                else accepted_cursor
            )
        if (
            expected is None
            or chunk.get("dataset") != expected["dataset"]
        ...
        if local_start != prior:
            raise Round0143Error("multilingual per-language row order changed")
    ...
    if (
        ...
        raise Round0143Error("canonical multilingual corpus does not close")
...
def source_manifest_summary(
    ...
    return {
        ...
        "heldout_prompted_languages": [
            item["language"]
            for item in LANGUAGE_TRANCHES
            if item["r0087_global_row_range"] is None
        ],
    }
