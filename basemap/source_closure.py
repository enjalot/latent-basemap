...
# Round 0017 reuses the unchanged Round-0016 science and exclusive observer,
# while binding only its fresh round/review/release metadata and node wrapper.
ROUND0017_RUNTIME_ENTRYPOINTS = (
    "basemap/gate_preparation.py",
    "basemap/queue_admission.py",
    "basemap/release_preflight.py",
    "basemap/round0014_program.py",
    "basemap/round0014_transform.py",
    "basemap/round0016_service.py",
    "basemap/round0017_admission.py",
    "basemap/round0017_program.py",
    "basemap/round0017_staging.py",
    "basemap/roundwatch_gate.py",
    "basemap/run_controller.py",
    "basemap/source_closure.py",
    "experiments/run_round0014_node.py",
    "experiments/run_round0017_node.py",
)
...
def round0017_source_closure_receipt(
        repo_root: str, *, require_tracked: bool = True) -> dict:
    """Bind the exact tracked runtime for the one Round 0017 queue."""
    return _source_closure_receipt(
        os.path.realpath(repo_root), require_tracked=require_tracked,
        entrypoints=ROUND0017_RUNTIME_ENTRYPOINTS,
        schema="round0017-runtime-source-closure-v1")
...
def validate_round0017_source_closure_receipt(
        recorded: dict, *, repo_root: str) -> dict:
    """Reopen the exact Round 0017 source closure without variants."""
    if (not isinstance(recorded, dict) or
            recorded.get("schema") != "round0017-runtime-source-closure-v1" or
            tuple(recorded.get("entrypoints") or ()) !=
            ROUND0017_RUNTIME_ENTRYPOINTS):
        raise RuntimeError("Round 0017 source closure identity/entrypoints changed")
    current = round0017_source_closure_receipt(
        repo_root, require_tracked=True)
    if current != recorded:
        raise RuntimeError("Round 0017 tracked runtime import closure changed")
    return current
