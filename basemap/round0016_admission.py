"""CUDA-uninitialized validation for the exact Round 0016 queue manifest."""
...
def validate_round0016_queue_manifest(data: dict[str, Any],
                                      path: str) -> dict[str, Any]:
    if not isinstance(data, dict) or set(data) != QUEUE_FIELDS:
        raise ValueError("Round 0016 queue manifest fields changed")
    if (data["schema_version"] != 1 or data["program"] != PROGRAM or
            data["round_id"] != ROUND_ID or data["round_sha256"] != ROUND_SHA256):
        raise ValueError("queue does not identify issued basemap-100m Round 0016")
    if (data["execution_authority"] != "owner-gpu" or
        ...
        raise ValueError("Round 0016 authority/review/cap policy changed")
    ...
    for name in ("gate_receipts_dir", "controller_checkpoints_dir"):
        ...
        if not os.path.isdir(current) or os.path.islink(current) or os.listdir(current):
            raise ValueError(f"Round 0016 {name} must be a fresh empty directory")
    ...
    for name in ("controller_terminal_summary", "lease_release_receipt"):
        if os.path.lexists(data[name]):
            raise FileExistsError(f"Round 0016 {name} must be absent before execution")
    if data["lease_path"] != GPU_LEASE_PATH:
        raise ValueError("Round 0016 GPU lease path changed")
    if data["allowed_processes"] != []:
        raise ValueError("Round 0016 requires allowed_processes=[]")
    ...
    if (not isinstance(child, dict) or set(child) != CHILD_KEYS or
        ...
        raise ValueError("Round 0016 child/cache environment changed")
    ...
    if (environment["freeze_sha256"] != data["environment_freeze_sha"] or
        ...
        raise ValueError("Round 0016 environment differs from child binding")
    ...
    if not isinstance(staging, dict) or set(staging) != fields:
        raise ValueError("Round 0016 input-staging fields changed")
    body = {key: staging[key] for key in staging if key != "identity_sha256"}
    if (staging["schema"] != "round0016-input-staging-v1" or
        ...
        raise ValueError("Round 0016 reference-only staging policy changed")
    ...
    if (not isinstance(fixture, dict) or set(fixture) != {
            ...
            fixture["schema"] != "round0016-canary-derivation-v1" or
            ...
            fixture["sha256"]):
        raise ValueError("Round 0016 canary identity changed")
    validate_roundwatch_binding(data["roundwatch_binding"])

    registry = data["global_input_registry"]
    if not isinstance(registry, list) or not registry:
        raise ValueError("Round 0016 global input registry is empty")
    registry_paths = []
    ...
    if registry_paths != sorted(registry_paths) or len(registry_paths) != len(
            set(registry_paths)):
        raise ValueError("Round 0016 global input registry is not sorted/unique")
    ...
    if staging["registered_file_count"] != context[
        ...
        raise ValueError("Round 0016 staged reference count changed")
    ...
    if (exclusive["policy"] != POLICY or exclusive["allowed_processes"] != [] or
        ...
        raise ValueError("Round 0016 exclusive GPU capacity decision changed")
    release_roles = [item["signature"] for item in data["program_inputs"]
                     if item["role"] == "release_preflight_receipt"]
    if len(release_roles) != 1:
        raise ValueError("Round 0016 release-preflight role is not unique")
    ...
    if release["release_sha"] != data["release_sha"]:
        raise ValueError("Round 0016 release preflight differs from queue")
    ...
    for job in data["jobs"]:
        if set(job) != JOB_FIELDS or job["node_policy"]["gpu_memory_cap_mb"] != \
                JOB_CAP_MIB:
            raise ValueError("Round 0016 job fields/memory cap changed")
    ...
    for position, (left_label, left) in enumerate(normalized):
        for right_label, right in normalized[position + 1:]:
            if _overlap(left, right):
                raise ValueError(
                    f"Round 0016 control/output alias: {left_label}/{right_label}")
        if any(_overlap(left, item) for item in registry_paths):
            raise ValueError("Round 0016 output/control overlaps a signed input")
        if any(_overlap(left, item) for item in roots):
            raise ValueError("Round 0016 output/control overlaps a cache")
    return context
