"""Reference-only staging for the immutable Round 0017 production inputs."""
...
from .round0017_program import (
    ...
    BASE_COMMIT, BASE_TREE, ROUND_ID, SEQUENCED_REVIEW_FILE,
    SEQUENCED_REVIEW_SHA256, TRAIN_CONFIG, TRAIN_CONFIG_SHA256,
    accepted_reference_records, round0017_release_chain,
)


REFERENCE_SCHEMA = "round0017-input-reference-manifest-v1"
ROUND_ROOT = "/data/latent-basemap/runs/round-0017"
...
def verify_release_chain(release_root: str, release_sha: str) -> dict[str, Any]:
    ...
    if root != release_root or _git(root, "rev-parse", "HEAD") != release_sha:
        raise RuntimeError("Round 0017 staging release checkout/commit changed")
    expected = round0017_release_chain(release_sha)
    if (_git(root, "rev-parse", "HEAD^") != BASE_COMMIT or
            _git(root, "rev-parse", f"{BASE_COMMIT}^{{tree}}") != BASE_TREE or
            _git(root, "rev-list", "--ancestry-path", "--reverse",
                 f"{BASE_COMMIT}..{release_sha}").splitlines() != [release_sha]):
        raise RuntimeError("Round 0017 release is not the exact one-commit chain")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise RuntimeError("Round 0017 staging release checkout is dirty")
    if subprocess.run(
            ["/usr/bin/git", "-C", root, "symbolic-ref", "-q", "HEAD"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL).returncode == 0:
        raise RuntimeError("Round 0017 staging requires the detached run checkout")
    body = {
        "schema": "round0017-one-commit-release-evidence-v1",
        "release_root": root,
        **expected,
...
def validate_release_evidence(value: dict[str, Any]) -> dict[str, Any]:
    required = {
        "schema", "release_root", "scientific_base", "scientific_base_tree",
        "execution_release", "implementation_commits", "ancestry",
        "commits_after_base", "tree", "parent", "detached", "clean",
        "identity_sha256",
    }
    if not isinstance(value, dict) or set(value) != required:
        raise ValueError("Round 0017 release evidence fields changed")
    body = {key: value[key] for key in value if key != "identity_sha256"}
    expected = round0017_release_chain(value.get("execution_release"))
    if (value["identity_sha256"] != sha256_bytes(canonical_json(body)) or
            value["schema"] != "round0017-one-commit-release-evidence-v1" or
            any(value.get(key) != expected_value
                for key, expected_value in expected.items()) or
            value["parent"] != BASE_COMMIT or value["detached"] is not True or
            value["clean"] is not True or
            not isinstance(value["release_root"], str) or
            not os.path.isabs(value["release_root"])):
        raise ValueError("Round 0017 exact release evidence changed")
    return value
...
def _expected_references(*, full_hash: bool) -> list[dict[str, Any]]:
    ...
    if accepted["sha256"] != ACCEPTED_MANIFEST_FILE_SHA256:
        raise RuntimeError("accepted manifest changed while staging Round 0017")
...
def validate_input_reference_manifest(value: dict[str, Any], *,
    ...
    if not isinstance(value, dict):
        raise ValueError("Round 0017 input-reference manifest must be an object")
    ...
    if (identity != sha256_bytes(canonical_json(body)) or set(value) != required or
        ...
        raise ValueError("Round 0017 input-reference fields/seal changed")
    if value["accepted_tuple"] != {
        ...
        raise ValueError("Round 0017 accepted input-pack tuple changed")
    if value["sequenced_review"] != {
        "path": SEQUENCED_REVIEW_FILE, "sha256": SEQUENCED_REVIEW_SHA256,
        "status": "accepted",
        "releases": ["capability:30m-input-pack-v1"], "blocks": [],
    }:
        raise ValueError("Round 0017 sequenced review binding changed")
    validate_release_evidence(value["release_evidence"])
    expected = _expected_references(full_hash=full_hash)
    if value["reference_count"] != len(expected) or value["references"] != expected:
        raise ValueError("Round 0017 reference-only closure changed")
    return value
...
def stage_input_references(*, round_root: str, release_root: str,
    ...
    if root != ROUND_ROOT or not os.path.isdir(root) or os.path.islink(root):
        raise ValueError(f"Round 0017 staging root must be existing {ROUND_ROOT}")
    release = verify_release_chain(release_root, release_sha)
    inputs = os.path.join(root, "inputs")
    create_fresh_directory(inputs, label="Round 0017 reference staging root")
    ...
    for path in (manifest_path, config_path, canary_path, transform_path):
        refuse_existing(path, label="Round 0017 staged reference record")
    ...
    canary = {
        "schema": "round0017-canary-derivation-v1", "round_id": ROUND_ID,
    ...
    return {
        "schema": "round0017-reference-staging-v1", "release": release,
