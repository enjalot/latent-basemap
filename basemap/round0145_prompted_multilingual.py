"""Prompted Jina embeddings for R0087's final multilingual tranche.

R0145 is deliberately a data-production capability.  It preserves the exact
R0087 row order for Turkish and Vietnamese while replacing the historical
...
ROWS_PER_LANGUAGE = 835_453
LANGUAGE_TRANCHES = (
    {
        "node_id": "embed_tur_Latn",
        "language": "tur_Latn",
        "dataset": "fineweb2-tur_Latn-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [0, ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [23_329_094, 24_164_547],
    },
    {
        "node_id": "embed_vie_Latn",
        "language": "vie_Latn",
        "dataset": "fineweb2-vie_Latn-chunked-500-jina-v5-nano",
        "corpus_global_row_range": [ROWS_PER_LANGUAGE, 2 * ROWS_PER_LANGUAGE],
        "r0087_global_row_range": [24_164_547, 25_000_000],
    },
)
CORPUS_ROWS = 2 * ROWS_PER_LANGUAGE
R0087_GLOBAL_START = 23_329_094
R0087_GLOBAL_STOP = 25_000_000
...
NODE_SCHEMA = "round0145-canonical-document-multilingual-node-v1"
CORPUS_SCHEMA = "jina-document-multilingual-tur-vie-1p671m-v1"
CAPABILITY = CORPUS_SCHEMA


class Round0145Error(RuntimeError):
    """The exact R0145 data-production contract was violated."""
...
def validate_seal(value: Mapping[str, Any], *, label: str) -> None:
    ...
    if value.get("identity_sha256") != sha256_bytes(canonical_json(body)):
        raise Round0145Error(f"{label} identity seal is invalid")
...
def load_inventory_manifest(
    ...
    *,
    ...
    if signature["sha256"] != expected_sha256:
        raise Round0145Error("R0087 inventory manifest bytes changed")
    ...
    if (
        ...
        raise Round0145Error("R0087 multilingual inventory capability is not exact")
    ...
    *,
    ...
    for tranche in LANGUAGE_TRANCHES:
        ...
        if (
            ...
            raise Round0145Error(f"R0087 range is malformed for {dataset}")
...
def tranche_for_node(node_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise Round0145Error(f"unknown R0145 work node {node_id!r}")
...
def source_for_node(
    ...
    if len(matches) != 1:
        raise Round0145Error(f"source layout does not close for {node_id}")
    return matches[0]


def validate_source_layout(layout: Sequence[Mapping[str, Any]]) -> None:
    if len(layout) != len(LANGUAGE_TRANCHES):
        raise Round0145Error("multilingual source layout has wrong tranche count")
    corpus_cursor = 0
    ...
    for expected, observed in zip(LANGUAGE_TRANCHES, layout, strict=True):
        ...
        if (
            ...
            raise Round0145Error("multilingual source layout identity changed")
    ...
    if corpus_cursor != CORPUS_ROWS or accepted_cursor != R0087_GLOBAL_STOP:
        raise Round0145Error("multilingual source layout did not close")
...
def validate_node_receipt(
    ...
    receipt = dict(value)
    validate_seal(receipt, label=f"R0145 node {node_id}")
    ...
    if (
        ...
        raise Round0145Error(f"R0145 node {node_id} contract changed")
    ...
    if (
        ...
        raise Round0145Error(f"R0145 node {node_id} byte closure changed")
    ...
    for chunk in chunks:
        ...
        if (
            ...
            raise Round0145Error(f"R0145 node {node_id} chunk coverage changed")
        cursor = stop
    ...
    if (
        ...
        raise Round0145Error(f"R0145 node {node_id} telemetry is invalid")
    return receipt
...
def validate_coverage(chunks: Sequence[Mapping[str, Any]]) -> None:
    ...
    for chunk in sorted(
        ...
        if local_start != prior:
            raise Round0145Error("multilingual per-language row order changed")
    ...
    if (
        ...
        raise Round0145Error("canonical multilingual corpus does not close")
