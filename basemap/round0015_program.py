"""Exact CPU-visible contract for the one Round 0015 production queue.

The scientific payload is the reviewed Round-0014 six-node program.  Only its
target wiring, exact known-service decision, per-job reservation, and terminal
lease-release destination are new.
"""
...
from .round0015_service import (JOB_CAP_MIB, POLICY, load_service_decision)
...
SEQUENCED_REVIEW_SHA256 = (
    "45403a2bce7926ecdc89b1ba04ffa5a0c2d43227548c75bf0d8392e2a8b611c5")
GPU_HOURS_CAP = 5.5
...
# Preserve every scientific field from the reviewed Round-0014 program.  The
# schema is target provenance, not a treatment change.
TRAIN_CONFIG: dict[str, Any] = copy.deepcopy(_ROUND0014_TRAIN_CONFIG)
TRAIN_CONFIG["schema"] = "round0015-production-config-v1"
TRAIN_CONFIG_SHA256 = sha256_bytes(canonical_json(TRAIN_CONFIG))
Round0015MaterializedArray = Round0014MaterializedArray
...
def derived_node_policy(spec: NodeSpec) -> dict[str, Any]:
    body = {
        "schema": "round0015-derived-node-policy-v1",
        "node_id": spec.node_id,
        "canonical_script": "experiments/run_round0015_node.py",
...
def _role_map(manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(entries, list):
        raise ValueError("Round 0015 program inputs must be an ordered list")
    roles: dict[str, dict[str, Any]] = {}
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {"role", "signature"}:
            raise ValueError("Round 0015 program input fields changed")
        if entry["role"] in roles:
            raise ValueError("Round 0015 program input role is duplicated")
...
def expected_argv(spec: NodeSpec, *, manifest: dict[str, Any],
    ...
    return [
        ...
        os.path.join(manifest["repo_root"], "experiments/run_round0015_node.py"),
...
def derive_program_context(manifest: dict[str, Any], *,
    ...
    for role in PROGRAM_INPUT_ROLES:
        ...
        if (not isinstance(path, str) or not os.path.isabs(path) or
            ...
            raise ValueError(f"Round 0015 program input changed: {role}")
        paths[role] = path
    fixed = {
    ...
    for role, (path, digest) in fixed.items():
        if paths[role] != path or roles[role]["sha256"] != digest:
            raise ValueError(f"Round 0015 fixed {role} binding changed")
    release = _validate_bound_release(manifest)
    ...
    if config_record != {
        ...
        raise ValueError("Round 0015 production config changed")
    with open(paths["input_reference_manifest"], encoding="utf-8") as handle:
        reference_manifest = json.load(handle)
    from .round0015_staging import validate_input_reference_manifest
    ...
    service = load_service_decision(
        paths["service_decision"],
        environment_manifest=paths["environment_manifest"],
        require_current=False)
    if service["allowed_processes"] != manifest.get("allowed_processes"):
        raise ValueError("Round 0015 service decision differs from queue allowlist")
    return {
...
def validate_exact_program(manifest: dict[str, Any], *, manifest_path: str,
    ...
    for position, (job, spec) in enumerate(zip(jobs, NODES)):
        ...
        if (set(job) != JOB_FIELDS or job["deps"] != dependency or
            ...
            raise ValueError(f"Round 0015 exact node wiring changed: {spec.node_id}")
    aggregate = sum(item.p90_wall_s * 1.15 for item in NODES)
    if aggregate > GPU_HOURS_CAP * 3600:
        raise AssertionError("Round 0015 registered p90+15% exceeds 5.5 hours")
    if manifest.get("lease_release_receipt") != os.path.join(
            queue_root, "lease-release.json"):
        raise ValueError("Round 0015 lease-release destination changed")
    if manifest.get("program_policy") != program_policy():
        raise ValueError("Round 0015 program policy changed")
    return context


def program_policy() -> dict[str, Any]:
    body = {
        "schema": "round0015-program-policy-v1",
