"""Exact CPU-visible contract for the one Round 0017 production queue."""
...
from .source_closure import validate_round0017_source_closure_receipt


ROUND_ID = "0017"
PROGRAM = "basemap-100m"
BASE_COMMIT = "af7283be749e62e6e18a86f88b11a4d80ea8ebb7"
BASE_TREE = "76e29c3055efdfe2c2c384b4c3d13c612d205278"
ROUND_FILE = "/home/enjalot/code/latent-labs/basemap-100m/round-0017-2026-07-18.md"
ROUND_SHA256 = "110e71458374484fc75526bc37ff38bf094796baa01f289d9d388720cd3decf1"
SEQUENCED_REVIEW_FILE = (
    "/home/enjalot/code/latent-labs/basemap-100m/review-0013-2026-07-18-02.md")
SEQUENCED_REVIEW_SHA256 = (
    "b14d8a4f4df5633bb3ff62f2a98a036f57ca34ffee251abc9e07733abc019ed0")
GPU_HOURS_CAP = 5.5
...
TRAIN_CONFIG["schema"] = "round0017-production-config-v1"
TRAIN_CONFIG_SHA256 = sha256_bytes(canonical_json(TRAIN_CONFIG))
Round0017MaterializedArray = Round0014MaterializedArray
...
def round0017_release_chain(release_sha: str) -> dict[str, Any]:
    if (not isinstance(release_sha, str) or
            not FULL_SHA.fullmatch(release_sha) or release_sha == BASE_COMMIT):
        raise ValueError("Round 0017 release SHA is invalid")
    return {
...
def derived_node_policy(spec: NodeSpec) -> dict[str, Any]:
    body = {
        "schema": "round0017-derived-node-policy-v1",
        "node_id": spec.node_id,
        "canonical_script": "experiments/run_round0017_node.py",
...
def _role_map(manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(entries, list):
        raise ValueError("Round 0017 program inputs must be an ordered list")
    roles: dict[str, dict[str, Any]] = {}
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {"role", "signature"}:
            raise ValueError("Round 0017 program input fields changed")
        if entry["role"] in roles:
            raise ValueError("Round 0017 program input role is duplicated")
        roles[entry["role"]] = entry["signature"]
    if tuple(sorted(roles)) != PROGRAM_INPUT_ROLES:
        raise ValueError("Round 0017 program input roles changed")
    return roles
...
def _validate_bound_release(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = round0017_release_chain(release["release_sha"])
    if release["implementation_commits"] != expected["implementation_commits"]:
        raise ValueError("Round 0017 release receipt changed")
    return release
...
def validate_global_input_registry(
    ...
    if not paths or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise ValueError("Round 0017 global input registry must be nonempty, sorted, and unique")
    if not required_paths.issubset(set(paths)):
        raise ValueError("Round 0017 global registry omits a required source/input")
    return paths
...
def expected_argv(spec: NodeSpec, *, manifest: dict[str, Any],
    ...
    return [
        ...
        os.path.join(manifest["repo_root"], "experiments/run_round0017_node.py"),
...
def derive_program_context(manifest: dict[str, Any], *,
    ...
    for role in PROGRAM_INPUT_ROLES:
        ...
        if (not isinstance(path, str) or not os.path.isabs(path) or
            ...
            raise ValueError(f"Round 0017 program input changed: {role}")
        paths[role] = path
    fixed = {
    ...
    for role, (path, digest) in fixed.items():
        if paths[role] != path or roles[role]["sha256"] != digest:
            raise ValueError(f"Round 0017 fixed {role} binding changed")
    release = _validate_bound_release(manifest)
    if (release["release_sha"] != manifest["release_sha"] or
            release["run_checkout_path"] != os.path.realpath(repo_root)):
        raise ValueError("Round 0017 release receipt differs from queue checkout")
    validate_round0017_source_closure_receipt(
    ...
    if config_record != {
            "schema": "round0017-production-config-receipt-v1",
            "config": TRAIN_CONFIG, "config_sha256": TRAIN_CONFIG_SHA256}:
        raise ValueError("Round 0017 production config changed")
    with open(paths["input_reference_manifest"], encoding="utf-8") as handle:
        reference_manifest = json.load(handle)
    from .round0017_staging import validate_input_reference_manifest
    ...
    if exclusive["allowed_processes"] != manifest.get("allowed_processes") != []:
        raise ValueError("Round 0017 exclusive decision differs from queue")
    return {
...
def validate_exact_program(manifest: dict[str, Any], *, manifest_path: str,
    ...
    if not isinstance(jobs, list) or [item.get("id") for item in jobs] != [
        ...
        raise ValueError("Round 0017 queue must contain its exact six nodes in order")
    required = {
    ...
    expected_chain = round0017_release_chain(manifest["release_sha"])
    if any(release_evidence.get(key) != value
           for key, value in expected_chain.items()):
        raise ValueError("Round 0017 exact one-commit ancestry changed")
    ...
    for position, (job, spec) in enumerate(zip(jobs, NODES)):
        ...
        if (set(job) != JOB_FIELDS or job["deps"] != dependency or
            ...
            raise ValueError(f"Round 0017 exact node wiring changed: {spec.node_id}")
    if sum(item.p90_wall_s * 1.15 for item in NODES) > GPU_HOURS_CAP * 3600:
        raise AssertionError("Round 0017 registered p90+15% exceeds 5.5 hours")
    if manifest.get("lease_release_receipt") != os.path.join(
            queue_root, "lease-release.json"):
        raise ValueError("Round 0017 lease-release destination changed")
    ...
    if manifest.get("program_policy") != expected_policy:
        raise ValueError("Round 0017 program policy changed")
    return context
...
def program_policy(*, registered_global_inputs: int,
    ...
    body = {
        "schema": "round0017-program-policy-v1",
