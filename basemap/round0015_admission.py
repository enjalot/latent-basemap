"""CUDA-uninitialized validation for the exact Round 0015 queue manifest."""
...
from .round0015_service import (JOB_CAP_MIB, MARKER, POLICY, SERVICE_CAP_MIB)
...
def validate_round0015_queue_manifest(data: dict[str, Any],
                                      path: str) -> dict[str, Any]:
    """Reopen the complete CPU-visible queue state without importing CUDA."""
    if not isinstance(data, dict) or set(data) != QUEUE_FIELDS:
        raise ValueError("Round 0015 queue manifest fields changed")
    if (data["schema_version"] != 1 or data["program"] != PROGRAM or
            data["round_id"] != ROUND_ID or data["round_sha256"] != ROUND_SHA256):
        raise ValueError("queue does not identify issued basemap-100m Round 0015")
    if (data["execution_authority"] != "owner-gpu" or
        ...
        raise ValueError("Round 0015 authority/review/cap policy changed")
    ...
    for name in ("gate_receipts_dir", "controller_checkpoints_dir"):
        ...
        if not os.path.isdir(current) or os.path.islink(current) or os.listdir(current):
            raise ValueError(f"Round 0015 {name} must be a fresh empty directory")
    ...
    for name in ("controller_terminal_summary", "lease_release_receipt"):
        if os.path.lexists(data[name]):
            raise FileExistsError(f"Round 0015 {name} must be absent before execution")
    if data["lease_path"] != GPU_LEASE_PATH:
        raise ValueError("Round 0015 GPU lease path changed")
    from .queue_admission import _validate_allowed_process
    allowed = data["allowed_processes"]
    if not isinstance(allowed, list) or len(allowed) != 1:
        raise ValueError("Round 0015 requires exactly one allowed service record")
    _validate_allowed_process(allowed[0], position=0)
    if (allowed[0]["marker"] != MARKER or
            allowed[0]["service_identity"] != MARKER or
            allowed[0]["gpu_memory_budget_mb"] != SERVICE_CAP_MIB):
        raise ValueError("Round 0015 allowed service marker/cap changed")
    ...
    if (not isinstance(child, dict) or set(child) != CHILD_KEYS or
        ...
        raise ValueError("Round 0015 child/cache environment changed")
    ...
    if (environment["freeze_sha256"] != data["environment_freeze_sha"] or
        ...
        raise ValueError("Round 0015 environment differs from child binding")
    ...
    if not isinstance(staging, dict) or set(staging) != fields:
        raise ValueError("Round 0015 input-staging fields changed")
    body = {key: staging[key] for key in staging if key != "identity_sha256"}
    if (staging["schema"] != "round0015-input-staging-v1" or
        ...
        raise ValueError("Round 0015 reference-only staging policy changed")
    ...
    if (not isinstance(fixture, dict) or set(fixture) != {
            ...
            fixture["schema"] != "round0015-canary-derivation-v1" or
            ...
            fixture["sha256"]):
        raise ValueError("Round 0015 canary identity changed")
    validate_roundwatch_binding(data["roundwatch_binding"])

    registry = data["global_input_registry"]
    if not isinstance(registry, list) or not registry:
        raise ValueError("Round 0015 global input registry is empty")
    registry_paths = []
    ...
    service_memory = context["service_decision"]["memory_reservation"]
    if (service_memory["job_cap_mib"] != JOB_CAP_MIB or
            service_memory["service_cap_mib"] != SERVICE_CAP_MIB or
            service_memory["combined_cap_mib"] >
            service_memory["device_total_mib"]):
        raise ValueError("Round 0015 service/job capacity decision changed")
    release_roles = [item["signature"] for item in data["program_inputs"]
                     if item["role"] == "release_preflight_receipt"]
    if len(release_roles) != 1:
        raise ValueError("Round 0015 release-preflight role is not unique")
    ...
    if release["release_sha"] != data["release_sha"]:
        raise ValueError("Round 0015 release preflight differs from queue")
    ...
    for job in data["jobs"]:
        if set(job) != JOB_FIELDS or job["node_policy"]["gpu_memory_cap_mb"] != \
                JOB_CAP_MIB:
            raise ValueError("Round 0015 job fields/memory cap changed")
    ...
    for position, (left_label, left) in enumerate(normalized):
        for right_label, right in normalized[position + 1:]:
            if _overlap(left, right):
                raise ValueError(
                    f"Round 0015 control/output alias: {left_label}/{right_label}")
        if any(_overlap(left, item) for item in registry_paths):
            raise ValueError("Round 0015 output/control overlaps a signed input")
        if any(_overlap(left, item) for item in roots):
            raise ValueError("Round 0015 output/control overlaps a cache")
    return context
