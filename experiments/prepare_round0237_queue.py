#!/usr/bin/env python3
"""Prepare, but never launch, the R0237 queue — the nested 50M rung.
...
drops the `assemble_50000k` node and binds that manifest at its full signature,
...
from experiments.round0237_nodes import (
    ASSEMBLE_ACTION,
    LADDER_ACTION,
    QUALIFY_ACTION,
    REACHABILITY_ACTION,
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0237"
QUEUE_ROOT = os.path.join(ROUND_ROOT, "queue")
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(LAB_ROOT, "round-0237-2026-08-09.md")
EMB = "/data/embeddings"
...
R0236_SUBSTRATE_MANIFEST = os.path.join(
    R0236_ROOT, "minilm-mixed-25000k-nested-substrate-and-reserves-v1",
    "substrate.json",
)
R0236_TRUTH_MANIFEST = os.path.join(
    R0236_ROOT, "minilm-mixed-25000k-uniform-probe-k15-truth-v1",
    "probe-k15-truth.json",
)
...
ASSEMBLE_P90_WALL_S = 4_200.0
TRUTH_P90_WALL_S = 3_600.0
LADDER_P90_WALL_S = 14_000.0
QUALIFY_P90_WALL_S = 3_600.0
...
def _issued_round(release_sha: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if (
        ...
        raise RuntimeError("R0237 round is not issued for this release")
    reviews = _frontmatter_list(frontmatter, "required_reviews")
    if not reviews:
        raise RuntimeError("R0237 round must declare its required reviews")
...
def _release_cpu_smoke(release_sha: str) -> dict[str, Any]:
    ...
    if observed != release_sha:
        raise RuntimeError(f"R0237 release checkout is at {observed}, not {release_sha}")
    environment = dict(os.environ)
    environment["CUDA_VISIBLE_DEVICES"] = ""
    completed = subprocess.run(
        [os.path.join(RELEASE_ROOT, ".venv/bin/python"), "-m", "pytest", "-q",
         "tests/test_round0237_contract.py", "tests/test_round0237_cpu_smoke.py"],
        cwd=RELEASE_ROOT, env=environment, capture_output=True, text=True,
        timeout=900,
    )
    if completed.returncode != 0:
        raise RuntimeError(
            f"R0237 release CPU smoke failed:\n{completed.stdout[-4000:]}"
        )
    return {
        "release_sha": release_sha,
        "cuda_visible_devices": "",
        "command": (
            "pytest -q tests/test_round0237_contract.py "
            "tests/test_round0237_cpu_smoke.py"
        ),
...
def _source_size_manifest(queue_root: str) -> str:
    ...
    for corpus, _rows in COMPOSITION:
        ...
        if not sizes:
            raise RuntimeError(f"R0237 found no shards for {corpus}")
        corpora[corpus] = {"shard_sizes": sizes}
    path = os.path.join(queue_root, "source-size-manifest.json")
    atomic_write_new_json(
        path, {"schema": "round0237-source-size-manifest-v1", "corpora": corpora},
        immutable=True,
    )
    return path
...
def prepare_round0237(
    *,
    ...
    inherited = {
        "parent_substrate": R0236_SUBSTRATE_MANIFEST,
        "parent_truth": R0236_TRUTH_MANIFEST,
    ...
    jobs: list[dict[str, Any]] = [
        {
            ...
            "done_marker": os.path.join(
                artifacts, "reachability_high_c_25000k.done.json"
            ),
            ...
            "parent_truth_manifest": signatures["parent_truth"],
    ...
    if not reused_substrate:
        jobs.append({
            ...
            "done_marker": os.path.join(artifacts, "assemble_50000k.done.json"),
            "expected_inputs": expected_inputs,
    ...
    jobs.extend([
        {
            ...
            "outputs": [truth_dir],
            "done_marker": os.path.join(artifacts, "truth_probe_50000k.done.json"),
            "expected_inputs": expected_inputs,
    ...
    queue.update({
        "schema": "round0237-50000k-nested-rung-queue-v1",
        "repo_root": RELEASE_ROOT,
        "queue_class": "gpu-graph",
        "required_reviews": list(required_reviews),
        "capability_dependencies": [
            "minilm-mixed-25000k-nested-substrate-and-reserves-v1",
            "minilm-mixed-25000k-uniform-probe-k15-truth-v1",
        ...
        "scientific_contract": {
            "question": (
                "can rung 4 be assembled so that it CONTAINS rung 3's "
                "25,000,000 training rows while still spanning every corpus, "
                "does cluster-spill-nnd at s = 8 with c chosen under the "
                "round's own imbalance margin reproduce brute-force truth on a "
                "registered uniform probe with no edgeless row in either "
                "direction, what is the reachability ceiling above c = 64 "
                "(never measured anywhere in this program), which c should the "
                "100M rung be registered at, and which page-cache regime does "
                "the first rung near the boundary actually land in?"
            ),
            "rows": ROWS,
            "parent_rows": PARENT_ROWS,
            "reserve_rows": RESERVE_ROWS,
            "reserve_source": (
                "inherited verbatim through R0236 and R0235 from R0233"
            ),
            "dimension": DIMENSION,
            "spill": SPILL,
