#!/usr/bin/env python3
"""Prepare, but never launch, matched balanced-30M GPU training."""
...
ROUND_ID = "0061"
ROUND_ROOT = "/data/latent-basemap/runs/round-0061"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(
    LAB_ROOT,
    "round-0061-2026-07-26.md",
)
SUBSTRATE_MANIFEST = (
...
def _require_issued_round() -> None:
    ...
    if statuses != ["issued"]:
        raise RuntimeError(
            f"R0061 requires one issued status; observed {statuses}"
        )


def prepare_round0061(
    *,
    release_sha: str,
    ...
    queue_root = create_fresh_directory(
        queue_root,
        label="Round 0061 queue",
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    output = os.path.join(artifacts, "train-balanced-30m-int8")
    inputs = _dedupe([
        *_file_inputs([
            ROUND_FILE,
            os.path.join(LAB_ROOT, "review-0046-2026-07-25.md"),
            os.path.join(LAB_ROOT, "review-0053-2026-07-26.md"),
            os.path.join(LAB_ROOT, "review-0060-2026-07-26.md"),
            SUBSTRATE_MANIFEST,
    ...
    manifest["schema"] = "round0061-balanced-30m-int8-train-queue-v1"
    manifest["repo_root"] = RELEASE_ROOT
    manifest["queue_class"] = "gpu-research"
    manifest["required_reviews"] = ["0046", "0053", "0060"]
    ...
    manifest["jobs"] = [{
        ...
        "production_config_receipt_schema": (
            "round0061-production-config-receipt-v1"
        ),
        "train_receipt_schema": "round0061-train-receipt-v1",
        "node_policy": {
            "gpu_required": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0061(
