#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0144 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0144"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0144-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-006-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    ...
    "spa_Latn": {"mean": 444.302, "p95": 590.0, "maximum": 630},
    "swe_Latn": {"mean": 435.997, "p95": 598.0, "maximum": 693},
    "tha_Thai": {"mean": 321.833, "p95": 395.05, "maximum": 521},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0144 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0144 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0144_inputs() -> dict[str, Any]:
...
def prepare_round0144(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0144 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0144 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0144_nodes",
    ...
    queue["schema"] = "round0144-canonical-prompted-multilingual-queue-v1"
