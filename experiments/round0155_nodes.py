"""Build the CPU-only R0155 historical-prefix/drop-only census."""
...
def _read_json(path: str, *, label: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(value, dict):
        raise Round0155Error(f"{label} is not a JSON object")
...
def _require_signature(expected: Mapping[str, Any], *, label: str) -> str:
    ...
    if expected_input_signature(path) != dict(expected):
        raise Round0155Error(f"{label} bytes changed")
    return path


def run_census(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"", "-1"}:
        raise Round0155Error("R0155 must run with CUDA hidden")
    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0155 historical-prefix census"
    )
    started = time.monotonic()
    activation, activation_signature = _read_json(
        _require_signature(job["r0153_evidence"], label="R0153 evidence"),
        label="accepted R0153 evidence",
    )
    decision = activation.get("decision")
    if (
        activation.get("round_id") != "0153"
        or activation.get("capability") != R0153_CAPABILITY
        ...
        raise Round0155Error("R0153 did not activate the F1 census branch")
    ...
    if (
        ...
        raise Round0155Error("R0087 inventory binding changed")
    eligibility_path = _require_signature(
    ...
    if (
        ...
        raise Round0155Error("R0132 U12 binding changed")
    ...
    if (
        ...
        raise Round0155Error("registered historical-prefix census changed")
    ...
    receipt = seal({
        "schema": "round0155-diverse-historical-prefix-census-v1",
        "round_id": ROUND_ID,
        "release_sha": active["manifest"]["release_sha"],
        "capability": CAPABILITY,
        "activation": activation_signature,
        "activation_decision_identity": decision["identity_sha256"],
        ...
        "group_ids": expected_input_signature(group_ids_path),
    ...
    atomic_write_new_json(
        os.path.join(output, "census.json"), receipt, immutable=True
    )


def run_job(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    if active.get("manifest", {}).get("round_id") != ROUND_ID:
        raise Round0155Error("R0155 handler requires its exact queue manifest")
    if job.get("action") != "build_historical_prefix_census":
        raise Round0155Error(f"unknown R0155 action: {job.get('action')}")
