    ...
    atomic_write_new_json(
        os.path.join(output, "production-config.json"),
        {
            "schema": str(job.get(
                ...
                "round0052-production-config-receipt-v1",
            )),
            "config": config,
...
def run_job(
    ...
    if round_id not in {ROUND_ID, "0063"}:
        raise RuntimeError("balanced-60M trainer received another queue")
