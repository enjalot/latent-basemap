#!/usr/bin/env python3
"""Prepare, but never launch, the R0154 raw seed-44/45 replay queue."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0154"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(LAB_ROOT, "round-0154-2026-08-02.md")
R0140_GRAPH_ROOT = (
...
GPU_HOURS_MINIMUM = 2.45
GPU_HOURS_EXPECTED = 2.65
GPU_HOURS_P90 = 3.00
GPU_HOURS_MAXIMUM = 4.25
...
def _issued_round(release_sha: str) -> dict[str, Any]:
    if _frontmatter(ROUND_FILE).get("status") != "issued":
        raise RuntimeError("R0154 round is not issued")
    if _frontmatter(ROUND_FILE).get("base_commit") != release_sha:
        raise RuntimeError("R0154 issued base_commit differs from release")
...
def _accepted_reviews() -> list[dict[str, Any]]:
    ...
    for path in REVIEW_FILES:
        if _frontmatter(path).get("status") != "accepted":
            raise RuntimeError(f"R0154 required review is not accepted: {path}")
...
def _pytest_smoke(*, release_sha: str) -> dict[str, Any]:
    ...
    if observed != release_sha:
        raise RuntimeError("R0154 pytest checkout is not at the requested release")
    command = [
        sys.executable,
        "-m",
        "pytest",
    ...
    receipt = seal({
        "schema": "round0154-release-pytest-v1",
        "round_id": ROUND_ID,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R0154 release pytest failed:\n{completed.stdout}\n{completed.stderr}"
        )
    return receipt


def _seed_cpu_smoke(graph: Mapping[str, Any]) -> dict[str, Any]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"", "-1"}:
        raise RuntimeError("R0154 CPU smoke requires CUDA hidden")
    reports: dict[str, Any] = {}
    for seed in SEEDS:
        config, digest = raw_seed_train_config(
            seed=seed,
        ...
        if mismatches or config["paired_invariant"]["seed"] != seed:
            raise RuntimeError(f"R0154 seed {seed} accounting smoke failed: {mismatches}")
        reports[f"seed{seed}"] = {
    ...
    return seal({
        "schema": "round0154-seed-config-accounting-cpu-smoke-v1",
        "round_id": ROUND_ID,
...
def prepare_round0154(
    *, release_sha: str, queue_root: str = os.path.join(ROUND_ROOT, "queue")
) -> str:
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0154 release SHA must be one full commit")
    round_signature = _issued_round(release_sha)
    reviews = _accepted_reviews()
    graph, graph_receipt_signature = _read_sealed(
        os.path.join(R0140_GRAPH_ROOT, "receipt.json"), label="accepted R0140 graph"
    )
    if graph.get("round_id") != "0140" or graph.get("graph_edges") != 151_013_146:
        raise RuntimeError("R0154 accepted R0140 graph changed")
    ...
    queue_root = create_fresh_directory(queue_root, label="R0154 seed queue")
    ...
    atomic_write_new_json(seed_smoke_path, _seed_cpu_smoke(graph), immutable=True)
    ...
    external_inputs = _dedupe([
        round_signature,
        *reviews,
        graph_receipt_signature,
        dict(graph["graph"]),
        dict(graph["graph_manifest"]),
    ...
    train_outputs = {
        seed: os.path.join(artifacts, f"raw_historical_seed{seed}", "train")
        for seed in SEEDS
    }
    panel_outputs = {
        seed: os.path.join(artifacts, f"raw-seed{seed}-functional-panel")
        for seed in SEEDS
    }
    module = "experiments.round0154_nodes"
    jobs: list[dict[str, Any]] = []
    prior: str | None = None
    for seed in SEEDS:
        train_id = f"train_raw_historical_seed{seed}"
        panel_id = f"score_raw_seed{seed}_functional"
        jobs.append({
            "id": train_id,
            "action": "train_raw_seed",
            "seed": seed,
            "cell": RAW,
            ...
            "done_marker": os.path.join(artifacts, f"train-raw-seed{seed}.done.json"),
        ...
        jobs.append({
            ...
            "seed": seed,
            "train_outputs": {RAW: train_outputs[seed]},
            "train_release_shas": {RAW: release_sha},
            **common_panel,
            "handler_module": module,
            "handler_callable": "run_job",
            "deps": [train_id],
            "outputs": [panel_outputs[seed]],
            "done_marker": os.path.join(artifacts, f"raw-seed{seed}-functional.done.json"),
            "expected_inputs": external_inputs,
            "p90_wall_s": 180.0,
    ...
    density_output = os.path.join(artifacts, "raw-seed-density-v2-panel")
    jobs.append({
        "id": "score_raw_seed_density_v2",
        "action": "score_raw_density_v2",
        "functional_panels": {str(seed): panel_outputs[seed] for seed in SEEDS},
        "r0108_calibration": calibration_signature,
        "cpu_workers": 4,
        "handler_module": module,
        "handler_callable": "run_job",
        "deps": [f"score_raw_seed{seed}_functional" for seed in SEEDS],
        "outputs": [density_output],
        "done_marker": os.path.join(artifacts, "raw-seed-density.done.json"),
        "expected_inputs": external_inputs,
        "p90_wall_s": 180.0,
    ...
    jobs.append({
        "id": "seal_raw_seed_evidence",
        "action": "seal_seed_evidence",
        "functional_panels": {str(seed): panel_outputs[seed] for seed in SEEDS},
        "density_output": density_output,
        "handler_module": module,
        "handler_callable": "run_job",
        "deps": ["score_raw_seed_density_v2"],
    ...
    queue.update({
        "schema": "round0154-raw-seed44-45-queue-v1",
        "repo_root": RELEASE_ROOT,
        "queue_class": "gpu-training",
        "required_reviews": ["0108", "0140", "0150"],
        ...
        "scientific_contract": {
            "question": "what is raw historical-row seed variation at seeds 44 and 45?",
            "seeds": list(SEEDS),
            "rows": 2_000_000,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0154(
