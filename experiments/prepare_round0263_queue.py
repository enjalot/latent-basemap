"""Build the R0263 queue — restore the two-sided ``k256`` band, then re-adjudicate.

The exact structural inverse of R0260. Two CPU nodes and one dependency edge, and
the edge is the point. The registration node is bound to the sealed twenty-nine-cell
2M family panel, the sealed R0256 repaired gate, and R0260's *own* sealed
one-sided-floor artifact (so it can prove bitwise that only ``k256`` is superseded and
``k1024``/``ffr`` are untouched) — and to **nothing from R0257 or the 6.25M rung**.
The re-adjudication node depends on it, binds its output by hash at read time, and
refuses to run unless the registration was sealed first. The runner enforces the
ordering and both directions are recorded in receipts rather than in prose.

No training, no GPU, no new cells, no new seeds. This is a CPU round that must not
touch the card (the GPU lane is busy): every job is ``cpu_heavy`` / ``gpu_required:
False`` and ``CUDA_VISIBLE_DEVICES`` is forced empty in the child environment.
...
from experiments.round0263_nodes import (
    ...
    ROUND_ID,
    SUPERSEDES_K256_CRITERION_IN,
    WHY_TWO_SIDED,
)


ROUND_ROOT = "/data/latent-basemap/runs/round-0263"
QUEUE_ROOT = os.path.join(ROUND_ROOT, "queue")
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(LAB_ROOT, "round-0263-2026-08-13.md")
...
#: R0260's OWN sealed 2M one-sided-floor artifact. Bound to node 1 so it can prove
#: bitwise that this round supersedes only k256 and leaves k1024/ffr untouched. Its
#: path carries no rung marker (`round-0257` / `6250k` / `ladder`), so the ordering
#: guard leaves it bound; R0260's *judged-rung* 6.25M artifact is a different path and
#: is NOT bound here.
R0260_ONE_SIDED = (
    "/data/latent-basemap/runs/round-0260/queue/artifacts/"
    f"{R0260_GATE_CAPABILITY}/minilm-one-sided-purity-floors-n29.json"
)
R0257_PANEL = (
...
def _issued_round(release_sha: str) -> tuple[dict, list[str]]:
    ...
    if (
        ...
        raise RuntimeError("R0263 round is not issued for this release")
    reviews = _frontmatter_list(frontmatter, "required_reviews")
    if not reviews:
        raise RuntimeError("R0263 round must declare its required reviews")
...
def _upstream_review_state(required: list[str]) -> dict:
    ...
    return {
        ...
        "note": (
            ...
            "This round executes a direct owner ruling (the 2026-08-13 §5-A "
            "addendum), so the re-registration is registered-and-contingent until "
            "R0263 itself carries an accepted review."
        ),
    }
...
def prepare_round0263(*, release_sha: str, queue_root: str = QUEUE_ROOT) -> str:
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0263 release SHA must be one full commit")
    round_signature, required_reviews = _issued_round(release_sha)
    review_state = _upstream_review_state(list(required_reviews))

    # Every sealed input's sha256 + bytes are computed from the file on disk HERE,
    # at prepare time, by `_sealed` -> `expected_input_signature`. Nothing is a
    # hardcoded constant; a byte that changed on disk changes the manifest.
    ...
    r0260_one_sided = _sealed(R0260_ONE_SIDED, "R0260 sealed one-sided-floor artifact")
    ...
    ensure_data_directory(ROUND_ROOT, label="R0263 round root")
    queue_root = create_fresh_directory(queue_root, label="R0263 queue")
    artifacts = ensure_data_directory(
        os.path.join(queue_root, "artifacts"), label="R0263 artifacts"
    )
    ...
    #: The registration node's declared inputs: the round file, the sealed 2M
    #: family, the sealed R0256 gate, and R0260's sealed one-sided-floor artifact.
    #: NOTHING from R0257 / the 6.25M rung. The node reads this list back out of its
    #: own job and raises if any declared path matches a rung marker; the ordering
    #: positive control then re-runs that refusal on a planted-rung manifest.
    register_inputs = _dedupe(
        [round_signature, panel_n29, r0256_gate, r0260_one_sided]
    )

    readjudicate_node = READJUDICATE_ACTION
    readjudicate_output = os.path.join(artifacts, READJUDICATION_CAPABILITY)
    jobs = [
        {
            "id": register_node,
            "action": REGISTER_ACTION,
            "handler_module": "experiments.round0263_nodes",
            ...
            "r0260_one_sided": r0260_one_sided,
    ...
    # This is a CPU round that must not touch the card (the GPU lane is busy):
    # force the child environment's device mask empty regardless of the default.
    queue["child_environment"]["CUDA_VISIBLE_DEVICES"] = ""
    queue.update({
        "schema": "round0263-two-sided-k256-purity-band-registration-queue-v1",
        ...
        "registered": {
            "what_this_round_is": (
                "the owner's 2026-08-13 §5-A addendum, executed: for "
                "purity_fidelity_k256 ONLY, the two-sided MAD_n band is restored as "
                "the gated criterion, re-fitted bitwise from the sealed n = 29 family "
                "at the sealed k2, superseding R0260's one-sided k256 floor. k1024 and "
                "ffr stay R0260's one-sided floors, carried through bitwise. The band "
                "is registered BEFORE R0257's three 6.25M maps are re-adjudicated "
                "under it. It is the exact structural inverse of R0260."
            ),
            ...
            "how_that_is_enforced": (
                ...
                "the manifest and raises if any declared path matches a rung marker "
                "(`round-0257` / `6250k` / `ladder`). R0260's 2M one-sided-floor "
                "artifact is legitimately bound (its path carries no such marker) to "
                "prove only k256 is superseded. The ordering guard is additionally run "
                "on a planted-rung manifest and must raise — the positive control "
            ...
            "acceptance_rule": (
                ...
                "manifest, and its ordering guard is shown to discriminate on a "
                "planted-rung manifest; the restricted calibration, the restored "
                "two-sided k256 band, the retained one-sided k256 floor and the "
                "untouched k1024/ffr floors all reproduce bitwise from the sealed "
                "family / R0255 / R0260; the multiplier sits below the identity bound "
                "and every defining cell can fail; every planted registry, family, "
                "sidedness and inertness defect lands as registered; and the "
                "re-adjudication node refuses unless the registration artifact was "
                "sealed before it started. NO NUMERICAL OUTCOME MAKES THIS ROUND A "
                "FAILURE: three failing 6.25M maps and one un-failed held-out cell are "
                "the computed result and are published as such."
            ),
            "a_failing_map_is_a_finding": (
                "If a re-adjudicated map fails, it is published failing, with its "
                "margin, direction and z. No retrain, no other seed, no adjusted band. "
                "The owner ruling's 'do not tune anything to preserve the published "
                "pass' applies here with full force, in both directions."
            ),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="prepare the R0263 queue")
    parser.add_argument("--release-sha", required=True)
    parser.add_argument("--queue-root", default=QUEUE_ROOT)
    args = parser.parse_args(argv)
    path = prepare_round0263(release_sha=args.release_sha, queue_root=args.queue_root)
