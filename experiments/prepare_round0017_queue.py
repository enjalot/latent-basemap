"""Build the immutable pre-gate queue for basemap-100m Round 0017."""
...
from basemap.round0017_program import (
    ...
    expected_outputs, program_policy, round0017_release_chain,
)
from basemap.round0016_service import (JOB_CAP_MIB, POLICY,
                                      load_exclusive_decision)
from basemap.round0017_staging import (validate_input_reference_manifest,
                                       verify_release_chain)
from basemap.roundwatch_gate import canonical_roundwatch_binding
from basemap.source_closure import round0017_source_closure_receipt
...
def _load_environment(path: str) -> dict:
    ...
    if not isinstance(value, dict) or any(
            ...
            for key in required):
        raise ValueError("Round 0017 environment manifest is incomplete")
    return value


def build_queue(args) -> dict:
    if args.implementation_commit != args.release_sha:
        raise ValueError("Round 0017 implementation must equal its release")
    round_root = canonical_data_path(args.round_root, label="Round 0017 root")
    if round_root != ROUND_ROOT or not os.path.isdir(round_root):
        raise ValueError(f"Round 0017 root must already exist at {ROUND_ROOT}")
    ...
    out = canonical_data_path(args.out, label="Round 0017 queue manifest")
    queue_root = os.path.dirname(out)
    if out != os.path.join(round_root, "queue", "queue.json"):
        raise ValueError("Round 0017 queue must be round-root/queue/queue.json")
    refuse_existing(queue_root, label="Round 0017 queue root")
    ...
    if not os.path.isfile(python):
        raise FileNotFoundError("Round 0017 sealed interpreter is missing")
    ...
    if decision["policy"] != POLICY or decision["allowed_processes"] != []:
        raise ValueError("Round 0017 queue requires the fresh empty allowlist")
    ...
    if not preflight["passed"]:
        raise RuntimeError("Round 0017 release preflight rejected: " +
    ...
    if staged["release_evidence"] != release_evidence:
        raise RuntimeError("Round 0017 staged release evidence changed")
    sources = round0017_source_closure_receipt(run_root, require_tracked=True)
    roundwatch = canonical_roundwatch_binding()

    create_fresh_directory(queue_root, label="Round 0017 queue root")
    artifacts = create_fresh_directory(
        os.path.join(queue_root, "artifacts"), label="Round 0017 artifact parent")
    receipts = create_fresh_directory(
        os.path.join(queue_root, "gate-receipts"),
        label="Round 0017 gate receipts")
    checkpoints = create_fresh_directory(
        os.path.join(queue_root, "controller-checkpoints"),
        label="Round 0017 controller checkpoints")
    create_fresh_directory(cache_root, label="Round 0017 cache parent")
    for key in CACHE_KEYS:
        create_fresh_directory(cache_environment[key],
                               label=f"Round 0017 cache {key}")
    ...
    for value in (terminal, lease_release, gate_preparation, release_receipt_path):
        refuse_existing(value, label="Round 0017 queue control")
    ...
    if not release_reports_equivalent(issued, preflight):
        raise RuntimeError("Round 0017 release changed during queue scaffolding")
    ...
    def signature(path: str) -> dict:
        ...
        if prior is not None and prior != observed:
            raise RuntimeError(f"Round 0017 input changed during capture: {canonical}")
    ...
    for reference in staged["references"]:
        ...
        if observed != expected:
            raise RuntimeError(f"Round 0017 staged input changed: {reference['role']}")
    role_paths = {
    ...
    if tuple(sorted(role_paths)) != PROGRAM_INPUT_ROLES:
        raise AssertionError("Round 0017 program-input role drift")
    extra_paths = [
    ...
    staging_body = {
        "schema": "round0017-input-staging-v1",
    ...
    manifest = {
        ...
        "execution_authority": "autonomous-gpu", "required_reviews": ["0013"],
    ...
    for spec in NODES:
        ...
        for value in (*outputs, *controls.values()):
            refuse_existing(value, label=f"Round 0017 {spec.node_id} control")
        policy = derived_node_policy(spec)
        if policy["gpu_memory_cap_mb"] != JOB_CAP_MIB:
            raise AssertionError("Round 0017 job cap is not exactly 31,488 MiB")
    ...
    if final_decision["allowed_processes"] != []:
        raise RuntimeError("Round 0017 GPU ceased to be exclusive before publication")
...
def main(argv=None) -> int:
    ...
    print(json.dumps({
        ...
        "implementation_commits": round0017_release_chain(
