...
QUALIFICATION_SCHEMA = "round0130-k49-fixed-policy-qualification-v1"
...
def _signature(path: str, expected_sha256: str, *, label: str) -> dict[str, Any]:
    ...
    if value["sha256"] != expected_sha256:
        raise Round0130Error(f"{label} bytes changed")
    return value
...
def _load_json(path: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Round0130Error(f"{path} is not a JSON object")
    return value
...
def _k49_policy_metrics(
    ...
    *,
    ...
    if (
        ...
        raise Round0130Error("k49 qualification arrays are malformed")
    overlap = (
...
def run_qualify_k49(
    ...
    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0130 fixed-policy k49 qualification"
    )
    ...
    if (
        ...
        raise Round0130Error("R0105 selected policy is not nprobe64/width128")
    ...
    if (
        ...
        raise Round0130Error("registered R0105 stratified sample changed")
    ...
    if (
        ...
        raise Round0130Error("accepted R0105 exact-truth arrays changed")
    ...
    if not np.array_equal(exact49[:, :R0105_K], accepted_exact15):
        raise Round0130Error("exact k49 truth does not extend R0105 exact top15")
...
def _quality_admission(job: Mapping[str, Any]) -> dict[str, Any]:
    ...
    receipt = read_sealed(
        path, label="R0130 k49 qualification", schema=QUALIFICATION_SCHEMA
    )
    if (
        ...
        raise Round0130Error("k49 graph quality admission is not positive")
...
def run_graph_assembly(
    ...
    if result.get("graph_quality_admission") != _quality_admission(job):
        raise Round0130Error("assembled graph lost k49 quality admission")
    return result
...
def _model_class(expected_reconstruction: Mapping[str, Any]) -> type:
    ...
    class Round0130ParametricUMAP(ParametricUMAP):
    ...
    return Round0130ParametricUMAP
...
def _control_config(job: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise Round0130Error("accepted R0107 production config changed")
...
def _accepted_r0107_lineage(
    ...
    *,
    ...
    if (
        ...
        raise Round0130Error("accepted R0107 seed/config/runtime lineage changed")
    model = receipt.get("model")
    if not isinstance(model, Mapping):
        raise Round0130Error("accepted R0107 model signature is missing")
    return {
...
def run_train(
    ...
    graph = read_sealed(graph_path, label="R0130 k49 graph", schema=GRAPH_SCHEMA)
    if (
        graph.get("round_id") != ROUND_ID
        or int(graph.get("k_real", -1)) != GRAPH_K
        or graph.get("graph_quality_admission") != _quality_admission(job)
    ):
        raise Round0130Error("R0130 k49 graph contract changed")
    ...
    receipt = run_train_contract(
        ...
        production_config_schema=PRODUCTION_CONFIG_SCHEMA,
        train_receipt_schema=TRAIN_RECEIPT_SCHEMA,
        output_label="R0130 diverse-Jina k49 treatment",
        graph_load_kwargs={
    ...
    if (
        ...
        raise Round0130Error("R0130 fixed treatment config/dose drifted")
    ...
    isolation_receipt = seal({
        "schema": "round0130-k49-treatment-isolation-v1",
        "round_id": ROUND_ID,
        "accepted_r0107_production_config": control_signature,
        "r0130_production_config": expected_input_signature(
            os.path.join(output, "production-config.json")
        ),
        "r0130_train_receipt": expected_input_signature(
        ...
        "frozen_dose_source": {
            "round_id": "0107",
            "successful_updates": FIXED_SUCCESSFUL_UPDATES,
            "derivation_for_r0130": "fixed-not-edge-derived",
        },
...
def _evaluation_job(job: Mapping[str, Any]) -> dict[str, Any]:
    ...
    graph_signature = expected_input_signature(graph_path)
    graph = read_sealed(
        graph_path, label="R0130 evaluation graph", schema=GRAPH_SCHEMA
    )
    if (
        graph.get("round_id") != ROUND_ID
        or int(graph.get("k_real", -1)) != GRAPH_K
        ...
        raise Round0130Error("R0130 evaluation graph lineage changed")
    selected["graph_manifest_sha256"] = graph_signature["sha256"]
...
def run_matched_density(
    ...
    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0130 matched R0040 diagnostic"
    )
    started = time.monotonic()
...
def _read_exact_receipt(
    ...
    *,
    ...
    if value.get("round_id") != round_id:
        raise Round0130Error(f"{label} round changed")
...
def run_decision(
    ...
    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0130 k49 rescue decision"
    )
    ...
    treatment_core = read_sealed(
        treatment_core_path, label="R0130 treatment core", schema=CORE_SCHEMA
    )
    treatment_ood = read_sealed(
        treatment_ood_path, label="R0130 treatment OOD", schema=OOD_SCHEMA
    )
    ...
    train_receipt = read_sealed(
        train_receipt_path,
        label="R0130 treatment train receipt",
    ...
    isolation = read_sealed(
        isolation_path,
        label="R0130 treatment isolation",
        schema="round0130-k49-treatment-isolation-v1",
    )
    ...
    if (
        ...
        or isolation.get("r0130_train_receipt") != train_signature
        ...
        raise Round0130Error("R0130 treatment isolation/dose lineage changed")
    ...
    treatment_arrays_path = verify_signature(
        treatment_core.get("arrays"), label="R0130 treatment core arrays"
    )
    ...
    for key in (
        ...
        if not np.array_equal(control_arrays[key], treatment_arrays[key]):
            raise Round0130Error(f"paired native anchor {key} changed")
    ...
    matched = read_sealed(
        matched_path,
        label="R0130 matched R0040 diagnostic",
        schema=MATCHED_DENSITY_SCHEMA,
    )
    ...
    receipt = seal({
        ...
        "capability_releases": [
            *(
                [f"capability:{DEGREE_RESCUE_CAPABILITY}"]
                if causal_release else []
            ),
            *(
                [f"capability:{ATLAS_QUALITY_CAPABILITY}"]
...
def run_job(
    ...
    if active.get("manifest", {}).get("round_id") != ROUND_ID or job is None:
        raise Round0130Error("R0130 handler requires its exact round/job")
    handlers = {
    ...
    try:
        ...
        raise Round0130Error(f"unknown R0130 action: {job.get('action')!r}") from exc
