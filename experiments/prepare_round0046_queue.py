#!/usr/bin/env python3
"""Prepare, but never launch, the Round 0046 source-exposure queue."""
...
ROUND_FILE = os.path.join(
    LAB_ROOT,
    "round-0046-2026-07-25.md",
)
ROUND_ROOT = "/data/latent-basemap/runs/round-0046"
...
def _require_issued_round() -> None:
    ...
    if statuses != ["issued"]:
        raise RuntimeError(
            f"R0046 requires one issued status; observed {statuses}"
        )


def prepare_round0046(
    *,
    release_sha: str,
    ...
    template_path = os.path.join(
        inputs_root,
        "round0046-transform-spec-template.json",
    )
    ...
    protocol_paths = [
        ...
        R0042_TRAIN_RECEIPT,
        os.path.join(
            os.path.dirname(R0042_TRAIN_RECEIPT),
    ...
    inputs = _dedupe([
        ...
        *_coordinate_inputs(R0042_COORDINATES),
    ])
    ...
    manifest["schema"] = "round0046-source-exposure-queue-v1"
    ...
    manifest["scientific_contract"] = {
        "control": (
            "accepted R0042 canonical graph, uniform positive-source law"
        ),
        "treatment": (
            "same graph, uniform valid canonical edge with replacement"
        ),
        ...
        "matched": [
            ...
            "seed42",
            "h2048 model",
            "optimizer and 500k successful updates",
            "bf16",
            "R0040 representative panel",
        ],
        "primary_question": (
            "does degree-proportional source exposure materially recover "
            "R0042 density without quality regression?"
        ),
        "no_scale_claim": True,
    }
    common = {
        "handler_module": "experiments.round0046_nodes",
    ...
    manifest["jobs"] = [
        {
            **common,
            "id": "train_seed42_edge_uniform_30m",
            "action": "train",
            "deps": [],
            "outputs": [train_output],
            "done_marker": os.path.join(
                artifacts,
                "train_seed42_edge_uniform_30m.done.json",
            ),
            "p90_wall_s": 5_100.0,
            "node_policy": {
                "gpu_required": True,
        ...
        {
            **common,
            "id": "transform_edge_uniform_30m",
            "action": "transform",
            "deps": ["train_seed42_edge_uniform_30m"],
            "outputs": [transform_output],
            "done_marker": os.path.join(
                artifacts,
                "transform_edge_uniform_30m.done.json",
            ),
            "p90_wall_s": 300.0,
        ...
        {
            **common,
            "id": "matched_source_exposure_panel",
            "action": "matched_panel",
            "deps": ["transform_edge_uniform_30m"],
            "outputs": [panel_output],
            "done_marker": os.path.join(
                artifacts,
                "matched_source_exposure_panel.done.json",
            ),
            "p90_wall_s": 600.0,
            "train_output": train_output,
    ...
    manifest["p90_gpu_seconds"] = {
        "train_seed42_edge_uniform_30m": 5_100.0,
        "transform_edge_uniform_30m": 300.0,
        "matched_source_exposure_panel": 600.0,
        "total": 6_000.0,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0046(
