"""Fresh-process handlers for the R0048 paired source-exposure replicate."""
...
def _load_graph_configs(
    ...
    if expected != observed:
        raise Round0042PipelineError("R0048 queue/config identities changed")
...
def _load_training_input(
    ...
    *,
    ...
    if arm not in ARMS:
        raise Round0042PipelineError(f"unknown R0048 arm: {arm!r}")
    ...
    if (
        ...
        raise Round0042PipelineError("R0048 eligibility changed")
    ...
    return Round0042TrainingInput(
        ...
        positive_source_law=(
            "uniform-edge" if arm == "edge_uniform" else "uniform-source"
        ),
    )
...
def _arm(job: Mapping[str, Any]) -> str:
    arm = str(job.get("arm"))
    if arm not in ARMS:
        raise Round0042PipelineError(f"invalid R0048 arm: {arm!r}")
    return arm
...
def run_train(
    ...
    output = create_fresh_directory(
        job["outputs"][0],
        label=f"Round 0048 {arm} train output",
    )
    atomic_write_new_json(
        os.path.join(output, "production-config.json"),
        {
            "schema": f"round0048-{arm}-production-config-receipt-v1",
            "config": config,
    ...
    wrapper = _load_training_input(graph, arm=arm)
    ...
    if mismatches:
        raise Round0042PipelineError(
            f"R0048 {arm} exact train accounting failed: {mismatches}"
        )
    ...
    if (
        ...
        raise Round0042PipelineError(
            f"R0048 {arm} runtime stamp differs: {runtime_mismatches}"
        )
    ...
    if (
        ...
        raise Round0042PipelineError(
            f"R0048 {arm} profiler did not close every window"
        )
    ...
    body = {
        "schema": "round0048-train-receipt-v1",
        "round_id": ROUND_ID,
        "arm": arm,
...
def run_transform(
    ...
    train = _read_sealed(
        train_path,
        label=f"Round 0048 {arm} train receipt",
    )
    if (
        train.get("schema") != "round0048-train-receipt-v1"
        ...
        raise Round0042PipelineError(
            f"R0048 {arm} transform lacks a valid train"
        )
    output = create_fresh_directory(
        job["outputs"][0],
        label=f"Round 0048 {arm} coordinate output",
    )
    ...
    body = {
        "schema": "round0048-transform-capability-v1",
        "round_id": ROUND_ID,
        "arm": arm,
        **result,
...
def _render_pair(
    *,
    ...
    points = {
        arm: np.asarray(coordinates[arm][compact], dtype=np.float32)
        for arm in ARMS
    }
    if any(
        ...
        raise Round0042PipelineError("R0048 render coordinates invalid")
    image_path = os.path.join(
        output,
        "seed43-source-vs-edge-uniform.png",
    )
...
def run_matched_panel(
    ...
    if selector_signature["sha256"] != SELECTOR_SHA256:
        raise Round0042PipelineError("R0048 selector changed")
    ...
    coordinate_roots = {
        arm: str(job[f"{arm}_transform_output"])
        for arm in ARMS
    }
    cells: dict[str, Any] = {}
    coordinate_views: dict[str, RepresentativeArrayView] = {}
    for arm in ARMS:
        root = coordinate_roots[arm]
        full, record = _coordinate_stream(
            root,
            expected_schema="round0048-transform-capability-v1",
        )
        if record.get("arm") != arm:
            raise Round0042PipelineError(
                f"R0048 coordinate arm mismatch: {arm}"
            )
        coordinates = RepresentativeArrayView(full, selector)
        coordinate_views[arm] = coordinates
        panel = score_panel(
            X,
            coordinates,
            config=config,
        ...
        cell_body = {
            "schema": "round0048-representative-cell-v1",
            "round_id": ROUND_ID,
            "arm": arm,
            "seed": SEED,
        ...
        cell_path = os.path.join(output, f"{arm}-panel.json")
        atomic_write_new_json(
            cell_path,
            _seal(cell_body),
            immutable=True,
    ...
    source = cells["source_uniform"]
    edge = cells["edge_uniform"]
    deltas = {
        key: edge["scalars"][key] - source["scalars"][key]
        for key in edge["scalars"]
    }
    projection_deltas = {
        key: edge["projection"][key] - source["projection"][key]
    ...
    trains = {
        arm: _read_sealed(
            ...
            label=f"R0048 {arm} train receipt",
        )
        for arm in ARMS
    }
    runtimes = {
        arm: trains[arm]["exact_execution_receipt"]
        for arm in ARMS
    }
    arm_configs = {
        arm: configs[arm][0]
    ...
    matched = {
        "row_universe": (
            arm_configs["source_uniform"]["row_universe"]
            == arm_configs["edge_uniform"]["row_universe"]
        ),
        "model": (
            arm_configs["source_uniform"]["model"]
            == arm_configs["edge_uniform"]["model"]
        ),
        "optimizer": (
            arm_configs["source_uniform"]["optimizer"]
            == arm_configs["edge_uniform"]["optimizer"]
        ),
        "graph": (
            trains["source_uniform"]["graph"]
            == trains["edge_uniform"]["graph"]
        ),
        "features": (
            trains["source_uniform"]["train_accounting"][
                "verified_hashes"
            ]["features"]
            == trains["edge_uniform"]["train_accounting"][
                "verified_hashes"
            ]["features"]
        ),
        "seed": all(trains[arm]["seed"] == SEED for arm in ARMS),
        "successful_updates": all(
            trains[arm]["train_accounting"][
                "positive_lr_optimizer_steps"
            ] == SUCCESSFUL_UPDATES
            for arm in ARMS
        ),
        "all_non_source_runtime_fields": all(
            runtimes["source_uniform"].get(key)
            == runtimes["edge_uniform"].get(key)
            for key in same_runtime_fields
        ),
        "source_arm_source_uniform": (
            runtimes["source_uniform"]["positive_sampling"].startswith(
                "uniform-retained-positive-source"
            )
        ),
        "edge_arm_edge_uniform": (
            runtimes["edge_uniform"]["positive_sampling"]
            == "uniform-valid-canonical-edge-with-replacement"
            and runtimes["edge_uniform"]["positive_source_sampling"]
            == "degree-proportional-over-positive-sources"
        ),
    }
    numerical_guards: dict[str, bool] = {}
    for arm, cell in cells.items():
        sealed = _read_sealed(
            cell["receipt"]["canonical_path"],
            label=f"R0048 {arm} panel",
        )
        numerical_guards[arm] = (
            sealed["panel"]["guards"].get("coords_finite") is True
            and sealed["panel"]["guards"].get("coords_collapsed") is False
        )
    thresholds = arm_configs["edge_uniform"]["decision_thresholds"]
    ...
    body = {
        "schema": "round0048-paired-source-exposure-comparison-v1",
        "round_id": ROUND_ID,
        "release_sha": active["manifest"]["release_sha"],
        "seed": SEED,
        "scientific_universe": {
        ...
        "decision_checks": {
            ...
            "both_profilers_have_200_windows": profilers_complete,
        ...
        "interpretation": {
            "paired_seed43_replication": True,
            "isolated_treatment": "positive-source exposure law only",
    ...
    path = os.path.join(
        output,
        "paired-source-exposure-comparison-v1.json",
    )
    atomic_write_new_json(path, receipt, immutable=True)
    return {**receipt, "receipt": expected_input_signature(path)}
...
def run_job(
    ...
    if active.get("manifest", {}).get("round_id") != ROUND_ID:
        raise RuntimeError("R0048 handler received another queue")
    selected = job if job is not None else active.get("job") or {}
    if len(selected.get("outputs") or []) != 1:
        raise RuntimeError("R0048 job output contract changed")
    handlers = {
        "train": run_train,
    ...
    if handler is None:
        raise RuntimeError(
            f"unknown R0048 action: {selected.get('action')!r}"
        )
