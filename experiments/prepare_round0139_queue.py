#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0139 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0139"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0139-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-003-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    ...
    "hin_Deva": {"mean": 543.527, "p95": 713.0, "maximum": 824},
    "ind_Latn": {"mean": 499.841, "p95": 651.05, "maximum": 752},
    "ita_Latn": {"mean": 440.569, "p95": 627.05, "maximum": 688},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0139 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0139 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0139_inputs() -> dict[str, Any]:
...
def prepare_round0139(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0139 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0139 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0139_nodes",
    ...
    queue["schema"] = "round0139-canonical-prompted-multilingual-queue-v1"
