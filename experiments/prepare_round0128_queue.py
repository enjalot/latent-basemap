#!/usr/bin/env python3
"""Prepare, but never launch, the conditional R0128 k49 rescue queue."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0128"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0128-*.md")
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0128 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def _job(
    ...
    **values: Any,
    ...
    return {
        "id": node_id,
        "action": action,
        "handler_module": "experiments.round0128_nodes",
...
def prepare_round0128(
    *,
    ...
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0128 release SHA must be one full commit")
    ...
    queue["schema"] = "round0128-direct-k49-degree-rescue-queue-v1"
    queue["repo_root"] = RELEASE_ROOT
    queue["queue_class"] = "gpu-research"
    queue["required_reviews"] = ["0105", "0106", "0107", "0108", "0124"]
    ...
    queue["capabilities_produced"] = [
        "jina-diverse-25m-native-k49-degree-rescue-v1",
        "jina-diverse-25m-k49-atlas-quality-v1",
    ]
    ...
    queue["scientific_contract"] = {
        ...
        "dose_rule": "fixed R0107 dose; never edge-derived for R0128",
        ...
        "registry_or_render_publication": (
            "not in R0128; separate CPU follow-up if atlas quality releases"
        ),
    }
    queue["jobs"] = jobs
