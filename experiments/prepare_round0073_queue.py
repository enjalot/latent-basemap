#!/usr/bin/env python3
"""Prepare the fixed balanced-90M GPU-native graph queue."""
...
from basemap.round0073_graph import ROUND_ID
...
from experiments.round0073_nodes import RUNTIME_SPEC


ROUND_ROOT = "/data/latent-basemap/runs/round-0073"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(
    LAB_ROOT,
    "round-0073-2026-07-27.md",
)
...
def _require_issued_round() -> None:
    if _frontmatter_status(ROUND_FILE) != "issued":
        raise RuntimeError("R0073 remains draft; refuse queue materialization")


def prepare_round0073(
    *,
    release_sha: str,
    ...
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0073 release SHA must be one full commit")
    ...
    if (
        ...
        raise RuntimeError("R0073 filtered-index bytes changed")
    ...
    if runtime["sha256"] != runtime_spec_sha256:
        raise RuntimeError("R0073 runtime specification changed")
    queue_root = create_fresh_directory(
        queue_root,
        label="Round 0073 GPU queue",
    )
    ...
    manifest["schema"] = "round0073-balanced-90m-graph-queue-v1"
    manifest["repo_root"] = RELEASE_ROOT
    manifest["queue_class"] = "gpu-research"
    manifest["required_reviews"] = ["0071", "0072"]
    ...
    manifest["scientific_contract"] = {
        ...
        "nprobe_selected_only_by_r0072": nprobe,
        "search_width": 128,
    ...
    manifest["jobs"] = [{
        "id": "build_gpu_native_graph_balanced_90m",
        "action": "build_balanced_90m_gpu_graph",
        "handler_module": "experiments.round0073_nodes",
        "handler_callable": "run_job",
        "deps": [],
        "outputs": [output],
        "done_marker": os.path.join(
            artifacts,
            "build_gpu_native_graph_balanced_90m.done.json",
        ),
    ...
    manifest["p90_gpu_seconds"] = {
        "build_gpu_native_graph_balanced_90m": 10_800.0,
        "total": 10_800.0,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0073(
