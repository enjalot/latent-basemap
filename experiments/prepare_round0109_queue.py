#!/usr/bin/env python3
"""Prepare the seed-43 replicate of the 25M diverse-Jina training queue."""
...
ROUND_ID = "0109"
SEED = 43
ROUND_ROOT = "/data/latent-basemap/runs/round-0109"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0109-*.md")
GRAPH_MANIFEST = (
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0109 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]


def _require_successful_r0108_terminal(
    path: str,
    *,
    expected_sha256: str,
) -> dict[str, Any]:
    ...
    if signature["sha256"] != expected_sha256:
        raise RuntimeError("R0108 terminal receipt bytes changed")
    with open(path, encoding="utf-8") as handle:
    ...
    if (
        ...
        raise RuntimeError("R0108 did not reach a clean terminal evaluation")
    return signature


def prepare_round0109(
    *,
    release_sha: str,
    r0107_review_path: str,
    r0107_review_sha256: str,
    r0108_terminal_path: str,
    r0108_terminal_sha256: str,
    queue_root: str = os.path.join(ROUND_ROOT, "queue"),
) -> str:
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0109 release SHA must be one full commit")
    ...
    r0108_terminal = _require_successful_r0108_terminal(
        r0108_terminal_path,
        expected_sha256=r0108_terminal_sha256,
    )
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0109 seed-43 diverse-Jina train queue"
    )
    ...
    output = os.path.join(artifacts, "train-diverse-jina-25m-seed43")
    node_id = "train_diverse_jina_25m_seed43"
    jobs = [{
        "id": node_id,
        "action": "train_diverse_jina_seed43",
        "handler_module": "experiments.round0109_nodes",
    ...
    queue["schema"] = "round0109-diverse-jina-seed43-training-queue-v1"
    ...
    queue["scientific_contract"] = {
        "role": "independent seed replicate; no seed-42 outcome tuning",
        "rows": RETAINED_ROWS,
        "dimension": DIMENSION,
        "seed": SEED,
        "seed42_comparator_review": reviews["0107"],
        "r0108_terminal_ordering_receipt": r0108_terminal,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--r0108-terminal", required=True)
    parser.add_argument("--r0108-terminal-sha256", required=True)
    parser.add_argument(
        "--queue-root", default=os.path.join(ROUND_ROOT, "queue")
    )
    args = parser.parse_args(argv)
    path = prepare_round0109(
        release_sha=args.release_sha,
        r0107_review_path=args.r0107_review,
        r0107_review_sha256=args.r0107_review_sha256,
        r0108_terminal_path=args.r0108_terminal,
        r0108_terminal_sha256=args.r0108_terminal_sha256,
