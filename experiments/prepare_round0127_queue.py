#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0127 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0127"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0127-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-002-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    ...
    "deu_Latn": {"mean": 429.908, "p95": 616.0, "maximum": 661},
    "ell_Grek": {"mean": 393.555, "p95": 499.0, "maximum": 580},
    "fra_Latn": {"mean": 440.627, "p95": 610.0, "maximum": 663},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0127 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0127 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0127_inputs() -> dict[str, Any]:
...
def prepare_round0127(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0127 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0127 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0127_nodes",
    ...
    queue["schema"] = "round0127-canonical-prompted-multilingual-queue-v1"
