"""Execute R0199's authority-corrected CPU-only GrEASE diagnosis."""
...
def _signature(expected: Mapping[str, Any], *, label: str) -> dict[str, Any]:
    ...
    if actual != dict(expected):
        raise Round0199Error(f"{label} bytes changed")
    return actual
...
def run_job(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    if (
        ...
        raise Round0199Error("unknown R0199 action or queue")
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"", "-1"}:
        raise Round0199Error("R0199 F1 is CPU-only")
    ...
    _signature(job["reference_script"], label="R0199 reference adapter")
    _signature(
        job["toolchain_python"]["resolved_interpreter"],
        label="R0199 toolchain interpreter",
    )

    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0199 GrEASE diagnosis"
    )
    ...
    if completed.returncode != 0:
        ...
        raise Round0199Error(
            f"R0199 reference diagnosis failed with {completed.returncode}: {tail}"
        )
    ...
    receipt = seal({
        "schema": "round0199-grease-batch-stability-diagnosis-v1",
        "round_id": ROUND_ID,
