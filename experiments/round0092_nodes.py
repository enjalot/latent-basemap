"""Train the deliberate balanced-150M rung on the canonical host-int8 path."""
...
class HostInt8Balanced150mCanonicalSampler(
    HostInt8SelectedCanonicalSampler
):
    """Canonical host-int8 sampler with an exact 150M execution stamp."""
...
class Round0092TrainingInput(Round0034TrainingInput):
    ...
    def prepare_round0034_training(
        ...
        *,
        ...
        "Round0092TrainingInput",
        HostInt8Balanced150mCanonicalSampler,
        int,
        dict[str, Any],
        dict[str, Any],
    ]:
...
def _load_scale_evidence(
    ...
    with open(str(job["scale_geometry"]), encoding="utf-8") as handle:
        scale = json.load(handle)
    validate_seal(scale, label="R0080 scale geometry")
    signature = expected_input_signature(str(job["scale_geometry"]))
    same_row = scale.get("same_row_90m_comparison") or {}
    decision = scale.get("decision") or {}
    density = scale.get("density_semantics") or {}
    if (
        signature["sha256"] != job["scale_geometry_sha256"]
        or scale.get("schema") != "round0080-scale-geometry-comparison-v1"
        or scale.get("round_id") != "0080"
        or same_row.get("passed") is not True
        or scale.get("full_120m_non_density_checks_passed") is not True
        or decision.get("120m_supported_as_deliberate_ladder_rung")
        is not True
        ...
        raise Round0034PipelineError("R0080 scale evidence changed")
    return signature
...
def _load_pipeline(
    ...
    scale_signature = _load_scale_evidence(job)
    ...
    # reading it into RAM.  Reuse the authenticated signatures within this
    # same process and perform only the required load.
    encoded = np.fromfile(
        outputs["int8"]["canonical_path"], dtype=np.int8
    ).reshape(ROW_COUNT, DIMENSION)
    scales = np.fromfile(
        outputs["scales"]["canonical_path"], dtype="<f2", count=ROW_COUNT
    )
    ...
    wrapper = Round0092TrainingInput(
        dataset,
        graph,
