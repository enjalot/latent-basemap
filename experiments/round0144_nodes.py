"""Independent GPU embedding nodes and CPU finalizer for R0144."""
...
from basemap.round0144_prompted_multilingual import (
    BATCH_SIZE,
    CAPABILITY,
    CHUNK_ROWS,
    COMPUTE_DTYPE,
...
def _rehash_boundary_inputs(
    ...
    *,
    ...
    if not bindings:
        raise Round0144Error("R0144 job has no authenticated boundary inputs")
    ...
    if (
        ...
        raise Round0144Error("R0144 boundary input role closure is incomplete")
    ...
    for binding in bindings:
        ...
        if actual != expected:
            raise Round0144Error(
                f"R0144 {role} bytes changed at job boundary: "
...
def _verify_semantic_closures(
    ...
    if inventory_signature != job.get("inventory_manifest_signature"):
        raise Round0144Error("R0087 inventory binding changed at job boundary")
    ...
    if r0114_signature != job.get("r0114_manifest_signature"):
        raise Round0144Error("R0114 model/prompt binding changed at job boundary")
    return {
...
def _verify_loaded_model_members(
    ...
    if loaded != bound:
        raise Round0144Error("loaded Jina model differs from the queue binding")
...
def _validate_node_source(source: Mapping[str, Any], *, node_id: str) -> None:
    ...
    if (
        ...
        raise Round0144Error(f"R0144 source-row identity changed for {node_id}")
...
def run_embed_document_rows(
    ...
    if (
        ...
        raise Round0144Error(f"R0144 node {node_id} is malformed")
    ...
    for index, chunk_start in enumerate(
        ...
        if (
            ...
            raise Round0144Error(
                "R0144 document embedding throughput regressed below "
        ...
        if (
            ...
            print(
                "[round0144] WARNING cumulative document rate "
                f"{cumulative_rate:.1f} rows/s is below "
                f"{EMBED_WARNING_ROWS_PER_S:.1f}",
                flush=True,
            )
        print(
            f"[round0144] {node_id} chunk {index + 1}/"
...
def _scan_bound_chunk(
    ...
    actual = expected_input_signature(str(expected["canonical_path"]))
    if actual != expected:
        raise Round0144Error("finalizer found changed embedding bytes")
...
def run_finalize(
    ...
    if sorted(
        ...
        raise Round0144Error("finalizer source-byte bindings changed")

    receipt_paths = [str(path) for path in job.get("node_receipts") or []]
    if len(receipt_paths) != len(LANGUAGE_TRANCHES):
        raise Round0144Error("R0144 finalizer has wrong node receipt count")
    ...
    for path in receipt_paths:
        ...
        if (
            ...
            raise Round0144Error(
                f"R0144 node {node_id} finalizer binding changed"
            )
        seen_nodes.add(node_id)
        receipts.append(receipt)
        receipt_signatures.append(signature)
    if seen_nodes != {str(item["node_id"]) for item in LANGUAGE_TRANCHES}:
        raise Round0144Error("R0144 finalizer is missing a work node")
    ...
    if scanned_rows != CORPUS_ROWS:
        raise Round0144Error("R0144 finalizer did not scan every row")

    output = create_fresh_directory(
        job["outputs"][0], label="R0144 multilingual prompted corpus"
    )
...
def run_job(
    ...
    if active.get("manifest", {}).get("round_id") != ROUND_ID:
        raise Round0144Error("R0144 handler received another queue")
    ...
    raise Round0144Error(f"unknown R0144 action: {action!r}")
