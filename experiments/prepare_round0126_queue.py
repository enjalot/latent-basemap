#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0126 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0126"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0126-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-001-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    "sampling": "1000 evenly spaced rows per corpus with literal Document prefix",
    "reference_pile": {"mean": 437, "p95": 572, "maximum": 1328},
    "arb_Arab": {"mean": 424, "p95": 531, "maximum": 626},
    "ces_Latn": {"mean": 399, "p95": 518, "maximum": 592},
    "cmn_Hani": {"mean": 599, "p95": 751, "maximum": 819},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0126 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0126 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0126_inputs() -> dict[str, Any]:
...
def prepare_round0126(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0126 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0126 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0126_nodes",
    ...
    queue["schema"] = "round0126-canonical-prompted-multilingual-queue-v1"
