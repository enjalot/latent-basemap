"""Execute R0200's corrected CPU-only GrEASE diagnosis."""
...
def _signature(expected: Mapping[str, Any], *, label: str) -> dict[str, Any]:
    ...
    if actual != dict(expected):
        raise Round0200Error(f"{label} bytes changed")
    return actual
...
def run_job(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    if (
        ...
        raise Round0200Error("unknown R0200 action or queue")
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"", "-1"}:
        raise Round0200Error("R0200 F1 is CPU-only")
    ...
    _signature(job["reference_script"], label="R0200 reference adapter")
    _signature(
        job["toolchain_python"]["resolved_interpreter"],
        label="R0200 toolchain interpreter",
    )

    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0200 GrEASE diagnosis"
    )
    ...
    if completed.returncode != 0:
        ...
        raise Round0200Error(
            f"R0200 reference diagnosis failed with {completed.returncode}: {tail}"
        )
    ...
    receipt = seal({
        "schema": "round0200-grease-batch-stability-diagnosis-v1",
        "round_id": ROUND_ID,
        ...
        "scope": {
            ...
            "numap_toy_fit": {
                ...
                "shares_f1_batch_geometry_cause": False,
