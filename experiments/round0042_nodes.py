"""Fresh-process handlers for the R0042 canonical-destination isolation."""
...
def _seal(body: Mapping[str, Any]) -> dict[str, Any]:
    payload = dict(body)
    return {**payload, "identity_sha256": sha256_bytes(
...
def _load_graph_config(
    ...
    if job.get("train_config_sha256") != config_sha256:
        raise Round0042PipelineError("R0042 queue/config identity changed")
...
def _load_training_input(
    ...
    if (
        ...
        raise Round0042PipelineError("R0042 eligibility changed")
...
def _exact_model(config: Mapping[str, Any]):
    from experiments.run_round0034_node import _exact_model

    return _exact_model(dict(config))
...
def run_train(
    ...
    output = create_fresh_directory(
        job["outputs"][0], label="Round 0042 train output"
    )
    atomic_write_new_json(
        os.path.join(output, "production-config.json"),
        {
            "schema": "round0042-production-config-receipt-v1",
            "config": config,
    ...
    if mismatches:
        raise Round0042PipelineError(
            f"R0042 exact train accounting failed: {mismatches}"
        )
    ...
    if (
        ...
        raise Round0042PipelineError(
            "R0042 runtime stamp differs from admission: "
    ...
    body = {
        "schema": "round0042-train-receipt-v1",
        "round_id": ROUND_ID,
...
def run_transform(
    ...
    train = _read_sealed(train_path, label="Round 0042 train receipt")
    if (
        train.get("production_config_sha256") != config_sha256
        or train.get("train_accounting", {}).get("budget_satisfied") is not True
    ):
        raise Round0042PipelineError("R0042 transform lacks a valid train")
    output = create_fresh_directory(
        job["outputs"][0], label="Round 0042 coordinate output"
    )
    ...
    body = {
        "schema": "round0042-transform-capability-v1",
        "round_id": ROUND_ID,
        **result,
...
def _coordinate_stream(
    ...
    *,
    expected_schema: str,
) -> tuple[CachedShardedArray, dict[str, Any]]:
    record_path = os.path.join(root, "actual-transform.json")
    record = _read_sealed(record_path, label=f"{expected_schema} coordinates")
    capability = record.get("stream_capability", {}).get(
        "capability_payload"
    )
    stream = record.get("stream_capability") or {}
    if (
        record.get("schema") != expected_schema
        or not isinstance(capability, dict)
        or capability.get("schema") != "round0013-stream-output-v1"
        or stream.get("capability_sha256")
        != sha256_bytes(canonical_json(capability))
        or capability.get("plan", {}).get("output", {}).get("shape")
        != [ROW_COUNT, 2]
        or capability.get("plan", {}).get("output", {}).get("dtype")
        != "<f4"
    ):
        raise Round0042PipelineError(
            f"{expected_schema} coordinate capability changed"
        )
    members = []
    cursor = 0
    ordered = capability.get("ordered_chunks")
    if not isinstance(ordered, list) or len(ordered) != 30:
        raise Round0042PipelineError("coordinate stream is not 30 chunks")
    for position, item in enumerate(ordered):
        ...
        if (
            ...
            or start != cursor
            or stop != min(start + 1_000_000, ROW_COUNT)
            or signature["sha256"] != item["sha256"]
            or signature["bytes"] != item["size_bytes"]
        ):
            raise Round0042PipelineError(
                f"coordinate chunk {position} changed"
            )
        members.append({
            "path": path,
...
def _panel_scalars(panel: Mapping[str, Any]) -> dict[str, float]:
    ...
    if any(not np.isfinite(value) for value in values.values()):
        raise Round0042PipelineError(
            f"non-finite panel scalar: {values}"
        )
    return values
...
def _projection(
    *,
    ...
    k_fraction = max(
        config.k_hit, int(math.ceil(config.frac * len(coordinates)))
    )
    low = cross_knn(
        query_coordinates,
        coordinates,
    ...
    high = np.asarray(truth["neighbors"], dtype=np.int64)[
        :, :config.k_hit
    ]
    return {
        "proj_ffr": round(
...
def _render_comparison(
    *,
    ...
    compact = np.sort(
        rng.choice(len(next(iter(coordinates.values()))), 50_000, replace=False)
    ).astype(np.int64)
    ...
    if any(
        ...
        raise Round0042PipelineError("comparison render coordinates invalid")
    image_path = os.path.join(output, "r0021-vs-r0042.png")
    ...
    def draw(path: str) -> None:
        ...
        for axis, label in zip(axes, ("r0021_raw", "r0042_canonical")):
            value = points[label]
            axis.scatter(
                value[:, 0], value[:, 1],
                s=0.15, alpha=0.35, linewidths=0, rasterized=True,
            )
...
def run_matched_panel(
    ...
    output = create_fresh_directory(
        job["outputs"][0], label="Round 0042 matched panel output"
    )
    started = time.monotonic()
    reference_signature = expected_input_signature(REFERENCE_RECEIPT)
    if reference_signature["sha256"] != REFERENCE_RECEIPT_SHA256:
        raise Round0042PipelineError("R0040 reference receipt changed")
    reference_receipt = _read_sealed(
        REFERENCE_RECEIPT, label="Round 0040 MiniLM reference"
    )
    ...
    if selector_signature["sha256"] != SELECTOR_SHA256:
        raise Round0042PipelineError("R0040 selector changed")
    ...
    centroids = {
        256: np.load(
            CENTROIDS_K256_PATH, mmap_mode="r", allow_pickle=False
        ),
        1024: np.load(
            CENTROIDS_K1024_PATH, mmap_mode="r", allow_pickle=False
        ),
    }
    coordinate_specs = {
        "r0021_raw": (
            R0021_COORDINATES,
            "round0021-transform-capability-v1",
        ),
        "r0042_canonical": (
    ...
    for label, (root, schema) in coordinate_specs.items():
        ...
        cell_body = {
            "schema": "round0042-representative-cell-v1",
            "round_id": ROUND_ID,
            "cell": label,
        ...
        atomic_write_new_json(
            cell_path, _seal(cell_body), immutable=True
        )
        cells[label] = {
    ...
    baseline = cells["r0021_raw"]
    treatment = cells["r0042_canonical"]
    deltas = {
    ...
    r0021_train = _read_sealed(
        R0021_TRAIN_RECEIPT, label="R0021 train receipt"
    )
    r0042_train_path = os.path.join(
        job["train_output"], "train-receipt.json"
    )
    r0042_train = _read_sealed(
        r0042_train_path, label="R0042 train receipt"
    )
    r0021_config_path = os.path.join(
        os.path.dirname(R0021_TRAIN_RECEIPT), "production-config.json"
    )
    with open(r0021_config_path, encoding="utf-8") as handle:
        r0021_config = json.load(handle)["config"]
    r0042_config = r0042_train["production_config"]
    matched = {
        "row_universe": (
            r0021_config["row_universe"] == r0042_config["row_universe"]
        ),
        "model": r0021_config["model"] == r0042_config["model"],
        "optimizer": (
            r0021_config["optimizer"] == r0042_config["optimizer"]
        ),
        "seed": (
            r0021_train["seed"] == r0042_train["seed"] == 42
        ),
        "successful_updates": (
            r0021_train["train_stats"]["positive_lr_optimizer_steps"]
            == r0042_train["train_accounting"][
                "positive_lr_optimizer_steps"
            ]
            == SUCCESSFUL_UPDATES
        ),
        "source_law": (
            r0021_train["train_stats"][
                "pipeline_multiplicity_positive_source_sampling"
            ]
            == "uniform_retained_rows_then_fixed_k_slot_with_replacement"
            and r0042_train["exact_execution_receipt"][
                "positive_sampling"
            ].startswith("uniform-retained-positive-source")
        ),
        "source_universe_difference_bounded_to_139_zero_degree_rows": (
            r0042_train["exact_execution_receipt"][
    ...
    guards = {
        label: (
            cell["scalars"]["ffr"] >= 0
            and cell["projection"]["proj_ffr"] >= 0
            and _read_sealed(
                cell["receipt"]["canonical_path"],
                label=f"{label} panel",
            )["panel"]["guards"].get("coords_finite") is True
            and _read_sealed(
                cell["receipt"]["canonical_path"],
                label=f"{label} panel",
            )["panel"]["guards"].get("coords_collapsed") is False
        )
        for label, cell in cells.items()
    }
    thresholds = r0042_config["decision_thresholds"]
    checks = {
        "matched_contract": all(matched.values()),
        "both_numerical_guards": all(guards.values()),
        "representative_ffr_noninferior": (
            deltas["ffr"]
            >= thresholds["representative_ffr_delta_min"]
        ),
        "representative_projection_ffr_noninferior": (
            projection_deltas["proj_ffr"]
            >= thresholds["representative_projection_ffr_delta_min"]
        ),
        "representative_purity_k256_noninferior": (
            deltas["purity_k256"]
            >= thresholds["representative_purity_delta_min"]
        ),
        "representative_purity_k1024_noninferior": (
            deltas["purity_k1024"]
            >= thresholds["representative_purity_delta_min"]
        ),
    }
    ...
    body = {
        "schema": "round0042-matched-canonical-comparison-v1",
        "round_id": ROUND_ID,
        "release_sha": active["manifest"]["release_sha"],
        "scientific_universe": {
            "rows": len(X),
        ...
        "cells": cells,
        "representative_deltas_canonical_minus_raw": deltas,
        "projection_deltas_canonical_minus_raw": projection_deltas,
        ...
        "interpretation": {
            "isolated_treatment": (
                "canonical representative destinations plus the unavoidable "
                "removal of 139 post-canonicalization zero-degree sources"
            ),
            "all_row_density_is_not_a_primary_geometry_metric": True,
            "scale_claimed": False,
            "near_or_text_deduplication_claimed": False,
        },
        "prior_R0021_all_row_panel": expected_input_signature(R0021_PANEL),
        "render": render,
        "wall_seconds": time.monotonic() - started,
    }
    receipt = _seal(body)
    path = os.path.join(
        output, "matched-canonical-comparison-v1.json"
    )
    atomic_write_new_json(path, receipt, immutable=True)
    return {**receipt, "receipt": expected_input_signature(path)}
...
def run_job(
    ...
    if active.get("manifest", {}).get("round_id") != ROUND_ID:
        raise RuntimeError("R0042 handler received another queue")
    selected = job if job is not None else active.get("job") or {}
    if len(selected.get("outputs") or []) != 1:
        raise RuntimeError("R0042 job output contract changed")
    handlers = {
        "train": run_train,
    ...
    if handler is None:
        raise RuntimeError(
            f"unknown R0042 action: {selected.get('action')!r}"
        )
