"""Build the immutable pre-gate queue for basemap-100m Round 0015."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--service-decision", required=True)
...
def build_queue(args) -> dict:
    ...
    if out != os.path.join(round_root, "queue", "queue.json"):
        raise ValueError("Round 0015 queue must be round-root/queue/queue.json")
    refuse_existing(queue_root, label="Round 0015 queue root")
    ...
    if not os.path.isfile(python):
        raise FileNotFoundError("Round 0015 sealed interpreter is missing")
    service_path = os.path.realpath(args.service_decision)
    service = load_service_decision(
        service_path, environment_manifest=environment_path,
        require_current=True)
    if service["policy"] != POLICY or len(service["allowed_processes"]) != 1:
        raise ValueError("Round 0015 queue cannot publish an empty/mismatched allowlist")
    ...
    if not preflight["passed"]:
        raise RuntimeError("Round 0015 release preflight rejected: " +
    ...
    artifacts = create_fresh_directory(
        os.path.join(queue_root, "artifacts"), label="Round 0015 artifact parent")
    receipts = create_fresh_directory(
        os.path.join(queue_root, "gate-receipts"),
        label="Round 0015 gate receipts")
    checkpoints = create_fresh_directory(
        os.path.join(queue_root, "controller-checkpoints"),
        label="Round 0015 controller checkpoints")
    create_fresh_directory(cache_root, label="Round 0015 cache parent")
    for key in CACHE_KEYS:
        create_fresh_directory(cache_environment[key],
                               label=f"Round 0015 cache {key}")
    ...
    for value in (terminal, lease_release, gate_preparation, release_receipt_path):
        refuse_existing(value, label="Round 0015 queue control")
    ...
    if not release_reports_equivalent(issued, preflight):
        raise RuntimeError("Round 0015 release changed while queue scaffolding was created")
    ...
    def signature(path: str) -> dict:
        ...
        if prior is not None and prior != observed:
            raise RuntimeError(f"Round 0015 input changed during capture: {canonical}")
    ...
    for reference in staged["references"]:
        ...
        if observed != expected:
            raise RuntimeError(
                f"Round 0015 staged input changed: {reference['role']}")
    role_paths = {
    ...
    staging_body = {
        "schema": "round0015-input-staging-v1",
    ...
    manifest = {
        ...
        "allowed_processes": service["allowed_processes"],
        ...
        "program_policy": program_policy(), "program_inputs": program_inputs,
    ...
    for spec in NODES:
        ...
        for value in (*outputs, *controls.values()):
            refuse_existing(value, label=f"Round 0015 {spec.node_id} control")
        policy = derived_node_policy(spec)
        if policy["gpu_memory_cap_mb"] != JOB_CAP_MIB:
            raise AssertionError("Round 0015 job cap is not exactly 31,488 MiB")
    ...
    # The exact process is re-probed at the final publication edge.  A restart,
    # extra process, cap excess, or changed list leaves no queue JSON.
    final_service = load_service_decision(
        service_path, environment_manifest=environment_path,
        require_current=True)
    if final_service["allowed_processes"] != manifest["allowed_processes"]:
        raise RuntimeError("Round 0015 service changed before queue publication")
