#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0143 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0143"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0143-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-005-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    ...
    "pol_Latn": {"mean": 476.218, "p95": 628.10, "maximum": 745},
    "por_Latn": {"mean": 437.357, "p95": 587.0, "maximum": 670},
    "rus_Cyrl": {"mean": 432.372, "p95": 562.05, "maximum": 668},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0143 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0143 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0143_inputs() -> dict[str, Any]:
...
def prepare_round0143(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0143 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0143 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0143_nodes",
    ...
    queue["schema"] = "round0143-canonical-prompted-multilingual-queue-v1"
    ...
    queue["scientific_contract"] = {
        ...
        "heldout_prompted_languages": ["pol_Latn"],
        ...
        "polish_source_role": (
            "R0087 authenticated heldout-language-probe; never claimed as a "
            "selected raw-atlas training range"
        ),
