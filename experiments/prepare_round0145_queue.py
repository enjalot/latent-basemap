#!/usr/bin/env python3
"""Rehearse or prepare, but never launch, R0145 multilingual production."""
...
ROUND_ROOT = "/data/latent-basemap/runs/round-0145"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
RUN_ENVIRONMENT_PREFIX = os.path.join(RELEASE_ROOT, ".venv")
RUN_PYTHON = os.path.join(RUN_ENVIRONMENT_PREFIX, "bin", "python")
ROUND_FILE_GLOB = os.path.join(LAB_ROOT, "round-0145-*.md")
OUTPUT_NAMESPACE = "canonical-jina-document-native8192-multilingual-007-v1"
REVIEW_DEFAULTS = {
    "0087": (
...
TOKEN_LENGTH_CALIBRATION = {
    "sampling": (
        "1000 numpy-linspace rows over local [0,835453), literal "
        "Document prefix, add_special_tokens=true, no truncation"
    ),
    "sample_indices_sha256": (
        "d4bde7ea9ccdf783f6a33ed818d7ceaefbbb89a155da61916f758dbb4cd4cedb"
    ),
    "reference_pile": {"mean": 437, "p95": 572, "maximum": 1328},
    "tur_Latn": {"mean": 384.885, "p95": 521.1, "maximum": 562},
    "vie_Latn": {"mean": 422.246, "p95": 527.0, "maximum": 605},
...
def _require_dedicated_run_environment() -> None:
    ...
    if observed_python != RUN_PYTHON or observed_prefix != RUN_ENVIRONMENT_PREFIX:
        raise RuntimeError(
            "R0145 queue preparation must use the dedicated run environment: "
...
def _require_issued_round() -> str:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"R0145 requires exactly one issued round; found {len(candidates)}"
        )
    return candidates[0]
...
def rehearse_round0145_inputs() -> dict[str, Any]:
...
def prepare_round0145(
    *,
    ...
    queue_root = create_fresh_directory(
        queue_root, label="R0145 multilingual prompted production queue"
    )
    artifacts = ensure_data_directory(os.path.join(queue_root, "artifacts"))
    namespace = ensure_data_directory(
        os.path.join(artifacts, OUTPUT_NAMESPACE),
        label="R0145 prompted multilingual output namespace",
    )
    common_bindings = [
        ...
        jobs.append(
            {
                ...
                "handler_module": "experiments.round0145_nodes",
    ...
    queue["schema"] = "round0145-canonical-prompted-multilingual-queue-v1"
