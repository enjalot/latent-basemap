#!/usr/bin/env python3
"""Prepare, but never launch, the balanced-60M GPU training queue."""
...
ROUND_ID = "0063"
ROUND_ROOT = "/data/latent-basemap/runs/round-0063"
RELEASE_ROOT = "/home/enjalot/code/latent-basemap-run"
ROUND_FILE = os.path.join(
    LAB_ROOT,
    "round-0063-2026-07-26.md",
)
SUBSTRATE_MANIFEST = (
...
GRAPH_MANIFEST = (
    "/data/latent-basemap/runs/round-0062/queue/artifacts/"
    "native-graph-balanced-60m/canonical-graph-v1.json"
)
...
def _require_issued_round() -> None:
    ...
    if statuses != ["issued"]:
        raise RuntimeError(
            f"R0063 requires one issued status; observed {statuses}"
        )


def prepare_round0063(
    *,
    release_sha: str,
    ...
    manifest["schema"] = "round0063-balanced-60m-train-queue-v1"
    manifest["repo_root"] = RELEASE_ROOT
    manifest["queue_class"] = "gpu-research"
    manifest["required_reviews"] = ["0046", "0049", "0062"]
    ...
    manifest["scientific_contract"] = {
        "purpose": (
            "train the balanced-60M scale rung for matched evaluation "
            "against the independent R0061 30M control"
        ),
        "rows": ROW_COUNT,
        ...
        "matched_30m_control": (
            "R0061 is an independent downstream comparison dependency, "
            "not a prerequisite for this training execution"
        ),
        "runtime_safety": {
    ...
    manifest["jobs"] = [{
        ...
        "production_config_receipt_schema": (
            "round0063-production-config-receipt-v1"
        ),
        "train_receipt_schema": "round0063-train-receipt-v1",
        "node_policy": {
            "gpu_required": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0063(
