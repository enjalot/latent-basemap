#!/usr/bin/env python3
"""Materialize, but never launch, the Round 0045 candidate-quality retry."""
...
ROUND_FILE = os.path.join(
    LAB_ROOT,
    "round-0045-2026-07-25.md",
)
ROUND_ROOT = "/data/latent-basemap/runs/round-0045"
R0044_ROOT = "/data/latent-basemap/runs/round-0044/queue"
...
def _require_issued_round() -> None:
    ...
    if statuses != ["issued"]:
        raise RuntimeError(
            f"R0045 requires one issued status; observed {statuses}"
        )


def prepare_round0045(
    *,
    release_sha: str,
    queue_root: str = os.path.join(ROUND_ROOT, "queue"),
) -> str:
    if not re.fullmatch(r"[0-9a-f]{40}", release_sha):
        raise ValueError("R0045 release SHA must be one full commit")
    _require_issued_round()
    queue_root = create_fresh_directory(
        queue_root,
        label="Round 0045 queue",
    )
    ...
    inputs = _dedupe(_file_inputs([
        ...
        os.path.join(LAB_ROOT, "result-0044-2026-07-25.md"),
        os.path.join(LAB_ROOT, "review-0044-2026-07-25.md"),
        os.path.join(R0044_ROOT, "runner-terminal.json"),
        os.path.join(
            R0044_ROOT,
            "artifacts",
            "candidate_quality_sweep_3m.failed.json",
        ),
        R0031_MEASUREMENT,
        INDEX_PATH,
    ...
    manifest["schema"] = "round0045-candidate-quality-retry-queue-v1"
    ...
    manifest["p90_gpu_seconds"] = {
        "candidate_quality_sweep_3m": 600.0,
        "total": 600.0,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        "queue_manifest": prepare_round0045(
