"""512-D real core resume twins and radius=1 control; tiny cyclic graph, no quality claim."""
import os
...
def main():
 ...
 with tempfile.TemporaryDirectory(dir=str(C.R.parent)) as td:
  ...
  for arm in C.ARMS:
   ...
   for tag,override in [('wrong_arm',{'arm':'wrong'}),('wrong_radius',{'radii_sha':'0'*64}),('wrong_input',{'input_manifest_sha':'0'*64})]:
    ...
    except AssertionError as exc:assert 'card049 admission-identity mismatch' in str(exc);checks.append(arm+' '+tag+' rejects before restore')
 ...
 result={'PASS':True,'checks':checks,'n_checks':len(checks),'endpoint_hashes':hashes,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'scope':'Actual512-D device core fit on512-row cyclic15 graph,18 successful updates. State twins, default-off control and specific identity failures; no quality claim.'};C.write(C.O/'card049-device-canary.json',result);print(result,flush=True)
