"""Card033 arm trainer (per card033-scale4m-conditional.md + root review). Two matched fresh 3D heads from the frozen
Card015 init on the 4M substrate: ordinary3d (baseline kernel) / actual3d (half-strength d²/sqrt(r_i·r_j), using square-root radius inputs).
n_components=3, constant LR .001, batch 16384, pos_ratio .10, rankneg 1000000 (25% of 4M), seed 42, EXACTLY
800000 successful updates. Inference snaps100/200/400/600/800K; GENUINE resumable checkpoints at100K step
intervals INCLUDING 800K + epoch boundaries, with the canonical immutable admission identity bound in.
...
LR/seed/dose (card033_validate.expected_identity); ALL loaded basemap modules verified UNDER this worktree;
...
only after strict validation. Usage: run_card033_arm.py <ordinary3d|actual3d> <STEPS>
"""
...
INIT3D = V.INIT3D; OUTD = SB / "card033-train"
...
def main():
    ...
    if (OUTD / f"manifest-{arm}.json").exists():
        ...
        print(f"[card033 {arm}] already complete + strict-valid; skip", flush=True); return 0

    dev = "cuda" if torch.cuda.is_available() else "cpu"
    assert dev == "cuda", "card033 arms are the production device path; refuse CPU fallback"
    ...
    assert V.full_sha(DATA / "edges-fixed15.npz") == V.EDGES_SHA256, "4M edges hash"
    assert V.full_sha(DATA / "substrate.f16.npy") == V.SUB_SHA256, "4M substrate hash"
    ...
    loaded = {"core": {"path": _core.__file__, "sha": V.full_sha(_core.__file__)[:16]},
              ...
              "run_card033_arm": {"path": __file__, "sha": V.full_sha(__file__)[:16]},
    ...
    assert pumap.fneg_weight == 1.0 and pumap.neg_tanh_gamma == 4.0 and pumap.positive_target_mode=="binary", "inherited negative recipe mismatch"
    ...
    if resume_from is None:
        adm = {"schema": "card033-admission-2026-09-12", "arm": arm, "written_before_steps": True, "n_components": 3,
               ...
               "checkpoint_every_epochs": 1, "resident_budget_gb": 14.0, "precision": "device_fp16", "card012_identity": identity,
    ...
    man = {"schema": "card033-arm-2026-09-12", "arm": arm, "n": int(n), "n_components": 3, "executed_steps": exec_steps,
    ...
    print(f"[card033 {arm}] steps={exec_steps} {wall:.0f}s 3D proc={proc_peak}GB global={global_used}GB "
