...
CALIB_SEED = 2202; N_BATCHES = 8; TARGET_FRAC = 0.10; MIN_FINITE = 5
...
def main():
    ...
    recs = list(p._calib_records)
    assert (getattr(p, "_pipeline_info", {}) or {}).get("x_residency") == "device_fp16", "pipeline not device_fp16"

    ratios = []
    for r in recs:
        gp, gs = r.get("grad_pairwise_l2"), r.get("grad_shape_l2")
        if gp is not None and gs is not None and math.isfinite(gp) and math.isfinite(gs) and gp > 0 and gs > 0:
            ratios.append(TARGET_FRAC * gp / gs)
    finite_ok = len(ratios) >= MIN_FINITE
    coeff = float(np.median(ratios)) if finite_ok else None
    stable = finite_ok and coeff is not None and math.isfinite(coeff) and coeff > 0
    R = {"schema": "card022-calibration-2026-09-12", "at": dt.datetime.now(dt.timezone.utc).isoformat(),
         ...
         "records": recs, "n_finite_positive_ratios": len(ratios), "min_required": MIN_FINITE,
         "coefficient": coeff, "wall_s": round(time.time() - t0, 1),
         "method": "coefficient = median(0.10 * ||grad_pairwise|| / ||grad_shape||) over 8 fixed actual-recipe "
                   "batches at the Card013 baseline endpoint; disjoint recorded calibration RNG (seed 2202); "
                   "no strength sweep; not tuned on evaluation scores.",
         "PASS": bool(stable)}
    if not stable:
        R["stop_reason"] = "required norm ratio undefined/unstable/nonfinite — stop and report; no strength sweep"
    (OC / "card022-calibration.json").write_text(json.dumps(R, indent=2))
    print(json.dumps({k: R[k] for k in ("coefficient", "n_finite_positive_ratios", "PASS")}, indent=1), flush=True)
