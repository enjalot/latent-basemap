...
def validate_arm(arm):
 ...
 model=torch.load(dest/'model.pt',map_location='cpu',weights_only=False);assert model['input_dim']==1536 and model['n_components']==8 and model['hidden_dim']==2048
 ...
 for step in C.SNAPS:
  snap=torch.load(dest/f'model-step{step}.pt',map_location='cpu',weights_only=False);ck=torch.load(dest/'ckpts'/f'ckpt-step{step}.pt',map_location='cpu',weights_only=False);assert ck['global_step']==step and ck['step_checkpoint'] is True;assert C.state_sha(snap['model_state_dict'])==C.state_sha(ck['model']);assert snap['n_components']==8 and snap['input_dim']==1536
