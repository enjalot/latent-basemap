...
CARD=sys.argv[1];assert CARD in ('086','088','089')
...
def run(instrument=True,plan='gradient_tail',resume=None,stop=None,observe=None,counter_fault=None,extra_clears=True):
 ...
 w=np.tile(np.r_[np.full(15,3.),np.ones(45)],n).astype('f4') if CARD=='088' else np.linspace(.01,1,len(src),dtype='f4')
 if CARD=='088':
...
def checks_main():
 ...
 for plan in ['gradient_tail','loss_tail']:
  full=run(plan=plan);plain=run(False,plan=plan);e=E.validate(full['stats']);rows=full['rows']
  ...
  checks[plan+'_exact_core_skip_tail_support']=True
  for key in full:
   if key!='stats':assert same(full[key],plain[key]),key
  checks[plan+'_offpath_model_optimizer_scaler_rng_ids']=True
  for split in [2,3,5]:
   partial=run(plan=plan,stop=split)
   with tempfile.TemporaryDirectory() as td:
    f=Path(td)/'state.pt';torch.save(partial,f);restored=torch.load(f,weights_only=False)
   result=run(plan=plan,resume=restored);assert same(full,result)
   checks[plan+'_fresh_observer_model_sampler_resume_'+str(split)]=True
  details[plan]=e
