"""Unmodified073/075 core fit with Card090 seed, identity and validation wrappers."""
...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,warm_path=None):
 dest=Path(dest);dest.mkdir(parents=True,exist_ok=True);start=time.monotonic();C.require_release();C.environment();assert torch.cuda.is_available();torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False;torch.set_float32_matmul_precision('highest')
 ...
 torch.set_num_threads(2);torch.manual_seed(C.seed(arm));torch.cuda.manual_seed_all(C.seed(arm));np.random.seed(C.seed(arm))
 ...
 p.fit(X,precomputed_edges_path=str(graph),random_state=C.seed(arm),verbose=False,warm_start_state=None if resume else warm,snapshot_steps=tuple(checkpoints or [dose]),snapshot_dir=str(dest),checkpoint_every_epochs=1,checkpoint_dir=str(dest/'ckpts'),resume_from=str(resume) if resume else None)
 ...
 assert p.rankneg_window==ident['rankneg_window'],'actual negative policy differs'
 if p.rankneg_window:assert p._rankneg_scale==(2*p.rankneg_window/len(X))**.75,'rank scale mismatch'
 else:assert p._rankneg_scale is None,'uniform scale mismatch'
 assert pi['sampler_class']=='DeviceEdgeSampler' and not pi['weighted_effective'] and not pi['positive_with_replacement'],'positive law mismatch'
...
def validate_arm(arm):
 ...
 model=torch.load(dest/'model.pt',map_location='cpu',weights_only=False);assert model['rankneg_window']==C.window(arm) and model['fneg_weight']==1. and model['neg_tanh_gamma']==4.
 ...
 ts=m['train_stats'];assert ts['attempted_batches']==ts['positive_lr_optimizer_steps']+ts['amp_overflow_skips']+ts['nonfinite_loss_skips']+ts['nonfinite_gradient_skips'],'attempt/skip accounting mismatch'
 assert ts['executed_iters']==ts['optimizer_steps_succeeded']==ts['positive_lr_optimizer_steps']==C.DOSE and ts['lr_used_min']==ts['lr_used_max']==ident['lr']
 assert m['lmc_stats'] is None
 ...
 return {'PASS':True,'runtime_sha':C.source_check(),'data_manifest_sha':C.sha(C.D/'inputs-manifest.json'),'arm':arm,'model_sha':m['model_sha'],'state_sha':m['state_sha'],'identity':ident,'checked_checkpoints':checks}
