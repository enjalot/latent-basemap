"""Card018 sequential GPU chain (per card018-scale-2m.md + root review). Lives INSIDE the isolated worktree.
Root owns the GPU queue and holds both leases; this chain runs inside them and does budget accounting only.

Stages: device_canary -> throughput_preflight (admission GATE) -> ordinary3d -> actual3d. Root-review repairs:
...
cap 18,000s, hard deadline 2026-09-13T01:52:44Z. Arms auto-resume from their latest checkpoint. Usage:
run_card018_chain.py
"""
...
CARD = OC / "card018-ledger.json"; WIN = OC / "cards-24h-window-ledger.json"
END = dt.datetime.fromisoformat("2026-09-13T01:52:44+00:00").timestamp()
CAP = 18000; PER_ARM_CAP = 8500; WIN_CAP = 86400; DOSE = V.DOSE
...
def notify(msg):
    subprocess.run([PY, str(OC / "notify.py"), "post", "codex-overseer", str(OC / "card018-execution.json"), msg], timeout=30)
    ...
    with (OC / "window-ledger-write.lock").open("a") as lk:
        ...
        for path, key in [(CARD, "batch_spent_s"), (WIN, "spent_s")]:
            ...
            v.setdefault("entries", []).append({"t": dt.datetime.now(dt.timezone.utc).isoformat(),
                "event": "gpu_stage", "card": "018", "tag": tag, "wall_s": seconds, "rc": rc,
...
def _completed_steps(arm):
    cdir = OC.parent / "card018-train" / arm / "ckpts"
    if not cdir.is_dir(): return 0
    from run_card018_arm import _latest_ckpt
    return _latest_ckpt(cdir)[1]


def _preflight_per_step():
    pf = json.loads((OC / "card018-preflight.json").read_text())
...
def main():
    if not CARD.exists():
        atomic(CARD, {"schema": "card018-ledger", "batch_cap_s": CAP, "batch_spent_s": 0, "entries": []})
    ...
    rc = run_stage("device_canary", "gpu_card018_canary.py", [], to)
    assert rc == 0, f"device_canary rc={rc}"
    assert json.loads((OC / "card018-canary.json").read_text())["PASS"], "resume/identity canary FAILED"
    ...
    rc = run_stage("throughput_preflight", "gpu_card018_preflight.py", [], to)
    if rc == 124:
        atomic(OC / "card018-execution.json", {"status": "PREFLIGHT_TIMEOUT", "at": dt.datetime.now(dt.timezone.utc).isoformat()})
        notify("Card018 preflight timed out; admission halted."); return
    pf = json.loads((OC / "card018-preflight.json").read_text())
    if rc == 3 or not pf.get("PASS"):
        atomic(OC / "card018-execution.json", {"status": "PREFLIGHT_STOP", "preflight": pf,
               "at": dt.datetime.now(dt.timezone.utc).isoformat(),
               "note": "Measured cost does not fit the 18,000s cap/deadline. Admission stopped; dose NOT truncated."})
        notify("Card018 preflight STOP: both 400K arms do not fit the cap/deadline. Admission halted, dose not truncated.")
        return
    ...
    for arm in ARMS:
        if (OC.parent/"card018-train"/f"manifest-{arm}.json").exists():
            completed.append(V.strict_validate_arm(arm, ROOT))
            atomic(OC / "card018-completion-validation.json", {"completed": completed, "both_valid": len(completed) == 2})
        ...
        rc = run_stage(arm, "run_card018_arm.py", [arm, str(DOSE)], to)
        if rc == 124:
            atomic(OC / "card018-execution.json", {"status": "ARM_TIMEOUT_CHECKPOINTED", "arm": arm,
                   "at": dt.datetime.now(dt.timezone.utc).isoformat(),
                   "note": "checkpoint preserved; re-queue resumes from latest ckpt (cumulative 8500s/arm; no truncation)."})
            notify(f"Card018 {arm} hit its budget with checkpoint preserved; re-queue resumes (cumulative per-arm cap). No truncation.")
            return
    ...
    atomic(OC / "card018-execution.json", {"status": "TRAINED_VALIDATED", "at": dt.datetime.now(dt.timezone.utc).isoformat(),
           "arms": completed, "quality": "NOT_YET_SCORED"})
    notify("Card018 both 3D arms (ordinary3d/actual3d) trained + strict-validated at 2M/400K: exact positive-LR "
           "dose, device_fp16, loadable snapshots + resumable step ckpts (incl 400K) with bound identity, isolated "
           "runtime under worktree, global VRAM<30GB. CPU scoring (score_card018.py) next.")
...
if __name__ == "__main__":
    try:
        ...
        atomic(OC / "card018-execution.json", {"status": "EXECUTION_FAILED", "error": repr(e), "at": dt.datetime.now(dt.timezone.utc).isoformat()})
        notify(f"Card018 chain stopped fail-closed: {e}. Preserve artifacts + reconcile ledger before repair.")
        raise
