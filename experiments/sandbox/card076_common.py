...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card076-finishing');TD=R.parent/'card076-train';CHAMP=R.parent/'card033-train/model-actual3d.pt'
ARMS=['uniform'];N=4000000;DIM=1536;DOSE=40000;BATCH=16384;SEED=42;LR={'uniform':.0001};SNAPS=[10000,20000,40000];PREP_STEPS=2000;PREP_BATCH=2048;PREP_SEED=65068
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card076-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card076 runtime changed';return sha(R/'card076-runtime-sha.json')
def input_check():
