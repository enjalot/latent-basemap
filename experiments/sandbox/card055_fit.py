"""Same core fit entry point for049 production, device canary and cost preflight."""
...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,unit_control=False):
 dest=Path(dest);dest.mkdir(parents=True,exist_ok=True);start=time.monotonic();assert torch.cuda.is_available()
 ...
 if radius_path is None and C.RAD[arm]:radius_path=C.D/C.RAD[arm]
 if radii is None and radius_path is not None:radii=np.load(radius_path).astype('f4')
 if radii is not None:assert len(radii)==len(X) and np.isfinite(radii).all() and (radii>0).all()
 ident=C.identity(arm,dose,len(X),graph,radius_path)
 actual_radius_sha=hashlib.sha256(np.ascontiguousarray(radii,dtype='f4').tobytes()).hexdigest() if radii is not None else None
 if unit_control:
  assert arm=='ordinary3d' and len(X)==512 and radius_path is None and np.array_equal(radii,np.ones(512,'f4'))
  ident.update(unit_radius_control=True,radii_values_sha=actual_radius_sha)
 else:assert actual_radius_sha==ident['radii_values_sha'],'actual radius array differs from identity'
 ...
 p=ParametricUMAP.load(str(C.CHAMP),device='cuda');C.configure(p,ident,radii,checkpoints or [dose]);warm=torch.load(C.D/'init.pt',map_location='cpu',weights_only=False)['model_state']
 assert C.state_sha(warm)==C.read(C.D/'inputs-manifest.json')['init_state_sha256'];expected_warm=hashlib.sha256(b''.join(np.ascontiguousarray(t.numpy()).tobytes() for t in warm.values())).hexdigest()[:16]
 ...
 ts=dict(p._train_stats);pi=dict(p._pipeline_info);assert ts['executed_iters']==ts['positive_lr_optimizer_steps']==dose;assert ts['lr_used_min']==ts['lr_used_max']==.001 and pi['x_residency']=='device_fp16'
...
def validate_arm(arm):
 ...
 model=torch.load(dest/'model.pt',map_location='cpu',weights_only=False);assert model['input_dim']==512 and model['n_components']==3 and model['hidden_dim']==2048
 assert C.state_sha(model['model_state_dict'])==m['state_sha'];assert m['train_stats']['positive_lr_optimizer_steps']==60000 and m['pipeline_info']['x_residency']=='device_fp16'
 ...
 ts=m['train_stats'];assert ts['executed_iters']==ts['optimizer_steps_succeeded']==ts['positive_lr_optimizer_steps']==C.DOSE and ts['lr_used_min']==ts['lr_used_max']==C.LR
 ...
 for step in C.SNAPS:
  snap=torch.load(dest/f'model-step{step}.pt',map_location='cpu',weights_only=False);ck=torch.load(dest/'ckpts'/f'ckpt-step{step}.pt',map_location='cpu',weights_only=False);assert ck['global_step']==step and ck['step_checkpoint'] is True;assert C.state_sha(snap['model_state_dict'])==C.state_sha(ck['model']);assert snap['n_components']==3 and snap['input_dim']==512
  states.append(C.state_sha(ck['model']))
  if step==60000:assert C.state_sha(snap['model_state_dict'])==m['state_sha']
