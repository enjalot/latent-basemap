...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card073-finishing');TD=R.parent/'card081-train';CHAMP=R.parent/'card023-train/model-actual3d.pt'
ARMS=['uniform','degree','shuffled'];N=2000000;DIM=1536;DOSE=60000;BATCH=16384;SEED=42;LR={a:.0001 for a in ARMS};SNAPS=[20000,40000,60000];PREP_STEPS=2000;PREP_BATCH=2048;PREP_SEED=65068
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card081-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card081 runtime changed';return sha(R/'card081-runtime-sha.json')
def input_check():
...
def identity(arm,dose=DOSE,nodes=N,graph=None,radii_path=None,warm_path=None,noise_q_path=None):
 ...
 return {'card':'081','arm':arm,'phase':'graph','output_dim':3,'initialization_sha':sha(TD/arm/'preparation.json'),'parent_sha':read(TD/arm/'preparation.json')['parent_sha'],'n_nodes':nodes,'dose':dose,'lr':LR[arm],'lr_schedule':'constant','batch_size':BATCH,'pos_ratio':.1,'seed':SEED,'warm_sha':sha(wp),'original_init_sha':sha(CHAMP),'scale_manifest_sha':sha(O/'card023-data-manifest.json'),'input_manifest_sha':sha(D/'inputs-manifest.json'),'graph_sha':sha(graph),'radii_sha':sha(rp),'radii_values_sha':hashlib.sha256(np.ascontiguousarray(np.load(rp),dtype='f4').tobytes()).hexdigest(),'runtime_manifest_sha':source_check(),'rankneg_window':0,'negative_policy':'degree_conditional_cdf','noise_q_path':str(noise_q_path or O/'degree-noise-readiness'/({'degree':'q.npy','shuffled':'q-shuffled.npy','uniform':'uniform.npy'}[arm])),'noise_q_sha':sha(noise_q_path or O/'degree-noise-readiness'/({'degree':'q.npy','shuffled':'q-shuffled.npy','uniform':'uniform.npy'}[arm])),'base_negative_weight':1.0,'precision':'device_fp16','gpu_resident_vram_budget_gb':14.,'fneg_weight':1.,'neg_tanh_gamma':4.,'positive_target_mode':'binary','replay_weight':0.,'lmc_weight':0.,'weight_decay':.01,'grad_clip':1.,'all_auxiliary_losses_off':True}
