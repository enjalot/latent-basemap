...
def main():
 ...
 for hi,name in enumerate(C.HEADS):
  m,c,radius=C.load(name,'cuda');stored=np.load(O/f'card059-scoring/{name}-coords.npz');exposed=np.load(O/f'card057-inputs/{name}-normalized.f32.npy');probe=C.project(m,exposed[:256],c)['xy'];assert np.array_equal(probe,stored['target_direction_constant_norm'][:256]),'isolated runtime exposed canary'
