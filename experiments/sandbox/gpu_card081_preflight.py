...
def main():
 ...
 with tempfile.TemporaryDirectory(dir=str(C.R.parent),prefix='card081-preflight-') as td:
  for arm in C.ARMS:
   fits[arm]={}
   ...
   checks[arm+'_time']=est[arm]['complete_arm_s']<=1800;checks[arm+'_vram']=max(x['global_vram_GiB'] for x in fits[arm].values())<30
 spent=C.read(C.O/'card081-ledger.json')['batch_spent_s'];win=C.read(C.O/'cards-24h-window-ledger.json')['spent_s'];used=time.monotonic()-start;need=sum(v['complete_arm_s'] for v in est.values())+120
 checks.update(card_cap=spent+used+need<=6000,window_cap=win+used+need<=165491,deadline=need<=END-time.time())
 r={'PASS':bool(all(checks.values())),'checks':{k:bool(v) for k,v in checks.items()},'fits':fits,'estimates':est,'fits_plus_reserve_s':need,'preflight_elapsed_s':used,'runtime_sha':C.source_check(),'scope':'Actual1536-D2M three-arm500/3500 successful-update fits; production epoch18316steps not reached in timing; full real checkpoint payload serialization separately measured and reserved, conservative repeated serialization, unchanged60K doses.'};C.write(C.O/'card081-preflight.json',r);print(r,flush=True);return 0 if r['PASS'] else 3
