...
import json,hashlib
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card054-landmark');TD=R.parent/'card060-train';CHAMP=R.parent/'card015-train/model-actual_full3d.pt'
WEIGHTS={'ordinary':0.,'medium':.1,'strong':.5};ARMS=list(WEIGHTS);TREATMENTS=['medium','strong'];N=300000;DIM=1536;NC=3;DOSE=30000;BATCH=16384;SEED=42;LR=.0001;SNAPS=[5000,15000,30000]
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card060-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'runtime source changed';return sha(R/'card060-runtime-sha.json')
def input_check():
