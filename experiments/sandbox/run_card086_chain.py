...
def verify():
    release=C.O/'card086-release.json'
    assert release.exists(),'NO ROOT GPU RELEASE'
    r=C.read(release)
    assert r['PASS'] and r['card']=='086' and r['limits']==B.LIMITS
...
def stage(tag,script,cap,args=(),arm=None,receipt_path=None):
    ...
    try:
        env=dict(os.environ,CARD086_RELEASE_SHA=release)
        ...
        C.write(C.O/f'card086-stage-{tag}.json',{'PASS':rc==0 and stage_error is None,'rc':rc,'error':stage_error,'receipt_proof':proof,'stage_started_ns':started_ns,'wall_s':elapsed,'peak_tree_rss_bytes':peak_rss,'peak_global_vram_gib':peak_vram,'runtime_sha':C.source_check()})
    verify()
    return elapsed
...
def main():
    ...
    try:
        ...
        for count in ['500','3500']:
            left=STAGE_RECEIPTS['preflight-all_one']['receipt']['fits'][count]['sampler']['actual_first_batch']
