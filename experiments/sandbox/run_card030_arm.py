"""Matched fresh full-D300K heads, fixed initial weights and final-activation intervention."""
...
DATA=Path('/data/latent-basemap/substrates/card030-fresh');OUT=SB/'card030-train'
HEAD=SB/'dino-arrival-t0/champion-bs16k/model.pt';ARMS=['relu','leaky'];ACT={'relu':'relu','leaky':'leaky_relu_slope_0p01'};SNAPS=[20000,40000,60000]
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def fit(arm,steps=60000,canary=False,resume_from=None,ckpt_steps=None,identity_override=None):
 ...
 init=torch.load(DATA/'init-card010.pt',map_location='cpu',weights_only=False)['model_state']
 assert sha(DATA/'init-card010.pt')=='7d313c265cb659c954951d79ec395fa4d267f0f34a0bfb6aafe0dd23d5985825'
 expected_warm=hashlib.sha256(b''.join(v.contiguous().numpy().tobytes() for v in init.values())).hexdigest()[:16]
 assert expected_warm=='589895f037d406ae'
 identity={'card':'030','arm':arm,'activation':ACT[arm],'negative_slope':.01 if arm=='leaky' else 0.,'steps':steps,'lr':.001,'lr_schedule':'constant','batch_size':16384,'rankneg_window':75000,'seed':42,'head_sha':sha(HEAD),'init_file_sha':sha(DATA/'init-card010.pt'),'warm_named_sha':state_sha(init),'expected_warm_parameter_sha':expected_warm,'data_manifest_sha':sha(DATA/'manifest.json'),'runtime':rt,'canary':canary}
 ...
 p.learning_rate=.001;p.lr_schedule='constant';p.warmup_steps=0;p.n_epochs=10000;p._max_train_steps=steps;p.batch_size=16384;p.rankneg_window=75000;p.x_residency='auto';p.required_input_pipeline='device'
 ...
 from card030_resume import validate_recipe
 recipe=validate_recipe(p);identity['recipe']=recipe
 if not canary:
  ...
  if resume_from:assert json.loads((dest/'admission.json').read_text())==identity
   ...
   assert not (dest/'admission.json').exists(),'existing incomplete admission requires explicit checkpoint resume'
 ...
 p.fit(X,precomputed_edges_path=str(DATA/'edges-fixed15.npz'),random_state=42,verbose=False,warm_start_state=init,
       checkpoint_every_epochs=1,checkpoint_dir=str(dest/'ckpt'),resume_from=resume_from,
 ...
 assert st['lr_used_min']==st['lr_used_max']==.001 and p._pipeline_info['x_residency']=='device_fp16'
 ...
 # Separate warmed single-forward deployment-style timing, no training mutation.
 tf32=torch.backends.cuda.matmul.allow_tf32;torch.backends.cuda.matmul.allow_tf32=False
 with torch.inference_mode():
  q=torch.nn.functional.normalize(torch.as_tensor(X[:8192],device='cuda'),dim=1);_=p.model(q).cpu().numpy();torch.cuda.synchronize();bt=time.monotonic()
  for s in range(0,100000,8192):
   q=torch.nn.functional.normalize(torch.as_tensor(X[s:min(s+8192,100000)],device='cuda'),dim=1);y=p.model(q).cpu().numpy();assert np.isfinite(y).all()
  torch.cuda.synchronize();bw=time.monotonic()-bt
 torch.backends.cuda.matmul.allow_tf32=tf32
 result['projection_benchmark']={'rows':100000,'wall_s':bw,'rows_per_s':100000/bw,'precision':'FP32,TF32disabled','input_contract':'host training FP16 castFP32, renormalize onGPU, singlehead, CPUcopy','limits':'Warm host inputs; raw disk and output writes excluded. Not observed full103.8M throughput.'}
 assert state_sha(p.model.state_dict())==result['endpoint_named_sha']
...
if __name__=='__main__':
 import os
 fit(sys.argv[1],resume_from=os.environ.get('CARD030_RESUME') or None)
