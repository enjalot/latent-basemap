"""Card018 arm trainer (per card018-scale-2m.md + root review). Two matched fresh 3D heads from the frozen
...
LR/seed/dose (card018_validate.expected_identity); ALL loaded basemap modules verified UNDER this worktree;
...
only after strict validation. Usage: run_card018_arm.py <ordinary3d|actual3d> <STEPS>
"""
...
INIT3D = V.INIT3D; OUTD = SB / "card018-train"
...
def main():
    ...
    if (OUTD / f"manifest-{arm}.json").exists():
        ...
        print(f"[card018 {arm}] already complete + strict-valid; skip", flush=True); return 0

    dev = "cuda" if torch.cuda.is_available() else "cpu"
    assert dev == "cuda", "card018 arms are the production device path; refuse CPU fallback"
    ...
    loaded = {"core": {"path": _core.__file__, "sha": V.full_sha(_core.__file__)[:16]},
              ...
              "run_card018_arm": {"path": __file__, "sha": V.full_sha(__file__)[:16]},
    ...
    if resume_from is None:
        adm = {"schema": "card018-admission-2026-09-12", "arm": arm, "written_before_steps": True, "n_components": 3,
    ...
    man = {"schema": "card018-arm-2026-09-12", "arm": arm, "n": int(n), "n_components": 3, "executed_steps": exec_steps,
    ...
    print(f"[card018 {arm}] steps={exec_steps} {wall:.0f}s 3D proc={proc_peak}GB global={global_used}GB "
