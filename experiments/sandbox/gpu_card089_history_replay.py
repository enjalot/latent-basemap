...
def main():
 C.require_release();start=time.monotonic();base=C.R.parent/'card086-train/all_one';assert (base/'validation.json').exists(),'baseline incomplete STOP';v=C.read(base/'validation.json');assert v['PASS'] and v['model_sha']==C.sha(base/'model.pt'),'baseline validation STOP'
