...
L=C.O/'card090-ledger.json';W=C.O/'cards-24h-window-ledger.json'
J=C.O/'card090-accounting-journal'
END=dt.datetime.fromisoformat('2026-09-13T23:50:55+00:00').timestamp()
LIMITS={'card_gpu_s':6500,'stage_gpu_s':dict.fromkeys(C.ARMS,1400),'shared_s':900,'cpu_wall_reserve_s':1800,'global_vram_gib':30,'rss_gib':32,'owner_aggregate_rss_gib':48,'root_scorer_rss_gib':16,'deadline':'2026-09-13T23:50:55Z'}
...
def _apply(event):
    ...
    for path,key in ((W,'spent_s'),(L,'batch_spent_s')):
        ...
        if path==L:
            ...
            r['shared_spent_s']+=event['shared_s']
        C.write(path,r)
...
def _event(tag,seconds,arm,kind):
    event={'transaction':uuid.uuid4().hex,'at':dt.datetime.now(dt.timezone.utc).isoformat(),
           'shared_s':seconds if arm is None else 0.,'card':'090','tag':tag,'event':kind,'wall_s':seconds,'arm_s':allocation(seconds,arm)}
...
def transact(tag,seconds,arm=None,kind='reservation',check=False):
    with (C.O/'window-ledger-write.lock').open('a') as f:
        ...
        if not L.exists():C.write(L,{'batch_cap_s':6500,'batch_spent_s':0.,'shared_spent_s':0.,'arm_spent_s':{a:0. for a in C.ARMS},'entries':[]})
        assert W.exists(),'root rolling ledger required'
        _recover()
        if check:
            l=C.read(L);w=C.read(W)
            assert l['batch_spent_s']+seconds<=6500 and w['spent_s']+seconds<=165491,'cumulative budget STOP'
            if arm is None:assert l['shared_spent_s']+seconds<=LIMITS['shared_s'],'shared cumulative STOP'
...
def available(arm=None):
    import time
    l=C.read(L);w=C.read(W)
    remaining=[6500-l['batch_spent_s'],165491-w['spent_s'],END-time.time()-LIMITS['cpu_wall_reserve_s']]
    if arm is None:remaining.append(LIMITS['shared_s']-l['shared_spent_s'])
