...
for arm in C.ARMS:
 ...
 if (dest/'preparation.json').exists():
  r=C.read(dest/'preparation.json');assert r['runtime_sha']==C.source_check() and r['parent_sha']==C.sha(C.CHAMP) and r['prepared_sha']==C.sha(dest/'prepared.pt');receipts[arm]=r;continue
...
receipts=C.validate_preparations();assert len({r['prepared_state_sha'] for r in receipts.values()})==1;C.write(C.O/'card086-cdf-repair-initialization.json',{'PASS':True,'data_manifest_sha':C.sha(C.GD/'manifest.json'),'arms':receipts,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check()});print('MATCHED INITIALIZATION PASS')
