...
receipts=C.validate_preparations();assert len({r['prepared_state_sha'] for r in receipts.values()})==1;C.write(C.O/'card086-reciprocal-initialization.json',{'PASS':True,'data_manifest_sha':C.sha(C.GD/'manifest.json'),'arms':receipts,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check()});print('MATCHED INITIALIZATION PASS')
