...
def main():
 C.require_release();ap=argparse.ArgumentParser();ap.add_argument('runtime',choices=['original','proposed']);ap.add_argument('mode',choices=['fresh','epoch']);ap.add_argument('dest');a=ap.parse_args();dest=Path(a.dest);dest.mkdir(parents=True)
 ...
 if a.mode=='epoch':
  epochs=sorted((hist/'ckpts').glob('ckpt-epoch*.pt'));assert epochs,'baseline has no retained epoch'
  resume=epochs[0];ck=torch.load(resume,map_location='cpu',weights_only=False);assert not ck['step_checkpoint'];step=ck['global_step'];del ck
 ...
 if a.runtime=='proposed':
  ...
  if resume:
   ...
   extra=nullcontext()
  else:extra=observe(p,'reciprocal',mask)
 ...
 end=dest/'ckpts'/f'ckpt-step{dose}.pt';C.write(dest/'receipt.json',{'PASS':True,'runtime':a.runtime,'mode':a.mode,'core_path':str(core.__file__),'core_sha':C.sha(core.__file__),'endpoint':str(end),'epoch_source':str(resume) if resume else None,'epoch_source_sha':C.sha(resume) if resume else None,'end_step':dose,'telemetry_scope':'New telemetry enabled for fresh8; historical retained-epoch replay checks unchanged numerical trainer/configuration.089 telemetry resume proven separately on089 states.'})
