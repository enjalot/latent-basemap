"""Card033 canonical strict validator + identity (per root review groups 2/3). ONE source of truth reused by
...
- strict_validate_arm: full completion validation (exact dose, mandatory/forbidden radius by arm, warm
  provenance, precision, finite endpoints, endpoint==800K snapshot, snapshot progress+freshness vs ORIGINAL
...
DATA = Path("/data/latent-basemap/substrates/card033-scale4m")
INIT3D = SB / "card015-init/init-card015-3d.pt"
DATA_MANIFEST = OC / "card033-data-manifest.json"
TD_DEFAULT = SB / "card033-train"
# FULL content hashes (from the admitted card033 data manifest; the trainer re-verifies the on-disk files)
SUB_SHA256 = "acb4cdf3464b101963384f8eea3f7d716964f2e3b386c41f686d48851c885ddc"
_GRAPH = json.loads((DATA / "manifest.json").read_text()) if (DATA / "manifest.json").exists() else {}
EDGES_SHA256 = _GRAPH.get("files", {}).get("edges-fixed15.npz")
R_ACTUAL_SHA256 = _GRAPH.get("files", {}).get("r_actual.npy")
DRAW_IDS_SHA256 = "f2842f20b88ff508c64d878b9f0612446aa52606056bfaa52a351d46eebe5046"
INIT_NAMED_SHA = "5544a31160054bcc"; WARM_PARAM_SHA = "b261492f84aa24bd"
N = 4000000; DOSE = 800000; RANKNEG = 1000000; BATCH = 16384; POS_RATIO = 0.10; SEED = 42; LR = 0.001
SNAP_STEPS = [100000, 200000, 400000, 600000, 800000]
STEP_CKPTS = list(range(100000, 800001, 100000))  # 800K resumable too
ARM_RADII = {"ordinary3d": None, "actual3d": "r_actual.npy"}
KERNEL = {"ordinary3d": "baseline umap 3D", "actual3d": "d2/sqrt(r_i*r_j) via half-radius inputs, detached 3D"}
...
def runtime_manifest_path(ROOT): return Path(ROOT) / "card033-runtime-sha.json"
...
def expected_identity(arm, ROOT):
    ...
    assert _GRAPH.get("PASS") and EDGES_SHA256 and R_ACTUAL_SHA256, "graph not admitted"
    radii_file = (str(DATA / ARM_RADII[arm]) if ARM_RADII[arm] else None)
    radii_sha = (R_ACTUAL_SHA256 if arm == "actual3d" else None)
    return {
        "card": "card033", "arm": arm, "kernel": KERNEL[arm],
        "base_config_artifact_sha": full_sha(SB/"dino-arrival-t0/champion-bs16k/model.pt"),
        "n_nodes":N,"n_components":3,"positive_target_mode":"binary","fneg_weight":1.0,"neg_tanh_gamma":4.0,
        ...
        "gpu_resident_vram_budget_gb": 14.0, "precision": "device_fp16", "x_residency": "auto", "required_input_pipeline": "device",
    }
...
def strict_validate_arm(arm, ROOT, base=None, recompute_substrate=False):
    ...
    independent data audit (CPU-cheap for edges/radii; hashes the 24GB host substrate too)."""
    import torch, numpy as np
    ...
    assert snap_sha[800000] == ep_sha, f"{arm}: endpoint != 800K snapshot"
