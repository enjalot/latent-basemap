"""Same core fit entry point for053 production, device canary and cost preflight."""
...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,unit_control=False):
 ...
 assert not unit_control;assert actual_radius_sha==ident['radii_values_sha'],'actual radius array differs from identity'
 ...
 ts=dict(p._train_stats);pi=dict(p._pipeline_info);assert ts['executed_iters']==ts['positive_lr_optimizer_steps']==dose;assert ts['lr_used_min']==ts['lr_used_max']==C.LR and pi['x_residency']=='device_fp16'
...
def validate_arm(arm):
 ...
 model=torch.load(dest/'model.pt',map_location='cpu',weights_only=False);assert model['input_dim']==C.DIM and model['n_components']==3 and model['hidden_dim']==2048
 ...
 for step in C.SNAPS:
  snap=torch.load(dest/f'model-step{step}.pt',map_location='cpu',weights_only=False);ck=torch.load(dest/'ckpts'/f'ckpt-step{step}.pt',map_location='cpu',weights_only=False);assert ck['global_step']==step and ck['step_checkpoint'] is True;assert C.state_sha(snap['model_state_dict'])==C.state_sha(ck['model']);assert snap['n_components']==3 and snap['input_dim']==C.DIM
