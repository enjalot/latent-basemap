...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card073-finishing');TD=R.parent/'card086-reciprocal-train';CHAMP=R.parent/'card023-train/model-actual3d.pt'
CD=Path('/data/latent-basemap/substrates/card086-reciprocal-cdf-v1')
...
def identity(arm,dose=DOSE,nodes=N,graph=None,radii_path=None,warm_path=None,cdf_values_sha=None,cdf_reciprocal_hex=None):
...
def validate_ckpt(ck,ident):
 import torch
 import card086_resume as V
 from card086_exposure import validate as validate_exposure
...
def assert_identity(actual,expected):
 assert set(actual)==set(expected),'card086 identity mismatch: keys'
 for k in sorted(expected):assert actual[k]==expected[k],'card086 identity mismatch: '+k

def require_release():
 import os
 p=O/'card086-release.json'
 assert p.is_file(), 'NO ROOT GPU RELEASE'
 r=read(p)
 assert r.get('PASS') is True and r.get('card')=='086', 'invalid root release'
