...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card073-finishing');TD=R.parent/'card089-train';CHAMP=R.parent/'card023-train/model-actual3d.pt'
GD=Path('/data/latent-basemap/substrates/card089-soft-reciprocity');ARMS=['reciprocal','rank_count_control'];N=2000000;DIM=1536;DOSE=60000;BATCH=16384;SEED=42;LR={a:.0001 for a in ARMS};SNAPS=[20000,40000,60000];PREP_STEPS=2000;PREP_BATCH=2048;PREP_SEED=65068
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card089-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card089 runtime changed';return sha(R/'card089-runtime-sha.json')
def input_check():
 m=read(D/'inputs-manifest.json');assert m['status']=='MATCHED_FINISH_READY_NO_DATA_CHANGED' and m['n']==N and m['dim']==DIM
 assert all(sha(D/n)==h for n,h in m['files'].items());g=read(GD/'manifest.json');assert g['PASS'] and all(sha(GD/n)==h for n,h in g['files'].items()),'card089 data artifact hash mismatch';assert all(sha(p)==h for p,h in g['inputs'].items()),'card089 data input hash mismatch';assert sha(R/'experiments/sandbox/card089_weights.py')==g['weight_law_sha'],'card089 weight source mismatch';return m
def loaded_modules():
 out={}
...
def identity(arm,dose=DOSE,nodes=N,graph=None,radii_path=None,warm_path=None,mutual_mask=None):
 if mutual_mask is None:mutual_mask=np.load(O/'reciprocity-readiness-20260913/mutual-mask.npy',mmap_mode='r')
 assert mutual_mask.shape==(nodes,15) and mutual_mask.dtype==bool,'actual mutual mask shape'
 ...
 return {'card':'089','arm':arm,'phase':'graph','exposure_schema':'card089-pair-exposure-v1','weighted_edge_sampling':True,'uniform_with_replacement':False,'data_manifest_sha':sha(GD/'manifest.json'),'weight_values_sha':weight_sha(graph),'endpoints_sha':endpoints_sha(graph),'protocol_sha':sha(O/'card089-soft-reciprocity.md'),'quality_prereg_sha':sha(O/'card089-quality-prereg.md'),'reference_sha':read(GD/'manifest.json')['weight_law_sha'],'mutual_mask_sha':sha(O/'reciprocity-readiness-20260913/mutual-mask.npy'),'actual_mutual_mask_values_sha':hashlib.sha256(np.ascontiguousarray(mutual_mask).tobytes()).hexdigest(),'raw_ratio':4,'row_normalization':'FP64_to_row15_then_FP32','favored_policy':arm,'output_dim':3,'initialization_sha':sha(TD/arm/'preparation.json'),'parent_sha':read(TD/arm/'preparation.json')['parent_sha'],'n_nodes':nodes,'dose':dose,'lr':LR[arm],'lr_schedule':'constant','batch_size':BATCH,'pos_ratio':.1,'seed':SEED,'warm_sha':sha(wp),'original_init_sha':sha(CHAMP),'scale_manifest_sha':sha(O/'card023-data-manifest.json'),'input_manifest_sha':sha(D/'inputs-manifest.json'),'graph_sha':sha(graph),'radii_sha':sha(rp),'radii_values_sha':hashlib.sha256(np.ascontiguousarray(np.load(rp),dtype='f4').tobytes()).hexdigest(),'runtime_manifest_sha':source_check(),'rankneg_window':0,'negative_policy':'uniform_nonself_rank_scaling_off','precision':'device_fp16','gpu_resident_vram_budget_gb':14.,'fneg_weight':1.,'neg_tanh_gamma':4.,'positive_target_mode':'binary','replay_weight':0.,'lmc_weight':0.,'weight_decay':.01,'grad_clip':1.,'all_auxiliary_losses_off':True}
...
def validate_ckpt(ck,ident):
 import torch
 import card089_resume as V
 from card089_exposure import validate as validate_exposure
...
def assert_identity(actual,expected):
 assert set(actual)==set(expected),'card089 identity mismatch: keys'
 for k in sorted(expected):assert actual[k]==expected[k],'card089 identity mismatch: '+k

def require_release():
 import os
 p=O/'card089-release.json'
 assert p.is_file(), 'NO ROOT GPU RELEASE'
 r=read(p)
 assert r.get('PASS') is True and r.get('card')=='089', 'invalid root release'
 assert r['runtime_sha']==source_check(), 'release runtime mismatch'
 assert os.environ.get('CARD089_RELEASE_SHA')==sha(p), 'missing root controller release binding'
