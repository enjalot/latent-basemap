...
import json,hashlib
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card052-global-pca');TD=R.parent/'card052-train';CHAMP=R.parent/'card015-train/model-actual_full3d.pt'
ARMS=['ordinary','weak_PCA','strong_PCA'];N=300000;DIM=1536;NC=3;DOSE=60000;BATCH=16384;SEED=42;LR=.0001;SNAPS=[20000,40000,60000]
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card052-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'runtime source changed';return sha(R/'card052-runtime-sha.json')
def input_check():
...
def configure(p,ident,radii,checkpoints,bank_path):
 ...
 assert not getattr(p,'_card011_schedule',None);p.replay_bank_path=str(bank_path) if ident['replay_weight'] else '';p.replay_weight=ident['replay_weight'];p.replay_fraction=.05;p.replay_seed=52052;p.clip_grad_norm=1.;p._card013_radii=radii;p._card012_identity=ident;p._checkpoint_step_targets=set(checkpoints)
def validate_ckpt(ck,ident):
 import torch
 import card052_resume as V
 assert ck['card012_identity']==ident,'card052 admission-identity mismatch';step=ck['global_step'];assert isinstance(step,int) and 0<step<=ident['dose'];V.validate_resume_payload(ck,step,n_nodes=ident['n_nodes']);assert ck['config']['learning_rate']==LR and ck['config']['lr_schedule']=='constant' and ck['config']['replay_weight']==ident['replay_weight'];assert ck['config']['replay_enabled']==bool(ident['replay_weight'])
