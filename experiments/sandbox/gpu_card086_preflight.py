...
def main():
 ...
 checks={'finite_complete_estimate':math.isfinite(estimate) and estimate>0,'vram':all(v['global_vram_GiB']<30 for v in fits.values()),'cdf_lost_mass':all(v['sampler']['cdf']['positive_zero_width_probability']<=release['cdf_lost_probability_cap'] for v in fits.values()),'successful_doses':all(v['positive_updates']==int(k) for k,v in fits.items())}
 out={'PASS':all(checks.values()),'admission_status':'MEASUREMENT_ONLY_CHAIN_MUST_CHECK_SETTLED_CUMULATIVE_SPEND','ledger_during_active_reservation':C.read(B.L),'checks':checks,'estimate':{'complete_arm_s':estimate,'per_step_s':slope,'setup_s':setup,'reserve_s':reserve,'epoch_construction_s_max':epoch_cost,'reserved_epoch_constructions':n_epochs},'fits':fits,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'data_manifest_sha':C.sha(C.GD/'manifest.json')};C.write(C.O/f'card086-preflight-{arm}.json',out);assert out['PASS'],'weighted full-dose preflight STOP'
