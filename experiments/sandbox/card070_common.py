...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card065-initialization');TD=R.parent/'card070-train';CHAMP=R.parent/'card015-train/model-actual_full3d.pt'
ARMS=['high'];N=300000;DIM=1536;DOSE=60000;BATCH=16384;SEED=42;LR=.001;SNAPS=[20000,40000,60000];PREP_STEPS=2000;PREP_BATCH=2048;PREP_SEED=65068
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card070-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card070 runtime changed';return sha(R/'card070-runtime-sha.json')
def input_check():
