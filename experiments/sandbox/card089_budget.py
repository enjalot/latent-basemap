...
L=C.O/'card089-ledger.json';W=C.O/'cards-24h-window-ledger.json'
J=C.O/'card089-accounting-journal'
END=dt.datetime.fromisoformat('2026-09-13T23:50:55+00:00').timestamp()
LIMITS={'card_gpu_s':3600,'stage_gpu_s':{'reciprocal':1600,'rank_count_control':1600},'global_vram_gib':30,'rss_gib':32,'owner_aggregate_rss_gib':48,'root_scorer_rss_gib':16,'deadline':'2026-09-13T23:50:55Z'}
...
def _event(tag,seconds,arm,kind):
    event={'transaction':uuid.uuid4().hex,'at':dt.datetime.now(dt.timezone.utc).isoformat(),
           'card':'089','tag':tag,'event':kind,'wall_s':seconds,'arm_s':allocation(seconds,arm)}
...
def transact(tag,seconds,arm=None,kind='reservation',check=False):
    with (C.O/'window-ledger-write.lock').open('a') as f:
        ...
        if not L.exists():C.write(L,{'batch_cap_s':3600,'batch_spent_s':0.,'arm_spent_s':{a:0. for a in C.ARMS},'entries':[]})
        assert W.exists(),'root rolling ledger required'
        _recover()
        if check:
            l=C.read(L);w=C.read(W)
            assert l['batch_spent_s']+seconds<=3600 and w['spent_s']+seconds<=165491,'cumulative budget STOP'
...
def available(arm=None):
    import time
    l=C.read(L);w=C.read(W)
    remaining=[3600-l['batch_spent_s'],165491-w['spent_s'],END-time.time()]
