...
def _event(tag,seconds,arm,kind):
    event={'transaction':uuid.uuid4().hex,'at':dt.datetime.now(dt.timezone.utc).isoformat(),
           'card':'085','tag':tag,'event':kind,'wall_s':seconds,'arm_s':allocation(seconds,arm)}
...
def transact(tag,seconds,arm=None,kind='reservation',check=False):
    with (C.O/'window-ledger-write.lock').open('a') as f:
        ...
        if not L.exists():C.write(L,{'batch_cap_s':10800,'batch_spent_s':0.,'arm_spent_s':{a:0. for a in C.ARMS},'entries':[]})
        assert W.exists(),'root rolling ledger required'
        _recover()
        if check:
            l=C.read(L);w=C.read(W)
            assert l['batch_spent_s']+seconds<=10800 and w['spent_s']+seconds<=165491,'cumulative budget STOP'
...
def available(arm=None):
    import time
    l=C.read(L);w=C.read(W)
    remaining=[10800-l['batch_spent_s'],165491-w['spent_s'],END-time.time()]
