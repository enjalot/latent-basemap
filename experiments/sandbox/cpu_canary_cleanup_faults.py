...
C.write(C.O/f'card{CARD}-functional-loss-readiness/hook-cpu-contracts.json',{'PASS':True,'n_checks':len(checks),'checks':checks,'record':a['record'],'scope':'CPU hooks tested through actual core AST branches; actual full production fit-path proof added to GPU canary but not executed.'});print('HOOK CPU PASS',CARD,len(checks))
