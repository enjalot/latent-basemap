"""512-D real core resume twins and radius=1 control; tiny cyclic graph, no quality claim; ordinary is a tiny canary only."""
import os
...
def main():
 start=time.monotonic();C.source_check();C.input_check();C.graph_check();checks=[];hashes={};keys=['model','optimizer','scheduler','scaler','torch_rng','cuda_rng','loader_gen','loader_perm','loader_pos_idx','loader_batch_no','loader_rank_of_node','loader_node_at_rank','rankneg_scale']
 with tempfile.TemporaryDirectory(dir=str(C.R.parent)) as td:
  ...
  for arm in C.ARMS:
   ...
   p,full,report=fit(arm,18,td/arm,**kw);hashes[arm]=report['state_sha'];del p;gc.collect();torch.cuda.empty_cache()
   candidates=[]
   ...
   for tag,override in [('wrong_arm',{'arm':'wrong'}),('wrong_radius',{'radii_sha':'0'*64}),('wrong_input',{'input_manifest_sha':'0'*64})]:
    try:fit(arm,18,td/(arm+'-'+tag),resume=mid,ident_override=override,**kw)
    except AssertionError as exc:assert 'card055 admission-identity mismatch' in str(exc);checks.append(arm+' '+tag+' rejects before restore')
    else:raise AssertionError('wrong identity accepted')
   if arm=='ordinary3d':
    p,unit,ur=fit(arm,18,td/'ones',X=X,graph=graph,radii=np.ones(n,'f4'),unit_control=True,checkpoints=[2,4,7,9,18]);assert all(same(full[k],unit[k]) for k in keys),'radius1 altered off path';checks.append('radius1 full-state bit-identical to off');del p,unit;gc.collect();torch.cuda.empty_cache()
  assert len(set(hashes.values()))==3,'radius interventions not exposed';checks.append('three radius arms actually diverge')
 result={'PASS':True,'checks':checks,'n_checks':len(checks),'endpoint_hashes':hashes,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'scope':'Actual512-D device core fit on512-row cyclic15 graph,18 successful updates. State twins, default-off control and specific identity failures; no quality claim; ordinary is a tiny canary only.'};C.write(C.O/'card055-device-canary.json',result);print(result,flush=True)
