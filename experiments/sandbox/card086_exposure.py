...
SCHEMA='card086-pair-exposure-v1'
...
def validate(stats):
 e=stats.get('card086_exposure');assert isinstance(e,dict) and e.get('schema')==SCHEMA,'missing exposure state'
 ...
 for k in ['batches','positive_slots','negative_slots','short_tail_batches']:
...
def observe(p):
 ...
 def update(opt,*args,**kwargs):
  ...
  if ours(opt):
   assert opt.param_groups[0]['lr']>0,'exposure zero-LR step forbidden';e=p._train_stats['card086_exposure'];npos,nneg,tail=pending[:3];e['successful_batches']+=1;e['successful_positive_slots']+=npos;e['successful_negative_slots']+=nneg;e['successful_short_tail_batches']+=tail
  return result
