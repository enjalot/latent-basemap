...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,warm_path=None,noise_q_path=None,calibration_callback=None,omit_hook=False):
 C.require_gpu_stage()
 ...
 if X is None:X=np.load(C.D/'train.f16.npy',mmap_mode='r')
 ...
 ident=C.identity(arm,dose,len(X),graph,radius_path,warm_path,noise_q_path,calibration=calibration_callback is not None)
 ...
 p=ParametricUMAP.load(str(C.CHAMP),device='cuda');C.configure(p,ident,radii,checkpoints or [dose])
 if calibration_callback is not None:p._card084_calibration=calibration_callback
 if omit_hook:
  assert ident.get('canary_off_control') and ident['coefficient']==0.
  del p._card084_attraction
 prepared=torch.load(warm_path or C.warm(arm),map_location='cpu',weights_only=False);assert prepared['READY'];warm=prepared['model_state']
 ...
 assert getattr(p,'_card084_attraction',None)==({k:ident[k] for k in ('family','coefficient','delta')} if not omit_hook else None)
...
def validate_arm(arm):
 ...
 assert m['actual_attraction']=={k:ident[k] for k in ('family','coefficient','delta')}
