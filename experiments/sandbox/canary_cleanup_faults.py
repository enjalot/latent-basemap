...
@contextmanager
def force(p,record,card,support=None):
 ...
 def next_batch(s):
  ...
  try:
   ...
   if support is not None:
    arm,ordered,mutual=support
    rows=torch.as_tensor(ordered,device=a.device,dtype=torch.long)[a]
    matches=rows==b[:,None];assert bool((matches.sum(1)==1).all()),'fault recorder actual endpoint mismatch'
    rank=matches.long().argmax(1);mask=torch.as_tensor(mutual,device=a.device,dtype=torch.bool)
    reciprocal=mask[a,rank];favored=reciprocal if arm=='reciprocal' else rank<mask[a].sum(1)
    pending.update(reciprocal=int(reciprocal.sum()),nonreciprocal=npos-int(reciprocal.sum()),favored=int(favored.sum()),unfavored=npos-int(favored.sum()),verified_positive=npos)
   return out
  finally:
...
def validate(record,stats,exposure=None):
 ...
 if exposure is not None:
  for prefix in ['attempted','successful']:
   ...
   for field,key in [('positive_slots','positive'),('negative_slots','negative'),('short_tail_batches','tail'),('original15_slots','original15'),('extra45_slots','extra45')]+[(k+'_slots',k) for k in ['reciprocal','nonreciprocal','favored','unfavored','verified_positive']]:
