"""Card023 measured throughput preflight (per card023-scale-2m.md + root review).
...
remaining-dose admission. PASS=false ⇒ chain STOPS admission with NO dose truncation. Usage: gpu_card023_preflight.py
"""
...
def main():
    ...
    production_estimate = per_arm_est
    checks = {"step_ckpt_observed": bool(ck_ok),
              "per_arm_le_cap": per_arm_est <= PER_ARM_CAP,
              "fits_gpu_cap": production_estimate <= cap_room,
              "fits_deadline": (production_estimate + 60) <= deadline_room,
              "global_vram_lt_30gb": global_used < 30.0,
              "rate_finite_positive": math.isfinite(it_per_s) and it_per_s > 0}
    R = {"schema": "card023-preflight-2026-09-12", "at": dt.datetime.now(dt.timezone.utc).isoformat(),
         ...
         "per_arm_estimate_s": per_arm_est, "per_arm_cap_s": PER_ARM_CAP, "production_estimate_s": production_estimate,
         ...
         "checks": checks,
         "decision": "One 400K half-strength arm + its checkpoint/endpoint reserves must fit the remaining 9,000s cap and deadline. On a miss admission STOPS with NO dose truncation.",
         "PASS": bool(all(checks.values()))}
    (OC / "card023-preflight.json").write_text(json.dumps(R, indent=1)); print(json.dumps(R, indent=1), flush=True)
