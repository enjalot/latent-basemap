...
from card086_exposure import observe,validate as validate_exposure
from card086_cdf_adapter import fixed_cdf,values_sha
from card086_serial_cdf import build as build_cdf

def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,warm_path=None,probe_record=None,allone_original_control=False,canary_faults=False,canary_observer_off=False):
 ...
 cdf_record={};fault_record={}
 from canary_cleanup_faults import force
 with (nullcontext() if allone_original_control else fixed_cdf(cdf,cdf_sha,weight_sha,require_old_exact=arm=='all_one',record=cdf_record,proof_path=C.O/f'card086-cdf-upload-{arm}-{dose}-{time.time_ns()}.json')), (probe(probe_record,weight_sha) if probe_record is not None else nullcontext()), (nullcontext() if canary_observer_off else observe(p)), (force(p,fault_record,'086') if canary_faults else nullcontext()):
 ...
 if canary_observer_off:
  ...
  validate_resume_payload(ck,dose,n_nodes=len(X));assert 'card086_exposure' not in ts,'off control observer was enabled'
 ...
 report={'canary_fault_record':fault_record,'cdf_proof':cdf_record,'exposure':exposure,'arm':arm,'dose':dose,'wall_s':time.monotonic()-start,'train_stats':ts,'pipeline_info':pi,'identity':ident,'state_sha':C.state_sha(ck['model']),'global_vram_GiB':(total-free)/2**30,'endpoint_checkpoint':str(end),'loaded_modules':C.loaded_modules(),'actual_warm_sha':getattr(p,'warm_start_sha256',None),'resumed_from':str(resume) if resume else None,'expected_fresh_warm_sha':expected_warm,'lmc_stats':None,'resume_start_step':int(torch.load(resume,map_location='cpu',weights_only=False)['global_step']) if resume else 0}
 return p,ck,report

def validate_arm(arm):
 dest=C.TD/arm;m=C.read(dest/'manifest.json');ident=C.identity(arm);q=m['cdf_proof'];assert q['uploaded_bit_identical'] and q['cdf_values_sha']==ident['cdf_values_sha'] and q['weight_values_sha']==ident['weight_values_sha'];assert q['decreasing_count']==0 and q['cdf_terminal']==1. and q['positive_zero_width_probability']<=1e-10 and q['zero_weight_interval_mass']==0 and q['zero_weight_nonzero_interval_count']==0;assert arm!='all_one' or (q['old_CDF_bit_identical'] and q['allone_actual100000_positive_and_negative_ID_RNG_exact']);assert m['identity']==ident and m['dose']==C.DOSE and m['model_sha']==C.sha(dest/'model.pt')
