...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,warm_path=None,probe_record=None,allone_original_control=False):
 ...
 cdf_record={}
 with (nullcontext() if allone_original_control else fixed_cdf(cdf,cdf_sha,weight_sha,require_old_exact=arm=='all_one',record=cdf_record,proof_path=C.O/f'card086-cdf-upload-{arm}-{dose}-{time.time_ns()}.json')), (probe(probe_record,weight_sha) if probe_record is not None else nullcontext()), observe(p):
