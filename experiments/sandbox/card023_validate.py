"""Card023 canonical strict validator + identity (per root review groups 2/3). ONE source of truth reused by
...
DATA = Path("/data/latent-basemap/substrates/card023-scale2m")
INIT3D = SB / "card015-init/init-card015-3d.pt"
DATA_MANIFEST = OC / "card023-data-manifest.json"
TD_DEFAULT = SB / "card023-train"
# FULL content hashes (from the admitted card023 data manifest; the trainer re-verifies the on-disk files)
...
R_ACTUAL_SHA256 = "f72fa64da7e6f3a8faa9edac9f69d6c798caae61acc642e1e16084e0e8ec7cca"
...
KERNEL = {"ordinary3d": "baseline umap 3D", "actual3d": "d2/(sqrt(parent_r_i)*sqrt(parent_r_j)) detached 3D"}
...
def runtime_manifest_path(ROOT): return Path(ROOT) / "card023-runtime-sha.json"
...
def expected_identity(arm, ROOT):
    ...
    return {
        "card": "card023", "arm": arm, "kernel": KERNEL[arm],
