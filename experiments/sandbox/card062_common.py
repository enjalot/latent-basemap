...
def verify():
 m=read(R/'card062-runtime-sha.json');assert all(sha(R/k)==v for k,v in m.items());release=read(O/'card062-engineering-release.json');assert release['PASS'] and all(sha(p)==v for p,v in release['files'].items());return sha(R/'card062-runtime-sha.json')
def load(name,device):
 ...
 m._card062_bias=m.proj_out.bias.detach().cpu().numpy().copy()
...
def project(m,x,c,mode='repair',collect=False):
 ...
 try:
  for lo in range(0,len(x),BATCH):
   xx=torch.from_numpy(np.array(x[lo:lo+BATCH],dtype='f4',copy=True)).to(device);teacher=m(xx);teacher_cpu=teacher.cpu().numpy()
   if mode=='repair':
    zz=held.pop('z')
    if not collect and not np.any(np.all(teacher_cpu==m._card062_bias,axis=1)):
     out.append(teacher_cpu);continue
    y,d,b=kernel(teacher,zz,m.proj_out.weight,c,collect=collect)
   ...
   else:out.append(teacher_cpu);continue
