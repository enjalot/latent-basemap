...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,bank_path=None,disabled_bank_control=False):
 ...
 if bank_path is None:bank_path=C.D/'landmark-bank.npz'
 assert len(radii)==len(X) and np.isfinite(radii).all() and (radii>0).all()
 ident=C.identity(arm,dose,len(X),graph,radius_path,bank_path)
 ...
 p=ParametricUMAP.load(str(C.CHAMP),device='cuda');C.configure(p,ident,radii,checkpoints or [dose],bank_path);warm=torch.load(C.D/'init.pt',map_location='cpu',weights_only=False)['model_state']
 ...
 if disabled_bank_control:
  assert arm=='ordinary' and ident['replay_weight']==0.;assert p._card054_lmc is None
 ...
 report={'arm':arm,'dose':dose,'wall_s':time.monotonic()-start,'train_stats':ts,'pipeline_info':pi,'identity':ident,'state_sha':C.state_sha(ck['model']),'global_vram_GiB':(total-free)/2**30,'endpoint_checkpoint':str(end),'loaded_modules':C.loaded_modules(),'actual_warm_sha':getattr(p,'warm_start_sha256',None),'resumed_from':str(resume) if resume else None,'expected_fresh_warm_sha':expected_warm,'lmc_stats':p._card054_lmc.stats() if p._card054_lmc else None,'resume_start_step':int(torch.load(resume,map_location='cpu',weights_only=False)['global_step']) if resume else 0}
 return p,ck,report
...
def validate_arm(arm):
 ...
 model=torch.load(dest/'model.pt',map_location='cpu',weights_only=False);assert model['input_dim']==1536 and model['n_components']==3 and model['hidden_dim']==2048
 ...
 if C.WEIGHTS[arm]:
  ls=m['lmc_stats'];assert ls and ls['weight']==C.WEIGHTS[arm] and ls['attempted_calls']>=C.DOSE-m['resume_start_step'] and ls['invalid_variance_calls']<ls['attempted_calls'],'no valid landmark exposure'
 else:assert m['lmc_stats'] is None
