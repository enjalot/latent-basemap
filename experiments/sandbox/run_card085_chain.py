...
def verify():
    release=C.O/'card085-release.json'
    assert release.exists(),'NO ROOT GPU RELEASE'
    r=C.read(release)
    assert r['PASS'] and r['card']=='085' and r['limits']==B.LIMITS
...
def validate_stage_receipt(path, started_ns, runtime_sha, history_cpu_sha):
    path=Path(path)
    ...
    assert receipt.get('history_cpu_sha')==history_cpu_sha, 'stage receipt calibration mismatch'
    return receipt
...
def stage(tag,script,cap,args=(),arm=None,receipt_path=None):
    ...
    runtime_sha=C.source_check();history_cpu_sha=C.sha((C.O/'card085-history-compatibility.json')) if receipt_path is not None else None
    started_ns=time.time_ns()
    try:
        env=dict(os.environ,CARD085_RELEASE_SHA=release)
        ...
        if receipt_path is not None:
            ...
            assert C.sha((C.O/'card085-history-compatibility.json'))==history_cpu_sha, 'stage calibration changed while running'
            receipt=validate_stage_receipt(receipt_path,started_ns,runtime_sha,history_cpu_sha)
        ...
        C.write(C.O/f'card085-stage-{tag}.json',{'PASS':rc==0 and stage_error is None,'rc':rc,'error':stage_error,'receipt_proof':proof,'stage_started_ns':started_ns,'wall_s':elapsed,'peak_tree_rss_bytes':peak_rss,'peak_global_vram_gib':peak_vram,'runtime_sha':C.source_check()})
    verify()
    return elapsed
...
def main():
    ...
    try:
        ...
        C.input_check()
        assert C.read(C.O/'card085-history-compatibility.json')['CPU_PASS'], 'CPU history STOP'
        assert C.read(C.TD/'fresh/preparation.json')['READY']
        stage_time+=stage('history_replay','gpu_card085_history_replay.py',300,receipt_path=C.O/'card085-history-device.json')
        stage_time+=stage('graph_canary','gpu_card085_graph_canary.py',180,receipt_path=C.O/'card085-graph-canary.json')
        for a in C.ARMS:stage_time+=stage('preflight-'+a,'gpu_card085_preflight.py',240,(a,),a,receipt_path=C.O/f'card085-preflight-{a}.json')
        estimates={}
        for a in C.ARMS:
            pf=STAGE_RECEIPTS['preflight-'+a]['receipt']
            assert pf['PASS'] and pf['runtime_sha']==C.source_check() and pf['history_cpu_sha']==C.sha((C.O/'card085-history-compatibility.json'))
            estimates[a]=pf['estimate']['complete_arm_s']
        ledger=C.read(B.L);window=C.read(B.W)
        checks={'card_cap':ledger['batch_spent_s']+sum(estimates.values())+240<=10800,
                'window_cap':window['spent_s']+sum(estimates.values())+240<=165491,
                'deadline':sum(estimates.values())+360<=B.END-time.time()}
        checks.update({a:ledger['arm_spent_s'][a]+estimates[a]+(240 if a=='finish' else 0)<=B.LIMITS['stage_gpu_s'][a] for a in C.ARMS})
        C.write(C.O/'card085-preflight.json',{'PASS':all(checks.values()),'checks':checks,'estimates':estimates,
            'ledger_at_admission':ledger,'runtime_sha':C.source_check(),'history_cpu_sha':C.sha((C.O/'card085-history-compatibility.json'))})
        if not all(checks.values()):
            C.write(C.O/'card085-execution.json',{'status':'ADMISSION_STOP','reason':'Full400K+60K measured dose does not fit cumulative limits; no truncation'})
            return
        for a in C.ARMS:
            assert B.available(a)>=estimates[a],'remaining full dose no longer fits'
            if a=='finish':
                from prepare_card085 import prepare
                prepare('finish')
                stage_time+=stage('preflight-finish-actual','gpu_card085_preflight.py',240,('finish',),'finish',receipt_path=C.O/'card085-preflight-finish.json')
                actual=STAGE_RECEIPTS['preflight-finish-actual']['receipt']
                assert actual['actual_stage_parent'], 'finish preflight did not use actual U400'
                estimates[a]=actual['estimate']['complete_arm_s']
                assert B.available(a)>=estimates[a]+5, 'actual finish full dose no longer fits'
            stage_time+=stage(a,'run_card085_arm.py',B.LIMITS['stage_gpu_s'][a],(a,),a,receipt_path=C.TD/a/'validation.json')
        C.write(C.O/'card085-execution.json',{'status':'TRAINED_VALIDATED','quality':'NOT_SCORED',
             'runtime_sha':C.source_check(),'history_cpu_sha':C.sha((C.O/'card085-history-compatibility.json'))})
    except BaseException as e:
        C.write(C.O/'card085-execution.json',{'status':'EXECUTION_STOP','error':repr(e)})
        raise
    finally:
