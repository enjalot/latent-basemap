"""Card023 device canary (real 2M fit path, short dose) — per card023-scale-2m.md + root review. Proves:
  ...
  3. wrong-arm and wrong-radius resumes FAIL CLOSED on the SPECIFIC admission-identity error (message match),
     not any ValueError.
Precision is asserted device_fp16 on every fit. Exit 0 = PASS. Usage: gpu_card023_canary.py
"""
...
def main():
    ...
    R = {"schema": "card023-canary-2026-09-12", "short_dose": SHORT, "ckpt_at": CKPT_AT}
    ...
    (OC / "card023-canary.json").write_text(json.dumps(R, indent=1)); print(json.dumps(R, indent=1), flush=True)
