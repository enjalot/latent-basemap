"""Card013 Hook C canary. On the REAL fit path (card010 300K draw + fixed15 graph + shared init, LR 0.001):
...
def run(radii):
    ...
    p = ParametricUMAP.load(str(CHAMPION), device="cuda"); p.model = None
...
def main():
    ...
    sha_ones = run(ones)
    sha_act = run(actual)
    R = {"schema": "card013-canary-2026-09-12", "sha_baseline": sha_base, "sha_ones": sha_ones, "sha_actual": sha_act,
    ...
    (OC / "card013-canary.json").write_text(json.dumps(R, indent=1))
