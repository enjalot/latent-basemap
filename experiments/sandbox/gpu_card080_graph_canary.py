...
import card080_common as C
from card080_fit import fit
...
with tempfile.TemporaryDirectory(dir=C.R.parent,prefix='card080-graph-canary-') as path:
 ...
 for arm in C.ARMS:
  ...
  for tag,override in [('arm',{'arm':'wrong'}),('warm',{'warm_sha':'0'*64}),('scale',{'scale_manifest_sha':'0'*64}),('dimension',{'output_dim':8}),('learning_rate',{'lr':.00037}),('policy',{'rankneg_window':123}),('base_weight',{'base_negative_weight':0.5**0.75})]:
   try:fit(arm,18,td/(arm+'-reject-'+tag),resume=min(mids)[1],ident_override=override,**kw)
   except AssertionError as e:assert 'card080 admission-identity mismatch' in str(e);checks.append(arm+' wrong '+tag+' rejected')
   else:raise AssertionError('wrong graph identity accepted')
 # Real ranked fit off-path versus override.5946: same endpoint and every sampler/RNG state.
 kw={'X':X,'graph':graph,'radius_path':rp,'checkpoints':[2,4,7,9,18]}
 p,off,_=fit('ranked',18,td/'off',ident_override={'base_negative_weight':None},**kw);del p;gc.collect();torch.cuda.empty_cache()
 p,one,_=fit('ranked',18,td/'one',ident_override={'base_negative_weight':0.5**0.75},**kw);del p;gc.collect();torch.cuda.empty_cache()
 assert all(same(off[k],one[k]) for k in keys);checks.append('actual fit override.5946 versus disabled model optimizer sampler RNG bit-identical')
 assert C.state_sha(off['model'])!=hashes['ranked'];checks.append('actual ranked weight1 endpoint diverges from legacy ranked')
 assert len(hashes)==1;checks.append('ranked weight1 small-graph endpoint validated')
C.write(C.O/'card080-graph-canary.json',{'PASS':True,'checks':checks,'n_checks':len(checks),'endpoints':hashes,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'scope':'Real1536D fit,512 nodes,full radii; midpoint and epoch resumes preserve model+optimizer+loader/RNG state; wrong warm/scale/arm reject. No assertion of identical sampled exposure across model-dependent rank orders or AMP skips.'});print('PASS',len(checks),flush=True)
