...
def admitted_canary(record):
 ...
 paths=set();pre_hashes={};pid=os.getpid();start=time.monotonic();env=os.environ.get('CARD086_RELEASE_SHA')
 def capture(p):
 ...
 record.update(pre_full_fingerprint_PASS=True,full_pre_files=pre_hashes,paths=len(paths),runtime_sha=runtime,release_sha=original_sha(C.O/'card086-release.json'),calls=0,scope='Only this process, maximum300s; fixture/checkpoint identity hashes remain actual on every fit')
 def guard():
  assert os.getpid()==pid,'canary admission PID changed'
  assert time.monotonic()-start<300,'canary admission expired'
  assert os.environ.get('CARD086_RELEASE_SHA')==env,'canary release environment changed'
