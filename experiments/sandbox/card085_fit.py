...
def fit(arm,dose,dest,*,X=None,graph=None,radii=None,radius_path=None,checkpoints=None,resume=None,ident_override=None,warm_path=None):
 ...
 p=ParametricUMAP.load(str(C.R.parent/'dino-arrival-t0/champion-bs16k/model.pt' if arm=='fresh' else C.CHAMP),device='cuda');C.configure(p,ident,radii,checkpoints or [dose]);prepared=torch.load(warm_path or C.warm(arm),map_location='cpu',weights_only=False);assert prepared['READY'];warm=prepared['model_state']
 ...
 assert p.rankneg_window==0 and p._rankneg_scale is None, 'actual negative policy differs'
...
def validate_arm(arm):
 dest=C.TD/arm;m=C.read(dest/'manifest.json');ident=C.identity(arm);assert m['identity']==ident and m['dose']==C.DOSES[arm] and m['model_sha']==C.sha(dest/'model.pt')
 ...
 assert C.state_sha(model['model_state_dict'])==m['state_sha'];assert m['train_stats']['positive_lr_optimizer_steps']==C.DOSES[arm] and m['pipeline_info']['x_residency']=='device_fp16'
 ...
 ts=m['train_stats'];assert ts['executed_iters']==ts['optimizer_steps_succeeded']==ts['positive_lr_optimizer_steps']==C.DOSES[arm] and ts['lr_used_min']==ts['lr_used_max']==ident['lr']
 ...
 assert checks>=len(C.SNAPS[arm])+1,'missing real epoch checkpoint'
 for step in C.SNAPS[arm]:
  ...
  if step==C.DOSES[arm]:assert C.state_sha(snap['model_state_dict'])==m['state_sha']
 assert len(set(states))==len(C.SNAPS[arm]),'identical checkpoint states'
 return {'PASS':True,'arm':arm,'model_sha':m['model_sha'],'state_sha':m['state_sha'],'identity':ident,'checked_checkpoints':checks,'runtime_sha':C.source_check(),'history_cpu_sha':C.sha(C.O/'card085-history-compatibility.json')}
