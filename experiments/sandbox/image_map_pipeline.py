...
DATASETS = {
    ...
    **{f"evolbench-S{k}": {
    ...
    # land at sandbox/evolbench-draw2-S{k}/. Same champion recipe; scored via EVOLBENCH_TRUTH_PREFIX=evolbench-draw2.
    **{f"evolbench-draw2-S{k}": {
        "load": (lambda kk=k: np.concatenate([
            np.asarray(np.load(f"/data/latent-basemap/substrates/evolbench-draw2/{t}/substrate.f32.npy",
                               mmap_mode="r"), dtype=np.float32)
            for t in (["T0"] + [f"T{j}" for j in range(1, kk + 1)])])),
        "subsets": None,
        "arms": {"champion-bs16k": {"md": "000", "dose": 4,
    ...
    **{f"monet-random-{sp}-2m": {
    ...
    # 12M-3D twin (owner 2026-09-08): 12M-pca768 recipe + n_components=3, REUSING the 12M graph (symlinked,
    # dimension-independent). Runs with grouped+reuse via envs (C1-adopted: 1.35x real elapsed, quality-neutral) —
    # grouped+reuse's first at-scale production run. 3D-vs-2D bonus caveat: 2D was plain-champion, but grouped+reuse
    # is quality-neutral (C1 Δ−0.0032), so the +3D bonus stays interpretable with that noted.
    "monet-random-dino-12m-pca768-3d": {"load": (lambda: np.asarray(np.load(
        "/data2/monet/random-dino-12m/pca768-substrate.f16.npy", mmap_mode="r"), dtype=np.float32)),
        "prenormalized": True, "subsets": None,
        "arms": {"champion-bs16k": {"md": "000", "dose": 4,
              "extra": {"fneg_weight": 1.0, "neg_tanh_gamma": 4.0, "pos_ratio": 0.10, "n_components": 3,
    ...
    "monet-random-clip-4m": {
        ...
        "arms": {
            ...
            "champion-bs16k": {"md": "000", "dose": 4,
              "extra": {"fneg_weight": 1.0, "neg_tanh_gamma": 4.0, "pos_ratio": 0.10,
                        "rankneg_window": 1_000_000, "batch_size": 16384, "gpu_resident_vram_budget_gb": 22.0}}}},
    # 4M 3D twin (owner 2026-09-05): SAME 4M clip-512 substrate + gated-4M champion recipe, ONLY n_components=3.
    # REUSE the 4M graph (dimension-independent) by symlinking monet-random-clip-4m/{edges-k15-fuzzy.npz,
    # knn_indices.npy} into this dir before train. Completes the {2M,4M}x{2D,3D} matrix. frame.py skipped in scoring.
    "monet-random-clip-4m-3d": {
        "load": (lambda: np.asarray(np.load("/data2/monet/random-clip-4m/clip-substrate.f32.npy", mmap_mode="r"), dtype=np.float32)),
        "subsets": None,
        "arms": {"champion-bs16k": {"md": "000", "dose": 4,
              "extra": {"fneg_weight": 1.0, "neg_tanh_gamma": 4.0, "pos_ratio": 0.10, "n_components": 3,
