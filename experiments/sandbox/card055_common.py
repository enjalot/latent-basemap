...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card055-clip-scale');TD=R.parent/'card055-train';CHAMP=R.parent/'dino-arrival-t0/champion-bs16k/model.pt'
CHAMP_SHA='d62b18cb0df94981f39fca11e4e55b42debef73ec5c3f7fff169717cf0690ad7'
ARMS=['ordinary3d','quarter3d','shuffled_quarter3d'];TRAIN_ARMS=ARMS[1:];RAD={'ordinary3d':None,'quarter3d':'r-quarter.npy','shuffled_quarter3d':'r-shuffled-quarter.npy'};N=300000;DIM=512;NC=3;DOSE=60000;BATCH=16384;SEED=42;LR=.001;SNAPS=[20000,40000,60000]
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card055-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'runtime source changed'
 return sha(R/'card055-runtime-sha.json')
def input_check():
 m=read(D/'inputs-manifest.json');assert m['PASS'] and m['n_train']==N and m['dim']==DIM
 for n,h in m['files'].items():assert sha(D/n)==h,n
 return m
def graph_check():
 m=read(D/'graph-manifest.json');assert m['PASS'] and m['input_manifest_sha']==sha(D/'inputs-manifest.json') and m['parent_graph_manifest_sha']==sha(O/'card055-parent-graph.json') and m['parent_runtime_sha']==sha(R.parent/'card049-code/card049-runtime-sha.json')
 for n,h in m['files'].items():assert sha(D/n)==h,n
 return m
...
def validate_ckpt(ck,ident):
 ...
 assert ck['card012_identity']==ident,'card055 admission-identity mismatch'
