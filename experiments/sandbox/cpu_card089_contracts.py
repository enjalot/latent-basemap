...
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  root=Path(td)/'code';base=Path(td)/'card086-observer-train/all_one';(base/'ckpts').mkdir(parents=True);epoch=base/'ckpts/ckpt-epoch1.pt';paths=[base/n for n in ['validation.json','admission.json','model.pt','prepared.pt','preparation.json','manifest.json']]+[epoch]
 ...
 old=C.R.parent/'card086-functional-loss-code';manifest=C.read(old/'card086-runtime-sha.json');baseline={p:h for p,h in manifest.items() if p.startswith('basemap/')};assert all(C.sha(old/p)==h and C.sha(C.R/p)==h for p,h in baseline.items());checks['all_basemap_bytes_match086']=True
 import ast
