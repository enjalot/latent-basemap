...
import card083_common as C
from card083_fit import fit
...
with tempfile.TemporaryDirectory(dir=C.R.parent,prefix='card083-graph-canary-') as path:
 ...
 for arm in C.ARMS:
  ...
  for tag,override in [('arm',{'arm':'wrong'}),('warm',{'warm_sha':'0'*64}),('learning_rate',{'lr':.00037}),('q_hash',{'noise_q_sha':'0'*64}),('band',{'fneg_weight':.5}),('cap',{'neg_tanh_gamma':2.}),('band_threshold',{'fneg_hi':.5}),('kernel',{'kernel_b':.7})]:
   ...
   except AssertionError as e:assert 'card083 admission-identity mismatch' in str(e);checks.append(arm+' wrong '+tag+' rejected')
   else:raise AssertionError('wrong graph identity accepted')
 p,base,r=fit('both_off',18,td/'registered_control',ident_override={'fneg_weight':1.,'neg_tanh_gamma':4.},**kw);assert r['fneg_telemetry']['final_band_hit_frac_of_neg']>0;hashes['control']=r['state_sha'];del p;gc.collect();torch.cuda.empty_cache()
 assert len(set(hashes.values()))==2;checks.append('joint ablation diverges from registered uniform control with actual band exposure')
 # Repeat control under the identical recipe repeated: attribution metadata does not alter fit.
 p,repeat,_=fit('both_off',18,td/'control_repeat',ident_override={'fneg_weight':1.,'neg_tanh_gamma':4.},**kw);assert all(same(base[k],repeat[k]) for k in keys);checks.append('control model optimizer sampler RNG identical across arm labels');del p
C.write(C.O/'card083-graph-canary.json',{'PASS':True,'checks':checks,'n_checks':len(checks),'endpoints':hashes,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'scope':'Real1536D512-node/radii/uniform-CDF fit. Two ablations and registered control differ; exact mid+epoch resumes; recipe/kernel/threshold/q identity rejects. No claim that loss clipping bounds coordinate gradients.'});print('PASS',len(checks),flush=True)
