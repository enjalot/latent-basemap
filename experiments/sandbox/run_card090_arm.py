...
from card090_fit import fit,validate_arm
arm=sys.argv[1];assert arm in C.ARMS;C.require_release();C.source_check();C.input_check();dest=C.TD/arm
if (dest/'manifest.json').exists():
 v=validate_arm(arm);C.write(dest/'validation.json',v);print(v);raise SystemExit(0)
resume=None
