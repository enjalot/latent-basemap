"""Card023 CPU self-test (per root review group 5). No GPU. Builds a synthetic but structurally-valid
card023-train fixture and asserts the canonical validator PASSES it; then applies independent single-fault
...
Proves the one validator's guards actually fire. Usage: cpu_card023_selftest.py
"""
...
def main():
    ...
    base = tempfile.mkdtemp(prefix="card023-selftest-", dir="/tmp")
    ...
    V.N=original_n
    out = {"schema": "card023-selftest-2026-09-12", "PASS": True, "n_cases": len(results), "results": results}
    (V.OC / "card023-selftest.json").write_text(json.dumps(out, indent=1)); print(json.dumps(out, indent=1), flush=True)
    return 0
