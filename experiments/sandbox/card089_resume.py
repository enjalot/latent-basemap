from card089_common import N,BATCH,SEED,LR
POS_RATIO=.1
def validate_resume_payload(ck, step, n_nodes=None):
    ...
    assert ident['weighted_edge_sampling'] is True and ident['uniform_with_replacement'] is False, 'weighted policy identity';assert ts['pipeline_weighted_requested'] is True and ts['pipeline_weighted_effective'] is True and ts['pipeline_positive_sampling']=='weighted_with_replacement','actual weighted pipeline mismatch';assert ident['rankneg_window']==0, 'uniform policy identity'
