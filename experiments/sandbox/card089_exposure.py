...
@contextmanager
def observe(p,arm,mutual_mask):
 advance=DeviceEdgeSampler.__next__;zero=torch.optim.AdamW.zero_grad;step=torch.optim.AdamW.step;pending=None;mask=None;degree=None
 def ours(opt):
  return opt.param_groups[0]['params'][0] is next(p.model.parameters())
 def next_batch(s):
  nonlocal pending,mask,degree
  out=advance(s);assert s.weighted_edge_sampling and s.positive_target_mode=='binary'
  if mask is None:
   mask=torch.as_tensor(mutual_mask.copy(),device=s.device,dtype=torch.bool);degree=mask.sum(1);mask=mask.reshape(-1)
  assert not s._per_batch and s.perm is not None,'unsupported exposure sampler'
  idx=s.perm[max(0,s.pos_idx-s.num_pos):min(s.pos_idx,s.n_pos)];nr=int(mask[idx].sum());nf=nr if arm=='reciprocal' else int((idx%15<degree[idx//15]).sum())
 ...
 def clear(opt,*args,**kwargs):
  if ours(opt):
   ...
   for k,v in [('reciprocal',pending[5]),('nonreciprocal',npos-pending[5]),('favored',pending[6]),('unfavored',npos-pending[6])]:e['attempted_'+k+'_slots']+=v
 ...
 def update(opt,*args,**kwargs):
  ...
  if ours(opt):
   ...
   for k,v in [('reciprocal',pending[5]),('nonreciprocal',npos-pending[5]),('favored',pending[6]),('unfavored',npos-pending[6])]:e['successful_'+k+'_slots']+=v
  return result
