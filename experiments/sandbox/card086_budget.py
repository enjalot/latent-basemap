...
L=C.O/'card086-ledger.json';W=C.O/'cards-24h-window-ledger.json'
J=C.O/'card086-accounting-journal'
END=dt.datetime.fromisoformat('2026-09-13T23:50:55+00:00').timestamp()
LIMITS={'card_gpu_s':3600,'stage_gpu_s':{'all_one':1600,'membership':1600},'global_vram_gib':30,'rss_gib':32,'deadline':'2026-09-13T23:50:55Z'}
...
def _event(tag,seconds,arm,kind):
    event={'transaction':uuid.uuid4().hex,'at':dt.datetime.now(dt.timezone.utc).isoformat(),
           'card':'086','tag':tag,'event':kind,'wall_s':seconds,'arm_s':allocation(seconds,arm)}
