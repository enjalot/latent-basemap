...
protocols={n:C.sha(C.O/n) for n in ['card088-positive-support.md','card088-quality-prereg.md']}
...
for arm in C.ARMS:
 ...
 if (dest/'preparation.json').exists():
  r=C.read(dest/'preparation.json');assert r['protocols']==protocols and r['READY'] and r['parent_sha']==C.sha(C.CHAMP) and r['runtime_sha']==C.source_check() and r['prepared_sha']==C.sha(dest/'prepared.pt');receipts[arm]=r;continue
 sd={k:v.clone() for k,v in base.items()};assert all(torch.equal(base[k],sd[k]) for k in base)
 torch.save({'READY':True,'arm':arm,'model_state':sd,'prepared_state_sha':C.state_sha(sd)},dest/'prepared.pt');r={'protocols':protocols,'READY':True,'arm':arm,'parent_sha':C.sha(C.CHAMP),'parent_path':str(C.CHAMP),'full_parent_tensors_bit_identical':True,'prepared_state_sha':C.state_sha(sd),'prepared_sha':C.sha(dest/'prepared.pt'),'runtime_sha':C.source_check()};C.write(dest/'preparation.json',r);receipts[arm]=r
assert len({r['prepared_state_sha'] for r in receipts.values()})==1;C.write(C.O/'card088-exposure-initialization.json',{'PASS':True,'arms':receipts,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check()});print('MATCHED INITIALIZATION PASS')
