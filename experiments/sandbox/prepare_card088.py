...
for arm in C.ARMS:
 ...
 if (dest/'preparation.json').exists():
  r=C.read(dest/'preparation.json');assert r['READY'] and r['parent_sha']==C.sha(C.CHAMP) and r['runtime_sha']==C.source_check() and r['prepared_sha']==C.sha(dest/'prepared.pt');receipts[arm]=r;continue
