...
def main():
 ...
 with tempfile.TemporaryDirectory(dir=str(C.R.parent),prefix='card060-canary-') as path:
  ...
  assert len(set(hashes.values()))==3;checks.append('three distinct exposed endpoint hashes');assert all(same(stream['ordinary'][k],stream[a][k]) for a in C.TREATMENTS for k in stream['ordinary']);checks.append('graph/global RNG unchanged at common early probe')
 C.write(C.O/'card060-device-canary.json',{'PASS':True,'checks':checks,'n_checks':len(checks),'endpoints':hashes,'wall_s':time.monotonic()-start,'runtime_sha':C.source_check(),'scope':'Real1536D fit with512-row graph and actual landmark callback,18successes; original052 core offcontrol in separate process, midpoint andepoch resume, wrongidentities, stateless sample/loss probes.'});print('PASS',len(checks),flush=True)
