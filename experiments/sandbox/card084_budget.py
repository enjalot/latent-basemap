...
L=C.O/'card084-ledger.json';W=C.O/'cards-24h-window-ledger.json'
J=C.O/'card084-accounting-journal'
END=dt.datetime.fromisoformat('2026-09-13T23:50:55+00:00').timestamp()
LIMITS={'card_gpu_s':4200,'per_arm_gpu_s':1800,'global_vram_gib':30,'rss_gib':32,'deadline':'2026-09-13T23:50:55Z'}

def allocation(seconds,arm=None):
    return {a:(seconds if arm==a else 0.) if arm else seconds/2 for a in C.ARMS}
...
def _event(tag,seconds,arm,kind):
    event={'transaction':uuid.uuid4().hex,'at':dt.datetime.now(dt.timezone.utc).isoformat(),
           'card':'084','tag':tag,'event':kind,'wall_s':seconds,'arm_s':allocation(seconds,arm)}
...
def transact(tag,seconds,arm=None,kind='reservation',check=False):
    with (C.O/'window-ledger-write.lock').open('a') as f:
        ...
        if not L.exists():C.write(L,{'batch_cap_s':4200,'batch_spent_s':0.,'arm_spent_s':{a:0. for a in C.ARMS},'entries':[]})
        assert W.exists(),'root rolling ledger required'
        _recover()
        if check:
            l=C.read(L);w=C.read(W)
            assert l['batch_spent_s']+seconds<=4200 and w['spent_s']+seconds<=165491,'cumulative budget STOP'
            assert all(l['arm_spent_s'][a]+v<=1800 for a,v in allocation(seconds,arm).items()),'per-arm cumulative STOP'
        return _event(tag,seconds,arm,kind)

def available(arm=None):
    import time
    l=C.read(L);w=C.read(W)
    remaining=[4200-l['batch_spent_s'],165491-w['spent_s'],END-time.time()]
    if arm:remaining.append(1800-l['arm_spent_s'][arm])
    else:remaining.extend(2*(1800-l['arm_spent_s'][a]) for a in C.ARMS)
