"""Root-released, dual-lease, resource-monitored durable Card088 chain. No scoring."""
import os
...
def verify():
    release=C.O/'card088-release.json'
    assert release.exists(),'NO ROOT GPU RELEASE'
    r=C.read(release)
    assert r['PASS'] and r['card']=='088' and r['limits']==B.LIMITS
...
def stage(tag,script,cap,args=(),arm=None,receipt_path=None):
    ...
    runtime_sha=C.source_check();data_manifest_sha=C.sha(C.GD/'manifest.json') if (C.GD/'manifest.json').exists() else None
    started_ns=time.time_ns()
    try:
        env=dict(os.environ,CARD088_RELEASE_SHA=release,CARD088_STAGE_AVAILABLE_S=str(available),CARD088_STAGE_LIMIT_S=str(limit))
        ...
        if receipt_path is not None:
            ...
            if tag=='graph_build':data_manifest_sha=C.sha(C.GD/'manifest.json')
            elif data_manifest_sha is not None:assert C.sha(C.GD/'manifest.json')==data_manifest_sha, 'stage calibration changed while running'
        ...
        C.write(C.O/f'card088-stage-{tag}.json',{'PASS':rc==0 and stage_error is None,'rc':rc,'error':stage_error,'receipt_proof':proof,'stage_started_ns':started_ns,'wall_s':elapsed,'peak_tree_rss_bytes':peak_rss,'peak_global_vram_gib':peak_vram,'runtime_sha':C.source_check()})
    verify()
    return elapsed
...
def main():
    ...
    try:
        ...
        stage_time+=stage('graph_build','build_card088_graph.py',1500,receipt_path=C.O/'card088-graph-ready.json')
        C.input_check()
        stage_time+=stage('prepare','prepare_card088.py',120)
        assert all(C.read(C.TD/a/'preparation.json')['READY'] for a in C.ARMS)
        stage_time+=stage('graph_canary','gpu_card088_graph_canary.py',300,receipt_path=C.O/'card088-graph-canary.json')
        for a in C.ARMS:stage_time+=stage('preflight-'+a,'gpu_card088_preflight.py',240,(a,),a,receipt_path=C.O/f'card088-preflight-{a}.json')
        estimates={}
        for a in C.ARMS:
        ...
        for count in ['500','3500']:
            left=STAGE_RECEIPTS['preflight-original15']['receipt']['fits'][count]['sampler']['actual_first_batch']
            right=STAGE_RECEIPTS['preflight-mixture']['receipt']['fits'][count]['sampler']['actual_first_batch']
        ...
        C.write(C.O/'card088-preflight.json',{'PASS':all(checks.values()),'checks':checks,'estimates':estimates,
            'ledger_at_admission':ledger,'runtime_sha':C.source_check(),'data_manifest_sha':C.sha((C.GD/'manifest.json'))})
        if not all(checks.values()):
            C.write(C.O/'card088-execution.json',{'status':'ADMISSION_STOP','reason':'Fulltwo60K measured dose does not fit cumulative limits; no truncation'})
            return
        for a in C.ARMS:
            assert B.available(a)>=estimates[a],'remaining full dose no longer fits'
            stage_time+=stage(a,'run_card088_arm.py',B.LIMITS['stage_gpu_s'][a],(a,),a,receipt_path=C.TD/a/'validation.json')
        C.write(C.O/'card088-execution.json',{'status':'TRAINED_VALIDATED','quality':'NOT_SCORED',
             'runtime_sha':C.source_check(),'data_manifest_sha':C.sha((C.GD/'manifest.json'))})
    except BaseException as e:
        C.write(C.O/'card088-execution.json',{'status':'EXECUTION_STOP','error':repr(e)})
        raise
    finally:
