...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card073-finishing');TD=R.parent/'card075-train';CHAMP=R.parent/'card023-train/model-actual3d.pt'
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card075-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card075 runtime changed';return sha(R/'card075-runtime-sha.json')
def input_check():
