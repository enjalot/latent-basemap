from card090_common import N,BATCH,LR
POS_RATIO=.1
def validate_resume_payload(ck, step, n_nodes=None):
    ...
    assert cfg.get("batch_size") == BATCH and cfg.get("random_state") == ident["seed"] and cfg.get("architecture") == "residual_bottleneck", "checkpoint sampler/model config"
    ...
    if ident['rankneg_window']==0:
        ...
        assert ck['rankneg_scale']==(2*ident['rankneg_window']/n_nodes)**.75,'rank scale mismatch'
    assert cfg['random_state']==ident['seed'],'seed identity mismatch'
    assert ts['attempted_batches']==step+ts['amp_overflow_skips']+ts['nonfinite_loss_skips']+ts['nonfinite_gradient_skips'],'attempt/skip accounting mismatch'
    for key in ['mn_gen','hold_gen']:
        assert cuda_state(ck[key]),'auxiliary device RNG schema: '+key
    for key in ['dens_gen','deriv_gen']:
        assert ck[key] is None,'disabled auxiliary device RNG: '+key
    for key in ['mn_rng','dens_rng','hold_rng']:
        np.random.RandomState().set_state(ck[key])
    assert cfg['n_epochs']==100000 and cfg['lr_horizon']==int(np.ceil(n_edges/int(BATCH*.1)))*100000,'legacy epoch/horizon mismatch'
