...
START=None;RC=999;CAP=300;OUT=C.O/'card062-engineering';OUT.mkdir(exist_ok=True)
...
def main():
 global START,RC
 START=time.monotonic();ledger=C.O/'card062-ledger.json';window=C.O/'cards-24h-window-ledger.json'
 ...
 for name in C.HEADS:
  ...
  for pop,x,ref_teacher,ref_mask,ref_y in [('target',target,old['target_teacher'],old['target_inactive'],expected['target_direction_constant_norm']),('mixed',mixed,old['reference_teacher'][indices],old['reference_inactive'][indices],expected['reference_direction_constant_norm'][indices])]:
   ...
   perm=np.arange(len(x)-1,-1,-1);pp=C.project(m,x[perm],c)['xy'];assert np.array_equal(pp[np.argsort(perm)],p['xy']);checks.append(name+pop+' fast-prefilter matched-shape ordering')
   fast=C.project(m,x,c)['xy'];assert np.array_equal(fast,p['xy']);checks.append(name+pop+' prefilter versus unfiltered output exact')
 ...
 C.verify();r={'status':'ENGINEERING_COMPLETE','heads':records,'correctness_PASS':True,'checks':checks,'n_checks':len(checks),'speed_PASS':all(v['speed_PASS'] for v in records.values()),'fresh_projection_authorized':False,'runtime_sha':runtime,'release_sha':C.sha(C.O/'card062-engineering-release.json'),'rss_MiB':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024,'wall_s_before_closeout':time.monotonic()-START};assert r['rss_MiB']<8192;C.write(C.O/'card062-engineering-result.json',r);RC=0
 subprocess.run(['/home/enjalot/code/latent-basemap/.venv/bin/python',str(C.O/'notify.py'),'post','codex-overseer',str(C.O/'card062-engineering-result.json'),f'Card062 engineering-only test complete: correctness PASS; mixed-speed pass={r["speed_PASS"]}. Previously exposed inputs only; no fresh projection or promotion. Root owns next release.'],timeout=30)
if __name__=='__main__':
 try:main()
 except Exception as e:C.write(C.O/'card062-engineering-execution-failure.json',{'error':repr(e),'at':dt.datetime.now(dt.timezone.utc).isoformat(),'fresh_projection_authorized':False});raise
 finally:
  if START is not None:
   seconds=time.monotonic()-START
   with (C.O/'window-ledger-write.lock').open('a') as f:
    fcntl.flock(f,fcntl.LOCK_EX)
    for p,key in [(C.O/'card062-ledger.json','batch_spent_s'),(C.O/'cards-24h-window-ledger.json','spent_s')]:
