"""Root-admitted sparse-scale chain. External launcher holds both GPU leases; every exit charged."""
...
def main():
 ...
 if not L.exists():C.write(L,{'schema':'card053-ledger','batch_cap_s':4500,'batch_spent_s':0.,'entries':[]})
 t=time.time();C.input_check();C.graph_check()
 assert stage('device_canary','gpu_card053_canary.py',600)==0,'device canary failed';fresh('card053-device-canary.json',t)
 rc=stage('preflight','gpu_card053_preflight.py',500)
 if rc==3:C.write(C.O/'card053-execution.json',{'status':'ADMISSION_STOP','at':dt.datetime.now(dt.timezone.utc).isoformat(),'reason':'unchanged doses do not fit measured time caps; no truncation'});return
 assert rc==0,'preflight failed';pf=fresh('card053-preflight.json',t);receipts={}
 for arm in C.ARMS:
  ...
  assert stage(arm,'run_card053_arm.py',1350,args=(arm,),arm=arm)==0,'arm stopped; preserve checkpoints';receipts[arm]=validate_arm(arm)
 assert len({r['state_sha'] for r in receipts.values()})==3,'no exposed intervention'
 C.write(C.O/'card053-execution.json',{'status':'TRAINED_VALIDATED','at':dt.datetime.now(dt.timezone.utc).isoformat(),'arms':receipts,'quality':'NOT_YET_SCORED','runtime_sha':C.source_check()})
 subprocess.run([PY,str(C.O/'notify.py'),'post','codex-overseer',str(C.O/'card053-execution.json'),'Card053 sparse-scale three matched4M continuation arms trained and validated. Separate CPU scoring next; no promotion yet.'],timeout=30)
if __name__=='__main__':
 try:main()
 except Exception as e:C.write(C.O/'card053-execution.json',{'status':'EXECUTION_FAILED','at':dt.datetime.now(dt.timezone.utc).isoformat(),'error':repr(e)});raise
 finally:
