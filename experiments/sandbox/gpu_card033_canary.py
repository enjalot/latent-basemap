"""Card033 device canary (real 4M fit path, short dose) — per card033-scale4m-conditional.md + root review. Proves:
  ...
  3. wrong-arm and wrong-radius resumes FAIL CLOSED on the SPECIFIC admission-identity error (message match),
     not any ValueError.
Precision is asserted device_fp16 on every fit. Exit 0 = PASS. Usage: gpu_card033_canary.py
"""
...
def _fresh(arm, radii):
    ...
    p.n_epochs = 100000; p.rankneg_window = RANKNEG; p.gpu_resident_vram_budget_gb = 14.0; p.x_residency = "auto"; p.required_input_pipeline = "device"
...
def _run(arm, radii, steps, warm, resume_from=None, ckpt_targets=None, ckpt_dir=None):
    gc.collect(); torch.cuda.empty_cache()  # release prior fit reservations before the4M resident-admission check
...
def main():
    ...
    R = {"schema": "card033-canary-2026-09-12", "short_dose": SHORT, "ckpt_at": CKPT_AT}
    ...
    (OC / "card033-canary.json").write_text(json.dumps(R, indent=1)); print(json.dumps(R, indent=1), flush=True)
