"""Fixed-parent continuation seeds: immutable inputs and legacy configuration."""
from pathlib import Path
import json,hashlib,sys
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card073-finishing');TD=R.parent/'card090-train';CHAMP=R.parent/'card023-train/model-actual3d.pt'
ARMS=['ranked43','uniform43','uniform44','ranked44'];N=2000000;DIM=1536;DOSE=60000;BATCH=16384;LR=dict.fromkeys(ARMS,.0001);SNAPS=[20000,40000,60000]
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card090-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card090 runtime changed';return sha(R/'card090-runtime-sha.json')
def input_check():
...
def identity(arm,dose=DOSE,nodes=N,graph=None,radii_path=None,warm_path=None):
 ...
 return {'card':'090','arm':arm,'phase':'graph','output_dim':3,'initialization_sha':sha(TD/arm/'preparation.json'),'parent_sha':read(TD/arm/'preparation.json')['parent_sha'],'n_nodes':nodes,'dose':dose,'lr':LR[arm],'lr_schedule':'constant','batch_size':BATCH,'pos_ratio':.1,'seed':seed(arm),'warm_sha':sha(wp),'original_init_sha':sha(CHAMP),'scale_manifest_sha':sha(O/'card023-data-manifest.json'),'input_manifest_sha':sha(D/'inputs-manifest.json'),'graph_sha':sha(graph),'radii_sha':sha(rp),'radii_values_sha':hashlib.sha256(np.ascontiguousarray(np.load(rp),dtype='f4').tobytes()).hexdigest(),'runtime_manifest_sha':source_check(),'rankneg_window':window(arm,nodes),'negative_policy':policy(arm),'environment':environment(),'protocol_sha':sha(O/'card090-continuation-seeds.md'),'quality_prereg_sha':sha(O/'card090-quality-prereg.md'),'precision':'device_fp16','gpu_resident_vram_budget_gb':14.,'fneg_weight':1.,'neg_tanh_gamma':4.,'positive_target_mode':'binary','replay_weight':0.,'lmc_weight':0.,'weight_decay':.01,'grad_clip':1.,'all_auxiliary_losses_off':True}
...
ENV_DEFAULTS={'GROUPED_NEGATIVES':'0','GROUPED_TAILS':'4','ENDPOINT_REUSE':'0','PER_BATCH_EDGE_THRESHOLD':'400000000','DEVICE_INT8_VRAM_MARGIN_GB':'3.5'}
def environment():
 import os
 actual={k:os.environ.get(k,v) for k,v in ENV_DEFAULTS.items()}
 assert actual==ENV_DEFAULTS,'Card090 environment policy mismatch'
 return actual
def seed(arm):
 assert arm in ARMS,'unknown Card090 arm'
 return int(arm[-2:])
def policy(arm):return 'ranked_auto_scaling' if arm.startswith('ranked') else 'uniform_nonself_rank_scaling_off'
def window(arm,nodes=N):return min(500000,nodes-1) if arm.startswith('ranked') else 0
def require_release():
 import os
 p=O/'card090-release.json';assert p.exists(),'NO ROOT GPU RELEASE'
 r=read(p);assert r.get('PASS') is True and r['card']=='090','invalid root release'
 assert os.environ.get('CARD090_RELEASE_SHA')==sha(p),'root release environment mismatch'
