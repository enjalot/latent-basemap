...
import numpy as np
R=Path(__file__).resolve().parents[2];O=R.parent/'overseer-codex';D=Path('/data/latent-basemap/substrates/card065-initialization');TD=R.parent/'card067-train';CHAMP=R.parent/'card015-train/model-actual_full3d.pt'
ARMS=['eight'];N=300000;DIM=1536;DOSE=60000;BATCH=16384;SEED=42;LR=.001;SNAPS=[20000,40000,60000];PREP_STEPS=2000;PREP_BATCH=2048;PREP_SEED=65068
def sha(p):
 h=hashlib.sha256()
...
def state_sha(sd):
 ...
 return h.hexdigest()
def source_check():
 m=read(R/'card067-runtime-sha.json');assert all(sha(R/n)==h for n,h in m.items()),'card067 runtime changed';return sha(R/'card067-runtime-sha.json')
def input_check():
...
def identity(arm,dose=DOSE,nodes=N,graph=None,radii_path=None,warm_path=None):
 ...
 return {'card':'067','arm':arm,'phase':'graph','output_dim':8,'initialization_sha':sha(TD/arm/'preparation.json'),'n_nodes':nodes,'dose':dose,'lr':LR,'lr_schedule':'constant','batch_size':BATCH,'pos_ratio':.1,'seed':SEED,'warm_sha':sha(wp),'original_init_sha':sha(D/'fresh-init.pt'),'scale_manifest_sha':sha(D/'scale-manifest.json'),'input_manifest_sha':sha(D/'inputs-manifest.json'),'graph_sha':sha(graph),'radii_sha':sha(rp),'radii_values_sha':hashlib.sha256(np.ascontiguousarray(np.load(rp),dtype='f4').tobytes()).hexdigest(),'runtime_manifest_sha':source_check(),'rankneg_window':75000 if nodes==N else min(75000,nodes-1),'precision':'device_fp16','gpu_resident_vram_budget_gb':14.,'fneg_weight':1.,'neg_tanh_gamma':4.,'positive_target_mode':'binary','replay_weight':0.,'lmc_weight':0.,'weight_decay':.01,'grad_clip':1.,'all_auxiliary_losses_off':True}
def configure(p,ident,radii,checkpoints):
 p.model=None;p.n_components=8;p.hidden_dim=2048;p.learning_rate=LR;p.lr_schedule='constant';p.warmup_steps=0;p.n_epochs=100000;p._max_train_steps=ident['dose'];p.rankneg_window=ident['rankneg_window'];p.batch_size=BATCH;p.gpu_resident_vram_budget_gb=14.;p.x_residency='auto';p.required_input_pipeline='device'
