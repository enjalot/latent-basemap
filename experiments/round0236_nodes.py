"""Execute R0236 — the nested 25M rung, its graph, the replicate grid, the I/O.

Four nodes, one queue, deliberately narrow (no map is trained here):

* `assemble_25000k` (GPU lease, CPU work) assembles the 25,000,000-row mixed
  substrate so that R0235's 12,500,000 training rows are its **positional
  prefix**, draws the increment uniformly from the complement of rung 2's
  training AND the inherited reserve rows, inherits that reserve verbatim,
  asserts nesting by hash and by provenance-key containment, asserts reserve
  disjointness, and asserts `>= 99.9%` shard coverage per corpus on the union
  *and* on the increment alone.
* `truth_probe_25000k` (GPU) computes exact brute-force k15 truth for the
  registered uniform probe of 1,000,000 query rows against **all** 25,000,000
  rows, streaming the substrate through the device because 38.4 GB no longer
  fits on a 31.37 GiB card. Full truth over every row costs `4.69 GPU-h` at this
  rung and the round says so instead of pretending otherwise.
* `ladder_25000k` (GPU) measures the **replicate grid** - three k-means seeds at
  each of 6.25M / 12.5M / 25M nested prefixes, at `c in (16, 32, 64, 128, 200,
  400)` - fits the device law on sealed points, selects `c` under a guard that
  applies the imbalance margin, and builds the one selected cell under a
  SIGNAL-FREE watchdog with the block-layer I/O instrument attached.
* `qualify_25000k` (GPU) scores the emitted graph against the probe truth,
  applies the R0215 degree-zero tripwire over **every** row in and out,
  symmetrises through R0216's identical fuzzy law, publishes the replicated
  drift table, fits the substrate-read scaling, and re-derives every remaining
  Phase-2 rung WITH the margin and WITH its tolerance to adverse imbalance.
...
EMB = "/data/embeddings"
BUILD_SCRIPT = "basemap/round0236_build.py"

ASSEMBLE_ACTION = "assemble_25000k"
TRUTH_ACTION = "truth_probe_25000k"
LADDER_ACTION = "ladder_25000k"
QUALIFY_ACTION = "qualify_25000k"

#: Truth blocking. The 38.4 GB substrate no longer fits the card, so the database
#: streams through in `TRUTH_DB_CHUNK` slices while the 1.54 GB probe query block
#: and the running top-k stay resident. Device: `1.54 + 6.14 + 4.29 + 0.19 GiB`.
    ...
    *,
    ...
    if os.path.exists(receipt_path):
        ...
        raise Round0236Error(
            f"R0236 child {config['setting_id']} produced no receipt "
...
def _verify_source_sizes(job: Mapping[str, Any]) -> str:
    ...
    if signature is None:
        raise Round0236Error("R0236 requires the bound source size manifest")
    path = prompt_contract.verify_signature(
        dict(signature), label="R0236 source size manifest"
    )
    with open(path, encoding="utf-8") as handle:
    ...
    if drift:
        raise Round0236Error(
            f"R0236 source shards changed size since preparation: {drift[:4]}"
        )
    return path


def _parent_substrate(job: Mapping[str, Any]) -> dict[str, Any]:
    """Read and fully verify R0235's sealed rung-2 substrate manifest."""
    path = prompt_contract.verify_signature(
        dict(job["parent_substrate_manifest"]), label="R0235 substrate manifest"
    )
    sealed = prompt_contract.read_sealed(path, label="R0235 substrate manifest")
    if str(sealed.get("round_id")) != PARENT_ROUND_ID or int(
        sealed.get("rows", -1)
    ) != PARENT_ROWS:
        raise Round0236Error("R0236 parent manifest is not R0235's 12.5M rung")
    resolved = {
        key: prompt_contract.verify_signature(
            dict(sealed[key]), label=f"R0235 {key}"
        )
        for key in (
...
def run_assemble(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    ...
    if str(manifest.get("round_id")) != ROUND_ID:
        raise Round0236Error("R0236 handler received another queue")
    size_manifest_path = _verify_source_sizes(job)
    parent = _parent_substrate(job)
    parent_sealed = parent["sealed"]
    output = create_fresh_directory(str(job["outputs"][0]), label="R0236 substrate")
    ...
    if parent_prov.shape[0] != PARENT_ROWS or parent_reserve_prov.shape[0] != (
        ...
        raise Round0236Error("R0236 parent provenance has the wrong row count")
    ...
    for index, (corpus, want_total) in enumerate(COMPOSITION):
        ...
        if int(sealed_source.get("corpus_rows", -1)) != total or int(
            ...
            raise Round0236Error(
                f"{corpus}: shard enumeration differs from R0235's sealed record "
        ...
        if parent_train.size != dict(PARENT_COMPOSITION)[corpus]:
            raise Round0236Error(f"{corpus}: parent training rows do not close")
        if parent_reserve.size != RESERVE_ROWS_PER_CORPUS:
            raise Round0236Error(f"{corpus}: parent reserve rows do not close")
        ...
        if np.intersect1d(selected, parent_train).size != 0:
            raise Round0236Error(f"{corpus}: increment overlaps rung 2's training")
        if np.intersect1d(selected, parent_reserve).size != 0:
            raise Round0236Error(f"{corpus}: increment overlaps the inherited reserve")
        ...
        gc.collect()
    if at != ROWS:
        raise Round0236Error(f"assembled {at} rows, expected {ROWS}")
    ...
    def _write(tmp: str) -> None:
        ...
        if source.shape != (PARENT_ROWS, DIMENSION):
            raise Round0236Error("R0235 substrate geometry is not (12500000, 384)")
        for begin in range(0, PARENT_ROWS, COPY_BLOCK):
            ...
            if not np.isfinite(block).all() or float(norms.min()) <= 0:
                raise Round0236Error("R0235 substrate prefix is degenerate")
            if float(np.abs(norms - 1.0).max()) > 1e-4:
                raise Round0236Error(
                    "R0235 substrate prefix is not unit-normalised; renormalising "
        ...
        for corpus, _want_total in COMPOSITION:
            ...
            if staged.shape != (want, DIMENSION):
                raise Round0236Error(f"{corpus}: staged increment has {staged.shape}")
            for begin in range(0, want, COPY_BLOCK):
                end = min(begin + COPY_BLOCK, want)
                block = np.asarray(staged[begin:end], dtype=np.float32)
                norms = np.linalg.norm(block, axis=1)
                if not np.isfinite(block).all() or float(norms.min()) <= 0:
                    raise Round0236Error(f"{corpus}: increment holds a degenerate row")
                train[cursor + begin:cursor + end] = block / norms[:, None]
                del block, norms
            cursor += want
            del staged
        if cursor != ROWS:
            raise Round0236Error(f"wrote {cursor} rows, expected {ROWS}")
        train.flush()
        del train
    ...
    if prefix_sha != parent_ordered:
        raise Round0236Error(
            "R0236 substrate prefix is not R0235's bytes: "
            f"{prefix_sha} != {parent_ordered}. The registered nesting is "
            "positional and byte-identical."
        )
    grandparent_sha = ordered_array_sha256(written[:GRANDPARENT_ROWS])
    ...
    nesting.update({
        ...
        "grandparent": {
            ...
            "note": (
                "R0235's own prefix is byte-identical to R0233's, so this "
                "substrate's first 6,250,000 rows are rung 1 and the imbalance "
                "replicate grid can be measured on prefixes of one file"
            ),
        },
    })
    ...
    for key, name in (
        ...
        if signature["sha256"] != dict(parent_sealed[key])["sha256"]:
            raise Round0236Error(f"R0236 inherited {key} copy is not byte-identical")
    ...
    if increment_overlap != 0:
        raise Round0236Error(
            f"R0236 increment captured {increment_overlap} reserve rows"
        )
...
def _substrate_from_manifest(job: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    path, _observed = _intra(job, "substrate_manifest", label="R0236 substrate manifest")
    ...
    if str(sealed.get("round_id")) != ROUND_ID or int(sealed.get("rows", -1)) != ROWS:
        raise Round0236Error("R0236 substrate manifest is not this round's rung")
    return prompt_contract.verify_signature(
        dict(sealed["substrate"]), label="R0236 substrate"
    ), sealed
...
def _host_substrate(path: str) -> np.ndarray:
    ...
    if host.shape != (ROWS, DIMENSION) or host.dtype != np.float32:
        raise Round0236Error(f"R0236 substrate geometry is {host.shape}/{host.dtype}")
    return host
...
def run_truth(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    ...
    output = create_fresh_directory(str(job["outputs"][0]), label="R0236 probe truth")
    started = time.monotonic()
    ...
    if int(job["probe_rows"]) != TRUTH_PROBE_ROWS or int(
        ...
        raise Round0236Error(
            "R0236 queue declares a probe this release did not register: "
    ...
    if probe_rows.shape != (TRUTH_PROBE_ROWS,):
        raise Round0236Error(
            f"R0236 probe has {probe_rows.shape} rows, registered {TRUTH_PROBE_ROWS}"
        )
    if int(np.unique(probe_rows).size) != TRUTH_PROBE_ROWS:
        raise Round0236Error("R0236 probe holds a duplicated query row")
    if int(probe_rows.min()) < 0 or int(probe_rows.max()) >= ROWS:
        raise Round0236Error("R0236 probe holds an out-of-range query row")
    if not np.array_equal(probe_rows, np.sort(probe_rows)):
        raise Round0236Error("R0236 probe rows are not ascending")
    ...
    if int((ids.astype(np.int64) == probe_rows[:, None]).sum()) != 0:
        raise Round0236Error("R0236 probe truth retained a self edge")
    if int(ids.min()) < 0 or int(ids.max()) >= ROWS:
        raise Round0236Error("R0236 probe truth holds an out-of-range id")
    if not np.isfinite(cosines).all():
        raise Round0236Error("R0236 probe truth cosines are not finite")
    if int((np.diff(cosines, axis=1) > 1e-5).sum()) != 0:
        raise Round0236Error("R0236 probe truth is not descending in cosine")
...
'''
    ...
    if completed.returncode != 0 or not os.path.exists(result_path):
        raise Round0236Error(
            f"R0236 imbalance grid failed ({completed.returncode}):\n"
...
def _inherited_imbalance(job: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            "2,000,000 is the one N this round does not hold as a prefix, so its "
            "cells stay single-realisation and are reported as such"
        ),
    }


def _inherited_io_points(job: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Architectural substrate reads from the sealed 6.25M and 12.5M ladders."""
    points: list[dict[str, Any]] = []
    for key, label in (
        ("r0233_ladder", "R0233"),
        ("r0235_ladder", "R0235"),
...
def run_ladder(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    ...
    substrate_path, _sealed = _substrate_from_manifest(job)
    output = create_fresh_directory(str(job["outputs"][0]), label="R0236 build ladder")
    ...
    imbalance, grid = _measure_imbalance_grid(
    ...
    if not measured:
        raise Round0236Error("R0236 measured no primary-seed imbalance at this rung")
    #: The selection is taken on the WORST realisation across replicate seeds, not
    #: on the primary alone: three seeds now exist and using the best of them
    #: would be picking a favourable draw. The primary is published beside it.
    worst = {
    ...
    for clusters in (selected_clusters,):
        cell_id = f"r0236-n{ROWS}-c{clusters}-s{SPILL}"
        guard = guard_decision(
            rows=ROWS, clusters=clusters, imbalance=worst[clusters],
            imbalance_source=(
                "worst of three replicate seeds measured on this substrate at "
                "this N"
            ),
...
def _score_probe(
    *,
    ...
    summary = {
        ...
        "strict": summarize(strict, label="R0236 strict recall@15"),
        "tie_aware": summarize(tie, label="R0236 tie-aware recall@15"),
...
def run_qualify(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    ...
    truth_manifest_path, _ = _intra(job, "truth_reference", label="R0236 truth")
    truth = prompt_contract.read_sealed(truth_manifest_path, label="R0236 truth")
    truth_rows_path, _ = _intra_signature(
        dict(truth["outputs"]["query_rows"]), label="R0236 probe rows"
    )
    truth_ids_path, _ = _intra_signature(
        dict(truth["outputs"]["ids"]), label="R0236 truth ids"
    )
    truth_cos_path, _ = _intra_signature(
        dict(truth["outputs"]["cosines"]), label="R0236 truth cosines"
    )
    ladder_path, _ = _intra(job, "ladder_reference", label="R0236 build ladder")
    ladder = prompt_contract.read_sealed(ladder_path, label="R0236 build ladder")

    output = create_fresh_directory(
        str(job["outputs"][0]), label="R0236 qualification"
    )
    started = time.monotonic()
    ...
    if truth_ids.shape != (TRUTH_PROBE_ROWS, GRAPH_K):
        raise Round0236Error("R0236 truth arrays have the wrong shape")
    if not np.array_equal(
        ...
        raise Round0236Error(
            "R0236 sealed probe rows are not the registered draw at seed "
            f"{TRUTH_PROBE_SEED}"
        )
    ...
    if not entry.get("graph_ids"):
        raise Round0236Error(
            f"R0236 selected c = {selected_clusters} emitted no graph"
        )
    ids_path = prompt_contract.verify_signature(
        dict(entry["graph_ids"]), label=f"R0236 {entry['cell']} graph"
    )
    ids = np.ascontiguousarray(
        np.load(ids_path, allow_pickle=False).astype(np.int32)
    )
    if ids.shape != (ROWS, GRAPH_K):
        raise Round0236Error(f"R0236 {entry['cell']} graph is {ids.shape}")
    ...
    if entry.get("graph_cos"):
        builder_cos_path = prompt_contract.verify_signature(
            dict(entry["graph_cos"]), label=f"R0236 {entry['cell']} builder cosines"
        )
    ...
    if int(structural["zero_degree_rows"]) > MAX_ZERO_DEGREE_ROWS:
        raise Round0236Error(
            f"R0236 R0215 tripwire on the raw graph: "
    ...
    if float(selected["tie_aware"]["mean"]) < RECALL_MEAN_FLOOR:
        raise Round0236Error(
            f"R0236 tie-aware recall {selected['tie_aware']['mean']} is below "
            f"the registered {RECALL_MEAN_FLOOR} floor"
        )
    if float(selected["tie_aware"]["p10"]) < RECALL_P10_FLOOR:
        raise Round0236Error(
            f"R0236 tie-aware p10 {selected['tie_aware']['p10']} is below the "
    ...
    if not np.isfinite(dists).all():
        raise Round0236Error("R0236 candidate distances are not finite")
    del cos_sorted
    gc.collect()
    ...
    if not np.isfinite(wts).all() or wts.min() <= 0 or wts.max() > 1:
        raise Round0236Error("R0236 fuzzy weights are invalid")
    ...
    if degrees["zero_degree_rows"] > MAX_ZERO_DEGREE_ROWS:
        raise Round0236Error(
            f"R0236 R0215 tripwire: {degrees['zero_degree_rows']} zero-degree rows"
        )
    ...
    for cell in ladder["ladder"]:
        ...
        if not build.get("fit") or largest <= 0 or not measured_bytes:
            continue
        own_points.append({
            "source": f"R0236 ladder {cell['cell']}",
            "cell": cell["cell"],
            "rows": ROWS,
            "clusters": int(cell["clusters"]),
            "spill": SPILL,
    ...
    drift = replicate_drift(grid, inherited=inherited_imbalance["by_rows"])
    ...
    for rung in PHASE2_RUNGS:
        with_margin = rung_derivation(
            ...
            imbalance_source=(
                f"worst of three replicate seeds at N = {ROWS:,}, s = 8, on this "
                "round's substrate"
            ),
        ...
        rungs[str(rung)] = {
            ...
            "point_estimate_no_margin": rung_derivation(
                rung=int(rung), imbalance_by_c=worst_now,
                imbalance_source=(
                    f"worst of three replicate seeds at N = {ROWS:,}, s = 8"
                ),
    ...
    receipt = prompt_contract.seal(json_safe({
        ...
        "capabilities": [GRAPH_CAPABILITY, IMBALANCE_CAPABILITY, IO_CAPABILITY],
        "rows": ROWS,
        "k": GRAPH_K,
        "spill": SPILL,
...
def run_job(active: Mapping[str, Any], job: Mapping[str, Any]) -> None:
    ...
    if action == ASSEMBLE_ACTION:
        ...
        raise Round0236Error(f"R0236 does not authorize action {action!r}")
