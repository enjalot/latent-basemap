"""Qualify GPU IVF-PQ candidate search on the fixed balanced-90M rung."""
...
def _build_filtered_index(
    *,
    ...
    if (
        ...
        raise Round0072Error("registered source IVF-PQ geometry changed")
    ...
    if (
        ...
        raise Round0072Error("physical 90M eligibility filtering changed")
    temporary = destination_path + ".partial"
    if os.path.exists(temporary) or os.path.exists(destination_path):
        raise Round0072Error("filtered 90M index output exists")
...
def _queries(
    ...
    if (
        ...
        raise Round0072Error("balanced-90M quality queries are invalid")
    values /= norms
...
def run_qualification(
    ...
    output = create_fresh_directory(
        str(job["outputs"][0]),
        label="Round 0072 balanced-90M GPU qualification",
    )
    ...
    if (
        ...
        raise Round0072Error("balanced-90M eligibility accounting changed")
    encoded = np.memmap(
    ...
    if index_signature["sha256"] != INDEX_SHA256:
        raise Round0072Error("registered 150M IVF-PQ bytes changed")
    ...
    if not passed:
        raise Round0072Error(
            "balanced-90M GPU qualification failed: "
...
def run_job(
    ...
    if active.get("manifest", {}).get("round_id") != ROUND_ID:
        raise Round0072Error("R0072 handler received another queue")
    selected = job if job is not None else active.get("job") or {}
    if selected.get("action") != "qualify_balanced_90m_gpu_ivfpq":
        raise Round0072Error("R0072 accepts only GPU qualification")
