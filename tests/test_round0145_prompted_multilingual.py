"""CUDA-hidden contract, encoder, and preparer tests for R0145."""
...
from basemap import round0145_prompted_multilingual as contract
from basemap.artifact_identity import expected_input_signature
from experiments import prepare_round0145_queue as prepare
from experiments import round0145_nodes as nodes
...
def _fixture_inventory(tmp_path) -> tuple[dict, str]:
        ...
        ranges.append(
            {
                ...
                "global_row_start": tranche["r0087_global_row_range"][0],
                "global_row_stop": tranche["r0087_global_row_range"][1],
    ...
    return {"selection": {"ranges": ranges}}, str(tmp_path / "chunks")
...
def test_registered_tranches_and_budget_close_exactly() -> None:
    ...
    assert [item["r0087_global_row_range"] for item in contract.LANGUAGE_TRANCHES] == [
        [23_329_094, 24_164_547],
        [24_164_547, 25_000_000],
    ]
    assert contract.ROWS_PER_LANGUAGE == 835_453
    assert contract.CORPUS_ROWS == 1_670_906
    assert contract.production_payload_bytes() == 2_566_511_616
    ...
    assert (
        ...
        == pytest.approx(2.609511695906433)
    )
    ...
    assert contract.CAPABILITY == (
        "jina-document-multilingual-tur-vie-1p671m-v1"
    )
    assert prepare.OUTPUT_NAMESPACE.endswith("multilingual-007-v1")
    assert prepare.TOKEN_LENGTH_CALIBRATION["tur_Latn"] == {
        "mean": 384.885,
        "p95": 521.1,
        "maximum": 562,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["vie_Latn"] == {
        "mean": 422.246,
        "p95": 527.0,
        "maximum": 605,
    }
...
def test_source_layout_binds_exact_r0087_rows_and_both_byte_streams(
    ...
    with pytest.raises(contract.Round0145Error, match="malformed"):
...
def test_boundary_rehash_rejects_mutated_large_input_equivalent(tmp_path) -> None:
    ...
    with pytest.raises(contract.Round0145Error, match="bytes changed"):
...
def test_preparer_materializes_three_independent_gpu_nodes_and_cpu_finalizer(
    ...
    round_file = tmp_path / "round-0145.md"
    ...
    queue_path = prepare.prepare_round0145(
        release_sha="a" * 40,
