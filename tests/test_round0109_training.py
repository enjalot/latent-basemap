...
from experiments import round0109_nodes
from experiments.prepare_round0109_queue import (
    GRAPH_MANIFEST,
    R0106_REVIEW,
    R0106_REVIEW_SHA256,
    SEED,
    _require_successful_r0108_terminal,
)
...
def test_seed43_config_changes_only_registered_identity_and_seed() -> None:
    ...
    treatment, treatment_sha = train_config(
        ...
        graph_signature=signature,
        seed=43,
        schema="round0109-diverse-jina-train-config-v1",
    )
    assert control_sha != treatment_sha
    assert treatment["schema"] == "round0109-diverse-jina-train-config-v1"
    assert treatment["optimizer"]["seed"] == 43
...
def test_round0109_wrapper_binds_seed_and_receipt_schemas(
    ...
    monkeypatch.setattr(round0109_nodes, "run_train_contract", fake)
    value = round0109_nodes.run_job(
        {"manifest": {"round_id": "0109"}},
        {"action": "train_diverse_jina_seed43"},
    )
    assert value == {"round_id": "0109"}
    assert observed == {
        "round_id": "0109",
        "seed": 43,
    ...
    with pytest.raises(Round0107Error):
        round0109_nodes.run_job(
            {"manifest": {"round_id": "0108"}},
            {"action": "train_diverse_jina_seed43"},
        )


def test_r0108_terminal_ordering_requires_clean_success(tmp_path) -> None:
    ...
    assert _require_successful_r0108_terminal(
...
def test_round0109_binds_reviewed_graph_and_seed42_review() -> None:
    assert SEED == 43
