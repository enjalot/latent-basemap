"""CUDA-hidden contract, encoder, and preparer tests for R0139."""
...
from basemap import round0139_prompted_multilingual as contract
from basemap.artifact_identity import expected_input_signature
from experiments import prepare_round0139_queue as prepare
from experiments import round0139_nodes as nodes
...
def test_registered_tranches_and_budget_close_exactly() -> None:
    ...
    assert [item["r0087_global_row_range"] for item in contract.LANGUAGE_TRANCHES] == [
        [14_139_100, 14_974_554],
        [14_974_554, 15_810_008],
        [15_810_008, 16_645_462],
    ]
    ...
    assert contract.CAPABILITY == (
        "jina-document-multilingual-hin-ind-ita-2p506m-v1"
    )
    assert prepare.OUTPUT_NAMESPACE.endswith("multilingual-003-v1")
    assert prepare.TOKEN_LENGTH_CALIBRATION["hin_Deva"] == {
        "mean": 543.527,
        "p95": 713.0,
        "maximum": 824,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["ind_Latn"] == {
        "mean": 499.841,
        "p95": 651.05,
        "maximum": 752,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["ita_Latn"] == {
        "mean": 440.569,
        "p95": 627.05,
        "maximum": 688,
    }
...
def test_source_layout_binds_exact_r0087_rows_and_both_byte_streams(
    ...
    with pytest.raises(contract.Round0139Error, match="malformed"):
...
def test_boundary_rehash_rejects_mutated_large_input_equivalent(tmp_path) -> None:
    ...
    with pytest.raises(contract.Round0139Error, match="bytes changed"):
...
def test_preparer_materializes_three_independent_gpu_nodes_and_cpu_finalizer(
    ...
    round_file = tmp_path / "round-0139.md"
    ...
    queue_path = prepare.prepare_round0139(
        release_sha="a" * 40,
