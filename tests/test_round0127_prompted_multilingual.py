"""CUDA-hidden contract, encoder, and preparer tests for R0127."""
...
from experiments import prepare_round0127_queue as prepare
...
def test_registered_tranches_and_budget_close_exactly() -> None:
    ...
    assert [item["r0087_global_row_range"] for item in contract.LANGUAGE_TRANCHES] == [
        [11_632_738, 12_468_192],
        [12_468_192, 13_303_646],
        [13_303_646, 14_139_100],
    ]
    ...
    assert contract.CAPABILITY == (
        "jina-document-multilingual-deu-ell-fra-2p506m-v1"
    )
    assert prepare.OUTPUT_NAMESPACE.endswith("multilingual-002-v1")
    assert prepare.TOKEN_LENGTH_CALIBRATION["deu_Latn"] == {
        "mean": 429.908,
        "p95": 616.0,
        "maximum": 661,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["ell_Grek"] == {
        "mean": 393.555,
        "p95": 499.0,
        "maximum": 580,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["fra_Latn"] == {
        "mean": 440.627,
        "p95": 610.0,
        "maximum": 663,
    }
...
def test_source_layout_binds_exact_r0087_rows_and_both_byte_streams(
    ...
    with pytest.raises(contract.Round0127Error, match="malformed"):
...
def test_boundary_rehash_rejects_mutated_large_input_equivalent(tmp_path) -> None:
    ...
    with pytest.raises(contract.Round0127Error, match="bytes changed"):
...
def test_preparer_materializes_three_independent_gpu_nodes_and_cpu_finalizer(
    ...
    round_file = tmp_path / "round-0127.md"
    ...
    queue_path = prepare.prepare_round0127(
        release_sha="a" * 40,
