"""CUDA-hidden node-receipt -> finalizer -> reload smoke for R0143."""
...
def test_post_embedding_receipts_finalize_without_cuda(
    ...
    for module in (contract, nodes):
        ...
        monkeypatch.setattr(module, "R0087_GLOBAL_STOP", 104)
    monkeypatch.setattr(
        nodes,
    ...
    assert result["source_order"] == ["pol_Latn", "por_Latn", "rus_Cyrl"]
    assert result["coverage_validation"]["scanned_rows"] == 6
    assert result["coverage_validation"]["gap_free"] is True
    assert result["r0087_selected_global_row_range"] == [100, 104]
    assert result["heldout_prompted_languages"] == ["pol_Latn"]
    ...
    contract.validate_seal(
        ...
        label="R0143 CPU smoke final manifest",
    )
    ...
    contract.validate_seal(
        published, label="reloaded R0143 CPU smoke final manifest"
    )
