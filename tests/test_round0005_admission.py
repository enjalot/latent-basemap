...
from experiments.run_round0005_fixture import (
    EnvironmentValidationError, _mutation_integrations, _prepared_admission, _publish_case,
    main as run_acceptance_fixture,
)
import experiments.run_round0005_fixture as round0005_fixture_module
...
def test_acceptance_fixture_requires_clean_detached_release_and_refuses_reuse(
    ...
    release_args, _evidence = round0005_clean_release
    repo = release_args.repo_root
    release = release_args.release_sha
    monkeypatch.setattr(
        round0005_fixture_module, "__file__",
        os.path.join(repo, "experiments", "run_round0005_fixture.py"))
    ...
    with pytest.raises(EnvironmentValidationError, match="environment manifest is incomplete"):
        run_acceptance_fixture(argv)
    assert not os.path.lexists(out)

    subprocess.check_call(["git", "-C", repo, "switch", "-q", "-c", "fixture-attached"])
    Path(repo, "tracked.txt").write_text("dirty attached release\n")
