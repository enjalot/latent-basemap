"""CUDA-hidden contract, encoder, and preparer tests for R0144."""
...
from basemap import round0144_prompted_multilingual as contract
from basemap.artifact_identity import expected_input_signature
from experiments import prepare_round0144_queue as prepare
from experiments import round0144_nodes as nodes
...
def test_registered_tranches_and_budget_close_exactly() -> None:
    ...
    assert [item["r0087_global_row_range"] for item in contract.LANGUAGE_TRANCHES] == [
        [20_822_732, 21_658_186],
        [21_658_186, 22_493_640],
        [22_493_640, 23_329_094],
    ]
    ...
    assert contract.CAPABILITY == (
        "jina-document-multilingual-spa-swe-tha-2p506m-v1"
    )
    assert prepare.OUTPUT_NAMESPACE.endswith("multilingual-006-v1")
    assert prepare.TOKEN_LENGTH_CALIBRATION["spa_Latn"] == {
        "mean": 444.302,
        "p95": 590.0,
        "maximum": 630,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["swe_Latn"] == {
        "mean": 435.997,
        "p95": 598.0,
        "maximum": 693,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["tha_Thai"] == {
        "mean": 321.833,
        "p95": 395.05,
        "maximum": 521,
    }
...
def test_source_layout_binds_exact_r0087_rows_and_both_byte_streams(
    ...
    with pytest.raises(contract.Round0144Error, match="malformed"):
...
def test_boundary_rehash_rejects_mutated_large_input_equivalent(tmp_path) -> None:
    ...
    with pytest.raises(contract.Round0144Error, match="bytes changed"):
...
def test_preparer_materializes_three_independent_gpu_nodes_and_cpu_finalizer(
    ...
    round_file = tmp_path / "round-0144.md"
    ...
    queue_path = prepare.prepare_round0144(
        release_sha="a" * 40,
