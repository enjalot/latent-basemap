"""Bounded CPU smoke for the R0111 train -> seal -> reload -> panel path."""
...
from experiments import round0107_nodes, round0111_nodes


def test_round0111_train_seal_reload_panel_cpu_smoke(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    """Exercise every post-fit R0111 statement without allocating CUDA.
    ...
    boundary that previously failed only after a full train: seed-specific
    wrapper dispatch, accounting, profiling, model publication, receipt
    sealing, checkpoint reload, and the panel scorer entry point.
    """
    import torch
    ...
    expected_stamp = {
        "schema": "round0111-cpu-smoke-pipeline-v1",
        "pipeline": "round0111_cpu_smoke",
        "sampler_class": "Round0111CpuSmokeSampler",
    }
    runtime = {
        **expected_stamp,
    ...
    graph = {
        ...
        "signature": {
            "kind": "file",
            "canonical_path": "/data/r0111-smoke/graph-manifest.json",
            "bytes": 1,
            "sha256": "c" * 64,
        },
    ...
    class SmokeDataset:
        def __init__(self, *args, **kwargs) -> None:
            ...
            self.substrate = {
                "signature": {
                    "kind": "file",
                    "canonical_path": "/data/r0111-smoke/substrate.json",
                    "bytes": 1,
                    "sha256": "d" * 64,
                }
            }
    ...
    def smoke_train_config(**kwargs) -> tuple[dict[str, Any], str]:
        observed_config.update(kwargs)
        return config, "e" * 64

    monkeypatch.setattr(round0107_nodes, "_graph", lambda active, job: graph)
    monkeypatch.setattr(round0107_nodes, "train_config", smoke_train_config)
    monkeypatch.setattr(
        round0107_nodes,
    ...
    result = round0111_nodes.run_job(
        {
            "manifest": {
                "round_id": round0111_nodes.ROUND_ID,
                "release_sha": "a" * 40,
            }
        },
        {
    ...
    assert observed_config["seed"] == 44
    assert observed_config["schema"] == round0111_nodes.TRAIN_CONFIG_SCHEMA
    ...
    assert production_config["round_id"] == "0111"
    assert production_config["schema"] == (
        round0111_nodes.PRODUCTION_CONFIG_SCHEMA
    )
    assert production_config["config"]["optimizer"]["seed"] == 44

    with (output / "train-receipt.json").open(encoding="utf-8") as handle:
        receipt = json.load(handle)
    validate_seal(receipt, label="R0111 CPU smoke receipt")
    assert receipt["round_id"] == "0111"
    assert receipt["schema"] == round0111_nodes.TRAIN_RECEIPT_SCHEMA
    ...
    panel = score_panel(
        ...
        provenance={"round": "0111", "mode": "cpu-smoke"},
    )
