...
from experiments import prepare_round0077_queue, round0077_nodes
...
def test_qualification_loader_accepts_smallest_passing_nprobe(
    ...
    assert loaded["receipt"]["selected_nprobe"] == 48
...
def test_qualification_loader_rejects_nonminimal_selection(
    ...
    receipt["rows_by_nprobe"]["48"]["passes_mean_floor"] = True
    body = {
        key: value
    ...
    path.write_text(json.dumps(receipt), encoding="utf-8")
    signature = expected_input_signature(str(path))
    with pytest.raises(Round0077Error):
        load_gpu_qualification(
            str(path),
            expected_sha256=signature["sha256"],
            substrate_signature=substrate,
...
def test_round0077_uses_exact_registered_120m_universe() -> None:
    spec = subset_spec("120m")
    assert round0077_nodes.ROW_COUNT == 120_000_000
    assert round0077_nodes.INTERVALS == tuple(spec["intervals"])
    assert round0077_nodes.ELIGIBILITY_SUMMARY == (
        spec["eligibility_summary"]
    )
    assert round0077_nodes.QUALITY_SAMPLE_ROWS == 4_096
    assert round0077_nodes.QUALITY_SEED == 77
    assert MEAN_RECALL_FLOOR == 0.90
    assert NPROBE_GRID == (16, 24, 32, 40, 48, 64, 96, 128)


def test_round0077_is_one_bounded_no_training_gpu_job() -> None:
    source = inspect.getsource(prepare_round0077_queue.prepare_round0077)
    ...
    assert '"no_120m_quality_claim": True' in source
    assert (
        '"required_reviews"] = ["0065", "0072", "0076"]'
        in source
    )
    assert (
...
def test_round0077_accepts_only_an_issued_round(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    round_file = tmp_path / "round-0077.md"
    monkeypatch.setattr(
        prepare_round0077_queue,
        "ROUND_FILE",
        str(round_file),
    )
    round_file.write_text("---\nstatus: issued\n---\n", encoding="utf-8")
    prepare_round0077_queue._require_issued_round()
    round_file.write_text("---\nstatus: draft\n---\n", encoding="utf-8")
    with pytest.raises(RuntimeError, match="remains draft"):
        prepare_round0077_queue._require_issued_round()


def test_round0077_queries_are_normalized() -> None:
    encoded = np.asarray([[3, 4], [0, 5]], dtype=np.int8)
    scales = np.asarray([2, 3], dtype=np.float16)
    values = round0077_nodes._queries(
        encoded,
        scales,
...
def test_round0077_modules_do_not_mutate_cuda_visibility(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("CUDA_VISIBLE_DEVICES", "sentinel")
    importlib.import_module("basemap.round0077_quality")
    importlib.import_module("experiments.round0077_nodes")
    importlib.import_module("experiments.prepare_round0077_queue")
