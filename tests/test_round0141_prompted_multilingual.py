"""CUDA-hidden contract, encoder, and preparer tests for R0141."""
...
from basemap import round0141_prompted_multilingual as contract
from basemap.artifact_identity import expected_input_signature
from experiments import prepare_round0141_queue as prepare
from experiments import round0141_nodes as nodes
...
def test_registered_tranches_and_budget_close_exactly() -> None:
    ...
    assert [item["r0087_global_row_range"] for item in contract.LANGUAGE_TRANCHES] == [
        [16_645_462, 17_480_916],
        [17_480_916, 18_316_370],
        [18_316_370, 19_151_824],
    ]
    ...
    assert contract.CAPABILITY == (
        "jina-document-multilingual-jpn-kor-nld-2p506m-v1"
    )
    assert prepare.OUTPUT_NAMESPACE.endswith("multilingual-004-v1")
    assert prepare.TOKEN_LENGTH_CALIBRATION["jpn_Jpan"] == {
        "mean": 515.161,
        "p95": 683.05,
        "maximum": 731,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["kor_Hang"] == {
        "mean": 413.641,
        "p95": 545.05,
        "maximum": 612,
    }
    assert prepare.TOKEN_LENGTH_CALIBRATION["nld_Latn"] == {
        "mean": 423.832,
        "p95": 606.0,
        "maximum": 687,
    }
...
def test_source_layout_binds_exact_r0087_rows_and_both_byte_streams(
    ...
    with pytest.raises(contract.Round0141Error, match="malformed"):
...
def test_boundary_rehash_rejects_mutated_large_input_equivalent(tmp_path) -> None:
    ...
    with pytest.raises(contract.Round0141Error, match="bytes changed"):
...
def test_preparer_materializes_three_independent_gpu_nodes_and_cpu_finalizer(
    ...
    round_file = tmp_path / "round-0141.md"
    ...
    queue_path = prepare.prepare_round0141(
        release_sha="a" * 40,
